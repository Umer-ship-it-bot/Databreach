...
window.MDIGITAL.TRACKER = (function (D, e) {
  var t = function () {
    D.MDIGITAL.md_tracker = D.MDIGITAL.md_tracker || [];
    if (!MDIGITAL.md_tracker.__SV) {
      var i, o;
      MDIGITAL.md_tracker._i = [];
      MDIGITAL.md_tracker.init = function (e, t) {
        function r(e, t) {
          var r = t.split(".");
        ...
        var n = MDIGITAL.md_tracker;
        "undefined" !== typeof t
          ? (n = MDIGITAL.md_tracker[t] = [])
          : (t = "md_tracker");
        i = ["trackEvent", "trackEventLater", "flush", "set_config"];
        for (o = 0; o < i.length; o++) r(n, i[o]);
        if (e) {
          MDIGITAL.md_tracker._i.push([e, t]);
        }
      };
      MDIGITAL.md_tracker.__SV = 1.2;
    }
  };
  var r = function () {
    var t = Array.prototype,
      ...
      o = n.userAgent,
      a;
    if (n.userAgentData && n.userAgentData.platform) {
      a = n.userAgentData
    ...
    var s = "md_tracker";
    var d = (MDIGITAL.md_tracker && MDIGITAL.md_tracker["__SV"]) || 0,
      l = D.XMLHttpRequest && "withCredentials" in new XMLHttpRequest(),
      p = !l && o.indexOf("MSIE") == -1 && o.indexOf("Mozilla") == -1;
    var _ = {},
      h = { api_host: "", loaded: function () {}, http_post: true },
      v = false;
    (function () {
      var a = t.forEach,
        e = Array.isArray,
        s = {};
      var n = (_.each = function (e, t, r) {
        if (e == null) return;
        if (a && e.forEach === a) {
          e.forEach(t, r);
        } else if (e.length === +e.length) {
          for (var n = 0, i = e.length; n < i; n++) {
            if (n in e && t.call(r, e[n], n, e) === s) return;
          }
        } else {
          for (var o in e) {
            if (c.call(e, o)) {
              if (t.call(r, e[o], o, e) === s) return;
            }
          }
        }
      });
      _.extend = function (r) {
        n(i.call(arguments, 1), function (e) {
          for (var t in e) {
            if (e[t] !== void 0) r[t] = e[t];
          }
        });
        return r;
      };
      ...
      _.toArray = function (e) {
        ...
        if (_.isArray(e)) return i.call(e);
        if (_.isArguments(e)) return i.call(e);
        return _.values(e);
      };
      _.values = function (e) {
        var t = [];
    ...
    _.register_event = (function () {
      var e = function (e, t, r, n) {
        if (!e) {
          u.log("No valid element provided to register_event");
          return;
        }
        if (e.addEventListener && !n) {
      ...
      properties: function () {
        return _.extend(
          _.strip_empty_properties({
            sessionScreenSize: D.screen.width + "x" + D.screen.height,
            sessionDua: n.userAgent,
            sessionPlatform: n.platform,
            referringUrl: f.referrer,
            pageTitle: f.title,
    ...
    var m = {
      critical: function () {
        if (!_.isUndefined(u) && u) {
          var t = ["MDIGITAL.md_tracker error:"].concat(_.toArray(arguments));
          try {
            u.error.apply(u, t);
          } catch (e) {
            _.each(t, function (e) {
    ...
    var y = function (e, t) {
      var r,
        n = t === s ? MDIGITAL.md_tracker : MDIGITAL.md_tracker[t];
      if (n && !_.isArray(n)) {
        u.log("You have already initialized " + t);
        return;
      }
      r = new g();
      r._init(e, t);
    ...
    g.prototype.init = function (e, t) {
      if (typeof t === "undefined") {
        u.log("You must name your new library: init(config, name)");
        return;
      }
      if (t === s) {
        u.log(
          "You must initialize the main MDIGITAL.md_tracker object right after you include the MDIGITAL.md_tracker js snippet"
        );
        return;
      }
      var r = y(e, t);
      MDIGITAL.md_tracker[t] = r;
      r._loaded();
      return r;
    };
    g.prototype._init = function (e, t) {
      ...
      if (e.custom_event_handler) {
        this.__customEventHandler = e.custom_event_handler;
      }
    };
    g.prototype._loaded = function () {
      this.get_config("loaded")(this);
    };
    g.prototype._dom_loaded = function () {
      _.each(
        this.__dom_loaded_queue,
        function (e) {
    ...
    g.prototype._track_dom = function (e, t) {
      if (!v) {
    ...
    g.prototype.trackEventLater = function (e, t) {
      if (typeof e === "undefined") {
        u.log("No event name provided to MDIGITAL.md_tracker.trackEventLater");
        return;
      }
      t = t || {};
      var r = new Date();
      var n = _.extend({}, _.info.properties(), {
        eventName: e,
        timestamp: r.toISOString(),
        eventTimezoneOffset: -1 * (r.getTimezoneOffset() / 60),
      });
      n = _.extend(n, t);
      if (a) {
        var i = this;
        a.then(function (e) {
          n = _.extend(n, { clientHints: e });
        })
          .catch(function () {})
          .then(function () {
            i.eventsArray.push(n);
          });
      } else {
    ...
    g.prototype.flush = function () {
      var e = this.eventsArray;
      this._send_request(e);
      this.eventsArray = [];
    };
    g.prototype._onError = function (e, t) {};
    g.prototype._send_request = function (e, r) {
      if (p) {
      ...
      if (
        ...
        this.__customEventHandler.handleRequest
      ) {
        var t = null;
        if (l) {
          t = "post";
        }
        this.__customEventHandler.handleRequest(this, e, t);
        return;
      }
      var n = this.get_config("api_host");
      var i = this.get_config("topic");
      var o = JSON.stringify(e);
      if (l) {
        var a = new XMLHttpRequest();
        a.open("POST", n, true);
        a.setRequestHeader("Content-type", "application/json");
        a.setRequestHeader("charset", "UTF-8");
        if (i) {
          a.setRequestHeader("Mec-Integration-Id", i);
        }
        a.onreadystatechange = function (e) {
          if (a.readyState === 4) {
            if (a.status === 200) {
              ...
              var t = "Bad HTTP status: " + a.status + " " + a.statusText;
              u.log(t);
            }
          }
        };
        ...
        var s = this;
        e = { data: _.base64Encode(o) };
        if (i) {
          e["mecIntegrationId"] = _.base64Encode(i);
        }
        n += "/da.gif?" + _.HTTPBuildQuery(e);
    ...
    g.prototype._execute_array = function (e) {
      var t,
        r = [],
        n = [],
        i = [];
        ...
        function (e) {
          if (e) {
            ...
            if (typeof e === "function") {
              e.call(this);
            } else if (_.isArray(e) && t === "alias") {
              r.push(e);
            } else if (
              _.isArray(e) &&
              (t.indexOf("track") != -1 || t.indexOf("flush") != -1) &&
    ...
    g.prototype.trackEvent = function (e, t, r) {
      if (typeof e === "undefined") {
        u.log("No event name provided to MDIGITAL.md_tracker.trackEvent");
        return;
      }
      t = t || {};
      var n = new Date();
      var i = _.extend({}, _.info.properties(), {
        eventName: e,
        timestamp: n.toISOString(),
        eventTimezoneOffset: -1 * (n.getTimezoneOffset() / 60),
      });
      i = _.extend(i, t);
      if (a) {
        a.then(function (e) {
          i = _.extend(i, { clientHints: e });
        })
          .catch(function () {})
          .then(this._send_events.bind(this, i, r));
      } else {
        this._send_events(i, r);
      }
    };
    g.prototype._send_events = function (e, t) {
      this._send_request([e], t);
    };
    g.prototype.set_config = function (e) {
      if (_.isObject(e)) {
        _.extend(this["config"], e);
      }
    };
    g.prototype.get_config = function (e) {
    ...
    _["info"] = _.info;
    g.prototype["init"] = g.prototype.init;
    g.prototype["track_event"] = g.prototype.trackEvent;
    g.prototype["track_event_later"] = g.prototype.trackEventLater;
    g.prototype["flush"] = g.prototype.flush;
    g.prototype["set_config"] = g.prototype.set_config;
    g.prototype["get_config"] = g.prototype.get_config;
    g.prototype["toString"] = g.prototype.toString;
    if (_.isUndefined(MDIGITAL.md_tracker)) {
      m.critical(
        "'MDIGITAL.md_tracker' object not initialized. Ensure you are using the latest version of the MDIGITAL.md_tracker JS Library along with the snippet we provide."
      );
      return;
    }
    if (MDIGITAL.md_tracker["__loaded"] || MDIGITAL.md_tracker["config"]) {
      u.log(
        "MDIGITAL.md_tracker library has already been downloaded at least once."
      );
      return;
    }
    if (d < 1.1) {
      u.critical(
        "Version mismatch; please ensure you're using the latest version of the MDIGITAL.md_tracker code snippet."
      );
      return;
    }
    var I = {};
    _.each(MDIGITAL.md_tracker["_i"], function (e) {
      var t, r;
      if (e && _.isArray(e)) {
        t = e[e.length - 1];
        r = y.apply(this, e);
    ...
    var A = function () {
      _.each(I, function (e, t) {
        if (t !== s) {
          MDIGITAL.md_tracker[t] = e;
        }
      });
      MDIGITAL.md_tracker["_"] = _;
    };
    MDIGITAL.md_tracker["init"] = function (e, t) {
      if (t) {
        if (!MDIGITAL.md_tracker[t]) {
          MDIGITAL.md_tracker[t] = I[t] = y(e, t);
          MDIGITAL.md_tracker[t]._loaded();
        }
      } else {
        var r = MDIGITAL.md_tracker;
        if (I[s]) {
          r = I[s];
        } else {
          r = y(e, s);
        ...
        D[s] = MDIGITAL.md_tracker = r;
        A();
      }
    };
    MDIGITAL.md_tracker["init"]();
    _.each(I, function (e) {
      e._loaded();
    });
    function k() {
    ...
    if (f.addEventListener) {
      ...
      f.attachEvent("onreadystatechange", k);
      var T = false;
      try {
        T = D.frameElement == null;
      } catch (e) {}
      if (f.documentElement.doScroll && T) {
        function L() {
          try {
    ...
    _.register_event(D, "load", k, true);
  };
  return { loadBaseTracker: t, loadFullTracker: r };
})(window, document);
(function (window) {
  window.MDIGITAL = window.MDIGITAL || {};
  var isGenericAlreadyLoaded = !!window.MDIGITAL.version;
  if (isGenericAlreadyLoaded) {
    var MD_ANALYTICS = window.MDIGITAL.ANALYTICS || MDIGITAL_ANALYTICS;
    if (
      MD_ANALYTICS &&
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DIGITAL_ANALYTICS
      ) &&
      window.MDIGITAL.md_tracker
    ) {
      MD_ANALYTICS.sendDigitalAnalytics({
        eventId: "MDigital_multipleGenericInjections",
        mdData: {
          additionalInfo: `version 2.60.2 was loaded after ${window.MDIGITAL.version}`,
        },
      });
    }
    throw new Error("MDIGITAL already loaded");
  }
  window.MDIGITAL.version = "2.60.2";
})(window);
window.KAMPYLE_CONSTANT = {
  PLACEHOLDERS: {
    ...
    USER_AGENT_CLIENT_HINTS_LIST:
      '["brands", "mobile", "platform", "architecture", "model", "bitness", "platformVersion", "fullVersionList", "wow64", "uaFullVersion"]',
    DIGITAL_ANALYTICS_STATIC_EVENTS_LIST: "[]",
    DEFAULT_ANALYTICS_SUBMIT_EVENTS_URL:
      "https://analytics-fe.digital-cloud.medallia.com/api/web/events",
    DEFAULT_MEC_INTEGRATION_ID: "target-apis-medallia-com-target",
  },
  SRC: {
    OPEN_SANS:
      "resources/form/FormBuilder/vendor/fonts-googleapis/css/open-sans-v14.css",
  ...
  PROVISIONS: {
    analyticsNewEvents: "cooladataNewEvents",
    ...
    ENABLE_SPA_OVERFLOW_FIX: "24dcr1_MPC7514_enableSPAOverflowFix",
    SERVER_BASED_TARGETING: "serverBasedTargeting",
    ANALYTICS_EVNETS_LIMIT: "cooladataEventsLimit",
    ...
    PREVENT_IFRAME_SCROLLBAR_FLICKERING:
      "20dcr32_NM11146_PreventIFrameScrollFlickering",
    BACKEND_GENERATED_UUID: "20dcr4_MPC2802_enable_backendGeneratedUUID",
    CHTML_LOGS: "chtmlLogs",
    EXEMPT_FORMS_FROM_QUARANTINE:
      "20dcr4_MPC2952_enable_exemptFormsFromQuarantine",
    ...
    GREATER_SMALLER_THAN_DECODE:
      "21dcr1_MPC3370_EnableGreaterSmallerThanSupport",
    NEW_MODAL_DIALOGS: "21dcr3_MPC3281_enable_newModalDialogs",
    IS_CHTML_HANDLE_INIT_FOCUS_INDEPENDENTLY:
      "21dcr3_MPC3281_enable_CHTMLHandleFocusIndependently",
    ...
    NOT_SCROLL_TO_LAST_POSITION_DISPLAY_TYPE_ANIMATION:
      "willNotScrollToLastPositionDisplayTypeAnimation",
    CLEAR_EMBEDDED_ON_UPDATE_PAGE:
      "21dcr2_MPC3571_enable_ClearEmbeddedFormOnUpdatePageView",
    ENABLE_EVENTS_FOR_CE_DEBUG: "21DCR2_MPC3662_EnableEventsForCEDebug",
    EXPOSE_SELECTION_QUESTIONS:
      "21DCR3_MPC3866_exposeSelectionQuestionsInCustomEvent",
    EXPOSE_VERBATIM_QUESTIONS:
      "21DCR3_MPC3866_exposeVerbatimQuestionsInCustomEvent",
    ...
    ENABLE_ZOOM_WCAG_REFLOW: "23dcr2_mpc36711_enableFormWCAGReflow",
    ENABLE_ANALYTICS_SESSION_EVENTS: "23dcr2_MPC6869_enableCXJSessionEvents",
    DISABLE_ANANLYTICS_NEW_PAGE_EVENTS: "23dcr2_MPC6869_disableNewPageEvents",
    ENABLE_INVITATION_SPINNER_FIX: "23dcr4_MPC7186_enableInvitationSpinnerFix",
    DECIBEL_INSIGHT: "decibelInsight",
    ENABLE_VIEWPORT_LOGIC_FIX: "24dcr2_MPC7960_enableViewportLogicFix",
    ENABLE_DIGITAL_ANALYTICS: "24dcr1_MPC_7567_enableDigitalFeedbackAnalytics",
    ENABLE_ONSITE_POST_MESSAGE_VERIFICATION:
      "24dcr2_MPC7960_enableOnsitePostMessageVerification",
    ENABLE_DYNAMIC_ANALYTICS_EVENTS:
      "25dc1_MPC8353_enableDynamicAnalyticsEvents",
    DISABLE_EXTRA_FEEDBACK_SUBMIT_EVENT:
      "25dcr1_MPC8593_disableExtraFeedbackSubmitEvent",
    ENABLE_DYNAMIC_FEEDBACK_BUTTON_SIZE:
      "25dcr2_MPC8565_enableDynamicFeedbackButtonSize",
    DISABLE_USER_PERCENTILE_AT_CREATE_SESSION:
      "25dcr2_MPC9116_disableUserPercentileAtCreateSession",
  },
  ...
  EVENTS: {
    ...
    MDIGITAL_BEHAVIORAL_TARGETING_PHASE_ENDED:
      "MDigital_behavioralTargetingPhaseEnded",
    MDIGITAL_QUARANTINED_SURVEY_PHASE_ENDED:
      "MDigital_quarantinedSurveyPhaseEnded",
    ...
    MDIGITAL_COULD_NOT_LOAD_FORM_APPLICATION:
    ...
    MDIGITAL_DYNAMIC_ONSITE_INITIALIZED_SUCCESSFULLY:
    ...
    DIRECT_GENERIC_INJECTION: "MDigital_directGenericInjection",
  },
  LOG_MESSAGES: {
    TRIGGER_MESSAGE_TEMPLATE:
      "Event {{EVENT_NAME}} has been {{STATUS}}. \n the data is: {{DATA}}. \n the context is :{{CONTEXT}}. \n the options are : {{OPTIONS}}.\n",
    ERROR_LOG_TEMPLATE: "Error Message is {{ERROR_MESSAGE}}",
    ERROR_DYNAMIC_ONSITE_INITIALIZATION: "Failed to fetch onsite data.",
    ERROR_ONSITE_SDK_CONFIGURATION_MISSING:
      "Failed to execute KAMPYLE_ONSITE_SDK.{{METHOD}}, due to missing onsite configuration.",
  },
  ...
  TARGETING_RULES: {
    INVITE_IF_ANY_SURVEY_ALREADY_SUBMITTED_PER_SESSION:
      "InviteAnySurveyAlreadySubmittedPerSession",
    INVITE_IF_ANY_INVITE_ALREADY_SHOWN_PER_SESSION: "InvitePerSession",
  },
  REGIONS: { DEV: "dev", QA: "qa", COLO_CRT_QA1: "digital-cloud-crt-qa1" },
  MEMORY_DATA_FIELDS: {
    ...
    EX_AUTHORIZATION_TOKEN: "md_ex_AuthorizationToken",
  },
  SESSION_DATA_FIELDS: {
  ...
  TEXTS: {
    ...
    SCREEN_CAPTURE: {
      ...
      DEFAULT_DESCRIPTION:
        'Click "Capture Image" button or drag the cursor to highlight a specific area',
    },
  },
  ...
  PROPERTY_MESSAGES: {
    NO_JSON_MESSAGE: {
      TYPE: "noJsonMessage",
      KEYS: { TITLE: "title", BODY: "body", CLOSE: "close" },
    },
  },
  ...
  ANALYTICS_HTTP_EVENTS_BY_METHOD: {
  ...
  BUTTON: { MAX_WIDTH: 300, MIN_HEIGHT: 81 },
...
window.MDIGITAL.CONFIGURATION = (function (window, document) {
  function init() {
    KAMPYLE_DATA.setMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.HAS_ONSITE_CONFIGURATION,
      false
    );
    var basicConfig = getOnsiteConfiguration();
    var region =
      (basicConfig && basicConfig.region) || KAMPYLE_CONSTANT.REGIONS.DEV;
    KAMPYLE_DATA.setMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.REGION,
      region
    );
  }
  function isOnsiteConfigurationMissing() {
    return !KAMPYLE_DATA.getMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.HAS_ONSITE_CONFIGURATION
    );
  }
  function getOnsiteConfiguration() {
    var dataJson = KAMPYLE_DATA.getMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.DATA_JSON
    );
    if (dataJson) {
      return dataJson;
    }
    return {
      provisions: {
        ...
        "24dcr2_MPC7960_enableOnsitePostMessageVerification": null,
        ...
        customCssUpload: null,
        willNotScrollToLastPositionDisplayTypeAnimation: null,
        cooladataNewEvents: null,
        "20dcr4_MPC2893_enable_ScreenCaptureAsComponent": true,
        adobeAnalytics: null,
        eventIncludeFeedbackContent: true,
        dynamicModalSizeInCustomHtml: null,
        "23dcr2_mpc36711_enableFormWCAGReflow": true,
        "24dcr3_MPC7720_enableNewLiveFormCSP": null,
        customHtmlFormsEnable: true,
        propertyMessages: true,
        cooladataEventsLimit: null,
        "21DCR5_MPC4030_EnableFixForSpaTimeOnPage": true,
        "25dcr1_MPC8593_disableExtraFeedbackSubmitEvent": true,
        targetingEventsReporting: true,
        "23dcr2_MPC6869_disableNewPageEvents": true,
        "21DCR3_MPC3866_exposeSelectionQuestionsInCustomEvent": true,
        ...
        "25dcr2_MPC8565_enableDynamicFeedbackButtonSize": null,
        enableOverflow: null,
        doNotSurveyOnUBTNetworkFail: null,
        "23dcr4_MPC7186_enableInvitationSpinnerFix": null,
        ...
        "21dcr3_mpc3662_delaySurveyUntilCSSLoad": null,
        mobileWebDynamicFormSizeInFormBuilder: null,
        cookieStorageDisable: null,
        "25dcr2_MPC9116_disableUserPercentileAtCreateSession": null,
        screenCaptureEnabled: true,
        skipInvitationOptimization: null,
        "23dcr5_MPC7566_extractProvisionsToOnsiteData": null,
        reusableResourcesLibrary: null,
        formTemplatesWCAG: true,
        formSpacing: null,
        "24dcr1_MPC7724_enableFormv2andInviteCSSDelay": null,
        ...
        "24dcr4_MPC8375_enableFeedbackPayloadOptimization": null,
        "22dcr2_MPC4277_useDecibelTriggers": null,
        "23dcr2_MPC6711_enableWCAGerrorMessagesPreQuestions": null,
        "21dcr6_MPC4296_enable_UBTIdentifierHash": null,
        onPremHybrid: null,
        enableZeroRating: null,
        responsiveRatingComponent: null,
        "23dcr3_MPC6996_enableOnsiteDomainApproval": null,
        "25dcr2_MPC9116_enableInviteWCAGReflow": true,
        "21dcr3_MPC3281_enable_newModalDialogs": true,
        "23dcr3_MPC4831_enableEncapsulation": null,
        UBTExtraEvents: null,
        adobeAnalyticsObjectName: null,
        mobileWebDynamicFormSize: true,
        "23dcr5_MPC7505_enableDescriptiveRequiredText": null,
        decibelInsight: null,
        ...
        "24dcr4_MPC8160_enableMultipleSubmitFix": null,
        "24dcr1_MPC_7567_enableDigitalFeedbackAnalytics": true,
        "24dcr1_MPC7514_enableSPAOverflowFix": true,
        ...
        "24dcr2_MPC7960_enableViewportLogicFix": true,
        "23dcr2_MPC6869_enableCXJSessionEvents": null,
        "25dc1_MPC8353_enableDynamicAnalyticsEvents": null,
        parallelTabSolution: null,
      },
      sbtServerDomain: "https://ubt-lb.digital-cloud.medallia.com",
      sbtEnvPrefix: "wdcus_",
      endUserIdentifier: {},
      formNodes: [
        {
          ...
          urlVersion: "V2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/19064/formDataV2_1756321036908_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/19064/formData1756321036908_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/19064/formDataV2_1756321036908_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/19064/invitationData1753175404216_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
            '[{"id":"8466","unique_name":"Child TCIN","type":"Number","source":"URL","source_name":"preselect","is_identifier":"false"}, {"id":"7138","unique_name":"Chat Origin","type":"Text","source":"Var","source_name":"window.chat_origin","is_identifier":"false"}, {"id":"7040","unique_name":"page","type":"Text","source":"URL","source_name":"page","is_identifier":"false"}, {"id":"7039","unique_name":"cartType","type":"Text","source":"URL","source_name":"cartType","is_identifier":"false"}, {"id":"6468","unique_name":"Chat ID","type":"Text","source":"Cookie","source_name":"LSKey-c$chat_id","is_identifier":"false"}, {"id":"6464","unique_name":"contained_chat_ended","type":"Boolean","source":"Var","source_name":"window.chat_ended","is_identifier":"false"}, {"id":"4793","unique_name":"feature","type":"Text","source":"Var","source_name":"medallia_innovation_survey.feature","is_identifier":"false"}, {"id":"4792","unique_name":"innovation_product","type":"Text","source":"Var","source_name":"medallia_innovation_survey.product","is_identifier":"false"}, {"id":"4791","unique_name":"feature old","type":"Text","source":"Var","source_name":"feature","is_identifier":"false"}, {"id":"4790","unique_name":"location.href","type":"Text","source":"Var","source_name":"location.href","is_identifier":"false"}, {"id":"4785","unique_name":"UrCapture","type":"Text","source":"Cookie","source_name":"UrCapture","is_identifier":"false"}, {"id":"4784","unique_name":"searchTerm2","type":"Text","source":"Var","source_name":"location.search","is_identifier":"false"}, {"id":"4783","unique_name":"searchTerm","type":"Text","source":"URL","source_name":"searchTerm","is_identifier":"false"}, {"id":"4782","unique_name":"GR guest type","type":"Text","source":"cookie","source_name":"GRInterceptIndicator","is_identifier":"false"}, {"id":"4781","unique_name":"registryType","type":"Text","source":"url","source_name":"registryType","is_identifier":"false"}, {"id":"4780","unique_name":"registryId","type":"Text","source":"url","source_name":"registryId","is_identifier":"false"}, {"id":"4779","unique_name":"GuestID","type":"Text","source":"cookie","source_name":"mid","is_identifier":"false"}, {"id":"4778","unique_name":"isRestock","type":"Text","source":"url","source_name":"mode","is_identifier":"false"}, {"id":"4777","unique_name":"GuestLocation","type":"Text","source":"cookie","source_name":"UserLocation","is_identifier":"false"}, {"id":"4776","unique_name":"AB Test","type":"Number","source":"Var","source_name":"medallia_ab","is_identifier":"false"}, {"id":"4775","unique_name":"cardType","type":"Text","source":"URL","source_name":"cardType","is_identifier":"false"}, {"id":"4774","unique_name":"OrderNumber","type":"Number","source":"URL","source_name":"orderNumber","is_identifier":"false"}, {"id":"4773","unique_name":"SDD_review","type":"Text","source":"Var","source_name":"medalliaData.review","is_identifier":"false"}, {"id":"4772","unique_name":"SDD_rating","type":"Number","source":"Var","source_name":"medalliaData.rating","is_identifier":"false"}, {"id":"4771","unique_name":"SDD_tip","type":"Number","source":"Var","source_name":"medalliaData.tip","is_identifier":"false"}, {"id":"4770","unique_name":"SDD_TeamMemberID","type":"Number","source":"Var","source_name":"medalliaData.teamMemberId","is_identifier":"false"}, {"id":"4769","unique_name":"InnovationOrderID","type":"Number","source":"Var","source_name":"medallia_innovation_survey.orderId","is_identifier":"false"}, {"id":"4768","unique_name":"innovation_action","type":"Text","source":"Var","source_name":"medallia_innovation_survey.action","is_identifier":"false"}, {"id":"4766","unique_name":"s_vi","type":"Text","source":"Var","source_name":"medalliaParams.s_vi","is_identifier":"false"}, {"id":"4765","unique_name":"visitorId","type":"Text","source":"cookie","source_name":"visitorId","is_identifier":"false"}, {"id":"4764","unique_name":"TLSessionID","type":"Text","source":"cookie","source_name":"TealeafAkaSid","is_identifier":"false"}, {"id":"4763","unique_name":"ref_url","type":"Text","source":"Var","source_name":"medalliaParams.ref_url","is_identifier":"false"}, {"id":"4762","unique_name":"abtestCampaign","type":"Text","source":"Var","source_name":"medalliaParams.abtestCampaign","is_identifier":"false"}]',
          formType: "invitation",
          formDisplay: {
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/19064/invitation1753175404186.html",
            inviteHeight: "400",
            inviteWidth: "700",
            skipInvitation: "true",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/19064/invitationData1753175404216_###LANGUAGE_CODE###.json",
          },
          onSiteData: {
            genericRule:
              '{"type":"criteriaGroup","conjunction":"AND","childrenCriterias":[{"type":"criteria","value":"true","formId":-1,"fieldId":6464,"formName":"All Forms","uniqueId":"customParam6464","condition":"equals","fieldName":"contained_chat_ended","fieldType":"Boolean","fieldOrigin":"customParam"}]}',
            kampyleInvitePerSession: { inviteOncePerSession: "false" },
            kampyleInvitePercentageUsers: { prec: "100.0" },
            devicetypes: ["desktop", "mobile", "tablet"],
          },
          urlRules: {
            exclusionUrl: { rule: "https://www.target.com/" },
            inclusionUrl: {
              rule: "https://www.localhost.ephemeral.target.com;https://localhost.ephemeral.target.com;http://ephemeral.target.com;https://www-stage.target.com/",
            },
          },
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15790/formDataV2_1756409602325_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15790/formData1756409602325_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15790/formDataV2_1756409602325_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/15790/invitationData1753175422932_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15790/invitation1753175422899.html",
            inviteHeight: "400",
            inviteWidth: "700",
            skipInvitation: "false",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15790/invitationData1753175422932_###LANGUAGE_CODE###.json",
          },
          onSiteData: {
            genericRule:
              '{"type":"criteriaGroup","conjunction":"AND","childrenCriterias":[{"type":"criteria","value":"5","formId":-1,"fieldId":4776,"formName":"All Forms","uniqueId":"customParam4776","condition":"greaterThan","fieldName":"AB Test","fieldType":"Number","fieldOrigin":"customParam"},{"type":"criteria","value":"|US","formId":-1,"fieldId":4777,"formName":"All Forms","uniqueId":"customParam4777","condition":"contains","fieldName":"GuestLocation","fieldType":"Text","fieldOrigin":"customParam"}]}',
            numberOfPagesViewed: {
              numberOfRepeats: "2",
            ...
            submitted: { days: "90" },
            declined: { days: "90" },
            kampyleTimeInSession: { time: "180" },
            kampyleInvitePerSession: { inviteOncePerSession: "false" },
            kampyleInvitePercentageUsers: { prec: "15.0" },
            kampyleTimeOnPage: { time: "30" },
            devicetypes: ["mobile", "tablet"],
          },
          urlRules: {
            exclusionUrl: {
              rule: "/gc;/cart;/order-confirmation;/checkout;http://weeklyad.target.com/;/c/terms-conditions/-/N-4sr7l;/c/target-privacy-policy/-/N-4sr7p;/c/california-residents-privacy-policy/-/N-m2wjt;/c/interest-based-advertising-privacy-policy/-/N-ztavm;/guest-privacy/your-privacy-choices-ca;/account;/account/giftcards;/account/giftcards/new;/account/giftcards/&ltcardId&gt/card-activity;/account/memberships;/account/payments;/account/payments/new;/account/payments/&ltcardId&gt/edit;/account/rateandreview;/account/savings;/account/addresses;/account/addresses/new;/account/addresses/&ltaddressId&gt/edit;/account/store;/account/settings/aboutme;/account/settings/signinsecurity;/account/settings/verification/student;/account/settings/verification/teacher;/account/settings/verification/military;/account/settings/contactpreferences;/account/settings;/account/shoppingpartners;/account/viewhistory;/orders;/orders/concierge;/c/team-member-services/-/N-4srq9;https://help.target.com/help;https://contactus.target.com/ContactUs;/gift-registry/create-registry;/gift-registry/search-results;/returns;/guest/return-label;https://www.target.com/gsp/static/v1/;https://weeklyad.target.com/;/s/;https://www.target.com/weekly-ad",
            },
            inclusionUrl: { rule: "" },
          },
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15788/formDataV2_1756409621982_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15788/formData1756409621982_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15788/formDataV2_1756409621982_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/15788/invitationData1753175386241_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15788/invitation1753175386210.html",
            inviteHeight: "340",
            inviteWidth: "580",
            skipInvitation: "false",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15788/invitationData1753175386241_###LANGUAGE_CODE###.json",
          },
          onSiteData: {
            genericRule:
              '{"type":"criteriaGroup","conjunction":"AND","childrenCriterias":[{"type":"criteria","value":"5","formId":-1,"fieldId":4776,"formName":"All Forms","uniqueId":"customParam4776","condition":"greaterThan","fieldName":"AB Test","fieldType":"Number","fieldOrigin":"customParam"},{"type":"criteria","value":"|US","formId":-1,"fieldId":4777,"formName":"All Forms","uniqueId":"customParam4777","condition":"contains","fieldName":"GuestLocation","fieldType":"Text","fieldOrigin":"customParam"},{"type":"criteria","value":"91","formId":-1,"fieldId":4776,"formName":"All Forms","uniqueId":"customParam4776","condition":"smallerThan","fieldName":"AB Test","fieldType":"Number","fieldOrigin":"customParam"}]}',
            numberOfPagesViewed: {
              numberOfRepeats: "2",
              compareString: "greaterThan",
            },
            submitted: { days: "180" },
            declined: { days: "90" },
            kampyleTimeInSession: { time: "60" },
            kampyleInvitePerSession: { inviteOncePerSession: "false" },
            kampyleInvitePercentageUsers: { prec: "12.0" },
            kampyleTimeOnPage: { time: "10" },
            devicetypes: ["desktop"],
          },
          urlRules: {
            exclusionUrl: {
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15770/formDataV2_1756409467470_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15770/formData1756409467470_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15770/formDataV2_1756409467470_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/15770/invitationData1753175397440_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15770/invitation1753175397386.html",
            inviteHeight: "400",
            inviteWidth: "700",
            skipInvitation: "false",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15770/invitationData1753175397440_###LANGUAGE_CODE###.json",
          },
          onSiteData: {
            genericRule:
              '{"type":"criteriaGroup","conjunction":"AND","childrenCriterias":[{"type":"criteria","value":"5","formId":-1,"fieldId":4776,"formName":"All Forms","uniqueId":"customParam4776","condition":"greaterThan","fieldName":"AB Test","fieldType":"Number","fieldOrigin":"customParam"},{"type":"criteria","value":"|US","formId":-1,"fieldId":4777,"formName":"All Forms","uniqueId":"customParam4777","condition":"contains","fieldName":"GuestLocation","fieldType":"Text","fieldOrigin":"customParam"}]}',
            ...
            kampyleTimeInSession: { time: "60" },
            kampyleInvitePerSession: { inviteOncePerSession: "false" },
            kampyleInvitePercentageUsers: { prec: "10.0" },
            kampyleTimeOnPage: { time: "15" },
            devicetypes: ["mobile", "tablet"],
          },
          urlRules: {
            exclusionUrl: {
              rule: "/gc;https://www.target.com/gsp/static/v1/;/account/payments/&ltcardId&gt/edit;/account/addresses/&ltaddressId&gt/edit",
            },
            inclusionUrl: {
              rule: "/account;/account/giftcards;/account/giftcards/new;/account/giftcards/&ltcardId&gt/card-activity;/account/memberships;/account/payments;/account/payments/new;/account/rateandreview;/account/savings;/account/addresses;/account/addresses/new;/account/store;/account/settings/aboutme;/account/settings/signinsecurity;/account/settings/verification/student;/account/settings/verification/teacher;/account/settings/verification/military;/account/settings/contactpreferences;/account/settings;/account/shoppingpartners;/account/viewhistory;/orders",
            },
          },
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15769/formDataV2_1756409450702_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15769/formData1756409450702_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15769/formDataV2_1756409450702_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/15769/invitationData1753175404031_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15769/invitation1753175403999.html",
            inviteHeight: "340",
            inviteWidth: "580",
            skipInvitation: "false",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15769/invitationData1753175404031_###LANGUAGE_CODE###.json",
          },
          onSiteData: {
            genericRule:
              '{"type":"criteriaGroup","conjunction":"AND","childrenCriterias":[{"type":"criteria","value":"5","formId":-1,"fieldId":4776,"formName":"All Forms","uniqueId":"customParam4776","condition":"greaterThan","fieldName":"AB Test","fieldType":"Number","fieldOrigin":"customParam"},{"type":"criteria","value":"|US","formId":-1,"fieldId":4777,"formName":"All Forms","uniqueId":"customParam4777","condition":"contains","fieldName":"GuestLocation","fieldType":"Text","fieldOrigin":"customParam"}]}',
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15768/formDataV2_1756409564739_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15768/formData1756409564739_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15768/formDataV2_1756409564739_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/15768/invitationData1753175403518_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15768/invitation1753175403487.html",
            inviteHeight: "400",
            inviteWidth: "700",
            skipInvitation: "false",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15768/invitationData1753175403518_###LANGUAGE_CODE###.json",
          },
          onSiteData: {
            genericRule:
              '{"type":"criteriaGroup","conjunction":"AND","childrenCriterias":[{"type":"criteria","value":"5","formId":-1,"fieldId":4776,"formName":"All Forms","uniqueId":"customParam4776","condition":"greaterThan","fieldName":"AB Test","fieldType":"Number","fieldOrigin":"customParam"}]}',
            numberOfPagesViewed: {
              numberOfRepeats: "2",
              compareString: "greaterThan",
            },
            ...
            kampyleTimeInSession: { time: "60" },
            kampyleInvitePerSession: { inviteOncePerSession: "false" },
            kampyleInvitePercentageUsers: { prec: "8.0" },
            kampyleTimeOnPage: { time: "10" },
            devicetypes: ["mobile", "tablet"],
          },
          urlRules: {
            exclusionUrl: { rule: "/gc;https://www.target.com/gsp/static/v1/" },
            inclusionUrl: {
              rule: "https://weeklyad.target.com/;https://www.target.com/weekly-ad",
            },
          },
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15767/formDataV2_1756409194905_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15767/formData1756409194905_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15767/formDataV2_1756409194905_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/15767/invitationData1753175395225_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15767/invitation1753175395184.html",
            inviteHeight: "340",
            inviteWidth: "580",
            skipInvitation: "false",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15767/invitationData1753175395225_###LANGUAGE_CODE###.json",
          },
          urlRules: {
            exclusionUrl: { rule: "/gc;https://www.target.com/gsp/static/v1/" },
            inclusionUrl: {
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15734/formDataV2_1756703604116_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15734/formData1756703604116_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15734/formDataV2_1756703604116_###LANGUAGE_CODE###.json",
          inviteJsonRelativePath:
            "wdcus/235967/forms/15734/invitationData1753175404031_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
          ...
          inviteData: {
            inviteUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15734/invitation1753175403999.html",
            inviteHeight: "400",
            inviteWidth: "700",
            skipInvitation: "true",
            invitationPosition: "top",
            inviteDataUrl:
              "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15734/invitationData1753175404031_###LANGUAGE_CODE###.json",
          },
          onSiteData: {
            genericRule:
              '{"type":"criteriaGroup","conjunction":"AND","childrenCriterias":[{"type":"criteria","value":"true","formId":-1,"fieldId":6464,"formName":"All Forms","uniqueId":"customParam6464","condition":"equals","fieldName":"contained_chat_ended","fieldType":"Boolean","fieldOrigin":"customParam"}]}',
          ...
          urlRules: {
            exclusionUrl: {
              rule: "/gc;/cart;/order-confirmation;/checkout;http://weeklyad.target.com/;/c/terms-conditions/-/N-4sr7l;/c/target-privacy-policy/-/N-4sr7p;/c/california-residents-privacy-policy/-/N-m2wjt;/c/interest-based-advertising-privacy-policy/-/N-ztavm;/guest-privacy/your-privacy-choices-ca;/c/team-member-services/-/N-4srq9;https://www.localhost.ephemeral.target.com;https://localhost.ephemeral.target.com;http://ephemeral.target.com;https://www-stage.target.com/",
            },
            inclusionUrl: { rule: "" },
          },
        },
        {
          ...
          urlVersion: "V2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.3/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/22013/formDataV2_1759260346344_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/22013/formData1759260346344_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/22013/formDataV2_1759260346344_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/21628/formDataV2_1756320969507_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/21628/formData1756320969507_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/21628/formDataV2_1756320969507_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/18558/formDataV2_1756409538830_###LANGUAGE_CODE###.json",
          formWidth: "450",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/18558/formData1756409538830_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/18558/formDataV2_1756409538830_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/17153/formDataV2_1756408956616_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/17153/formData1756408956616_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/17153/formDataV2_1756408956616_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          urlVersion: "V2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.18.2/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15806/formDataV2_1724438320752_###LANGUAGE_CODE###.json",
          formWidth: "450",
          formHeight: "450",
          isFixedSize: "false",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15806/formData1724438320752_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15806/formDataV2_1724438320752_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
            '[{"id":"8466","unique_name":"Child TCIN","type":"Number","source":"URL","source_name":"preselect","is_identifier":"false"}, {"id":"7138","unique_name":"Chat Origin","type":"Text","source":"Var","source_name":"window.chat_origin","is_identifier":"false"}, {"id":"7040","unique_name":"page","type":"Text","source":"URL","source_name":"page","is_identifier":"false"}, {"id":"7039","unique_name":"cartType","type":"Text","source":"URL","source_name":"cartType","is_identifier":"false"}, {"id":"6468","unique_name":"Chat ID","type":"Text","source":"Cookie","source_name":"LSKey-c$chat_id","is_identifier":"false"}, {"id":"6464","unique_name":"contained_chat_ended","type":"Boolean","source":"Var","source_name":"window.chat_ended","is_identifier":"false"}, {"id":"4793","unique_name":"feature","type":"Text","source":"Var","source_name":"medallia_innovation_survey.feature","is_identifier":"false"}, {"id":"4792","unique_name":"innovation_product","type":"Text","source":"Var","source_name":"medallia_innovation_survey.product","is_identifier":"false"}, {"id":"4791","unique_name":"feature old","type":"Text","source":"Var","source_name":"feature","is_identifier":"false"}, {"id":"4790","unique_name":"location.href","type":"Text","source":"Var","source_name":"location.href","is_identifier":"false"}, {"id":"4785","unique_name":"UrCapture","type":"Text","source":"Cookie","source_name":"UrCapture","is_identifier":"false"}, {"id":"4784","unique_name":"searchTerm2","type":"Text","source":"Var","source_name":"location.search","is_identifier":"false"}, {"id":"4783","unique_name":"searchTerm","type":"Text","source":"URL","source_name":"searchTerm","is_identifier":"false"}, {"id":"4782","unique_name":"GR guest type","type":"Text","source":"cookie","source_name":"GRInterceptIndicator","is_identifier":"false"}, {"id":"4781","unique_name":"registryType","type":"Text","source":"url","source_name":"registryType","is_identifier":"false"}, {"id":"4780","unique_name":"registryId","type":"Text","source":"url","source_name":"registryId","is_identifier":"false"}, {"id":"4779","unique_name":"GuestID","type":"Text","source":"cookie","source_name":"mid","is_identifier":"false"}, {"id":"4778","unique_name":"isRestock","type":"Text","source":"url","source_name":"mode","is_identifier":"false"}, {"id":"4777","unique_name":"GuestLocation","type":"Text","source":"cookie","source_name":"UserLocation","is_identifier":"false"}, {"id":"4776","unique_name":"AB Test","type":"Number","source":"Var","source_name":"medallia_ab","is_identifier":"false"}, {"id":"4775","unique_name":"cardType","type":"Text","source":"URL","source_name":"cardType","is_identifier":"false"}, {"id":"4774","unique_name":"OrderNumber","type":"Number","source":"URL","source_name":"orderNumber","is_identifier":"false"}, {"id":"4773","unique_name":"SDD_review","type":"Text","source":"Var","source_name":"medalliaData.review","is_identifier":"false"}, {"id":"4772","unique_name":"SDD_rating","type":"Number","source":"Var","source_name":"medalliaData.rating","is_identifier":"false"}, {"id":"4771","unique_name":"SDD_tip","type":"Number","source":"Var","source_name":"medalliaData.tip","is_identifier":"false"}, {"id":"4770","unique_name":"SDD_TeamMemberID","type":"Number","source":"Var","source_name":"medalliaData.teamMemberId","is_identifier":"false"}, {"id":"4769","unique_name":"InnovationOrderID","type":"Number","source":"Var","source_name":"medallia_innovation_survey.orderId","is_identifier":"false"}, {"id":"4768","unique_name":"innovation_action","type":"Text","source":"Var","source_name":"medallia_innovation_survey.action","is_identifier":"false"}, {"id":"4766","unique_name":"s_vi","type":"Text","source":"Var","source_name":"medalliaParams.s_vi","is_identifier":"false"}, {"id":"4765","unique_name":"visitorId","type":"Text","source":"cookie","source_name":"visitorId","is_identifier":"false"}, {"id":"4764","unique_name":"TLSessionID","type":"Text","source":"cookie","source_name":"TealeafAkaSid","is_identifier":"false"}, {"id":"4763","unique_name":"ref_url","type":"Text","source":"Var","source_name":"medalliaParams.ref_url","is_identifier":"false"}, {"id":"4762","unique_name":"abtestCampaign","type":"Text","source":"Var","source_name":"medalliaParams.abtestCampaign","is_identifier":"false"}]',
          formType: "embedded",
          formDisplay: { displayType: "embedded" },
          embeddedData: { pageElementId: "med_report_survey" },
          onSiteData: { devicetypes: ["desktop", "tablet", "mobile"] },
          urlRules: {
            exclusionUrl: { rule: null },
            inclusionUrl: { rule: null },
          },
        },
        {
          formId: "15766",
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15766/formDataV2_1756409123360_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15766/formData1756409123360_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15766/formDataV2_1756409123360_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.3/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15765/formDataV2_1758810505443_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15765/formData1758810505443_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15765/formDataV2_1758810505443_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.3/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15764/formDataV2_1758810570552_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15764/formData1758810570552_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15764/formDataV2_1758810570552_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.3/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15763/formDataV2_1758810586600_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15763/formData1758810586600_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15763/formDataV2_1758810586600_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15762/formDataV2_1756409646905_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15762/formData1756409646905_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15762/formDataV2_1756409646905_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.1/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15761/formDataV2_1756409520188_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15761/formData1756409520188_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15761/formDataV2_1756409520188_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/md-form/website/1.22.3/index.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/15735/formDataV2_1758810546351_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/15735/formData1758810546351_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/15735/formDataV2_1758810546351_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        {
          formId: "14703",
          isCustomHtmlEnabled: "true",
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/14703/1711565559486/customForm1717492790665.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/14703/1711565559486/formData1717492790736_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "675",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/14703/1711565559486/formData1717492790736_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/14703/1711565559486/formDataV2_1717492790736_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12630/1711565621123/customForm1717492790391.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12630/1711565621123/formData1717492790483_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "500",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/12630/1711565621123/formData1717492790483_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/12630/1711565621123/formDataV2_1717492790483_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12627/1711566228891/customForm1717492790691.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12627/1711566228891/formData1717492790822_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "650",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/12627/1711566228891/formData1717492790822_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/12627/1711566228891/formDataV2_1717492790822_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12623/1756409697782/customForm1756409698507.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12623/1756409697782/formData1756409698613_###LANGUAGE_CODE###.json",
          formWidth: "450",
          formHeight: "600",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/12623/1756409697782/formData1756409698613_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/12623/1756409697782/formDataV2_1756409698613_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12620/1711565687555/customForm1717492790386.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12620/1711565687555/formData1717492790471_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "650",
          isFixedSize: "true",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/12620/1711565687555/formData1717492790471_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/12620/1711565687555/formDataV2_1717492790471_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12619/1711620937044/customForm1717492790407.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12619/1711620937044/formData1717492790492_###LANGUAGE_CODE###.json",
          formWidth: "450",
          formHeight: "650",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/12619/1711620937044/formData1717492790492_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/12619/1711620937044/formDataV2_1717492790492_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12616/1711566251928/customForm1717492775496.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12616/1711566251928/formData1717492775572_###LANGUAGE_CODE###.json",
          formWidth: "550",
          formHeight: "450",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/12616/1711566251928/formData1717492775572_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/12616/1711566251928/formDataV2_1717492775572_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
        ...
        },
        {
          ...
          formVersion: "2",
          formHtmlUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12601/1711565663154/customForm1717492790662.html",
          formDataUrl:
            "https://resources.digital-cloud.medallia.com/wdcus/235967/forms/12601/1711565663154/formData1717492790724_###LANGUAGE_CODE###.json",
          formWidth: "580",
          formHeight: "675",
          isFixedSize: "false",
          isFullWidthEmbedded: "null",
          ...
          formJsonRelativePath:
            "wdcus/235967/forms/12601/1711565663154/formData1717492790724_###LANGUAGE_CODE###.json",
          formJsonV2RelativePath:
            "wdcus/235967/forms/12601/1711565663154/formDataV2_1717492790724_###LANGUAGE_CODE###.json",
          availableLanguages: ["en"],
          triggerAvailableLanguages: ["en"],
          functionFileUrl: "",
          customParams:
      ...
      domainsConfiguration: { allDomainsAllowed: true },
      integrations: {
        recordedSessions: [
          {
            isEnabled: [
            ...
            integrationName: "fullStory",
          },
        ],
      },
      usersnapEnvId: "7df5f0a9-62a0-4a8b-bbd4-0496bcd11803",
      usersnapOnpremWidgetLoaderPath: "usersnap-widget/4.10.0/loader.js",
      propertyMessages: {
        ...
        messagesData: {
          noJsonMessage: {
            en: {
              type: "noJsonMessage",
              title: "Oops",
              body: "We can't collect your feedback at the moment",
      ...
      globalResourcesUrl:
        "https://resources.digital-cloud.medallia.com/liveApp/resources",
      submitUrlPrefix: "https://feedback.digital-cloud.medallia.com/",
      submitUrlSuffix: "feedback/api/feedback/submit",
      analyticsSubmitEventsUrl:
        "https://analytics-fe.digital-cloud.medallia.com/api/web/events",
      mecIntegrationId: "target-apis-medallia-com-target",
      submitUrlWithBackendUUID:
        "https://feedback.digital-cloud.medallia.com/feedback/api/v2/feedback/submit",
      textAreaLimit: null,
      customParams: null,
      digitalAnalyticsDynamicEventsList: [
        "nebula_page_view",
        "nebula_form_show",
        "nebula_invitation_show",
        "nebula_form_submitted",
        "nebula_invite_accepted",
        "nebula_after_http_post_request_on_error",
        "nebula_after_http_post_request_on_error_status_code",
        "nebula_after_http_post_request_on_timeout",
        "nebula_submitFeedbackFailed",
        "nebula_popup_blocked",
        "MDigital_Invalid_Origin",
        "nebula_feedbackUuidIsNull",
      ],
      region: "digital-cloud-us",
      websiteId: 235967,
      accountId: 235953,
      packageVersion: null,
      isSpa: false,
  ...
  function fetchOnsiteExternalConfiguration(onsiteDataUrl) {
    KAMPYLE_UTILS.httpRequest(
      ...
      onsiteDataUrl,
      null,
      MDIGITAL.CONFIGURATION.onsiteDataSuccess,
      MDIGITAL.CONFIGURATION.onsiteDataFailed
    );
  }
  function onsiteDataSuccess(data) {
    if (!data || data.error) {
      ...
      if (formattedData) {
        formattedData.provisions = Object.assign(
          {},
          formattedData.provisions,
          MDIGITAL.CONFIGURATION.getOnsiteConfiguration().provisions
        );
      }
      var fullConfiguration = Object.assign(
        {},
        MDIGITAL.CONFIGURATION.getOnsiteConfiguration(),
        formattedData
      );
      if (
        fullConfiguration.provisions &&
        fullConfiguration.provisions[
          KAMPYLE_CONSTANT.PROVISIONS.DYNAMIC_ONSITE_EVENTS
        ]
      ) {
        KAMPYLE_EVENT_DISPATCHER.trigger(
          KAMPYLE_CONSTANT.EVENTS
            .MDIGITAL_DYNAMIC_ONSITE_INITIALIZED_SUCCESSFULLY
        );
      }
  ...
  function onsiteDataFailed(error) {
    var eventData = {
      errorMessage:
        KAMPYLE_CONSTANT.LOG_MESSAGES.ERROR_DYNAMIC_ONSITE_INITIALIZATION,
...
window.KAMPYLE_FUNC = (function () {
  function init() {
    window.MDIGITAL &&
      window.MDIGITAL.FORM_STATE &&
      window.MDIGITAL.FORM_STATE.init();
    subscribeFirstToPageEvents();
    KAMPYLE_VIEW.init();
    window.MDIGITAL &&
      window.MDIGITAL.EMBEDDED &&
      window.MDIGITAL.EMBEDDED.init();
    if (checkProvision(KAMPYLE_CONSTANT.PROVISIONS.NEW_MODAL_DIALOGS)) {
      window.MDIGITAL &&
        window.MDIGITAL.MODAL_DIALOGS &&
        window.MDIGITAL.MODAL_DIALOGS.init();
    } else if (
      checkProvision(KAMPYLE_CONSTANT.PROVISIONS.WCAG_TRAP_FOCUS_IN_MODAL)
    ) {
    ...
    var context =
      KAMPYLE_UTILS.getUrlParam("nebContext") ||
      window.MDIGITAL.nebContext ||
      "live";
    ...
    KAMPYLE_DATA.setMemoryData("currentUrl", window.location.href);
    var urlInterval = setInterval(function () {
      if (window.location.href !== KAMPYLE_DATA.getMemoryData("currentUrl")) {
        KAMPYLE_DATA.setMemoryData("currentUrl", window.location.href);
  ...
  function subscribeFirstToPageEvents() {
    var eventMapping = {
      ...
      neb_sdkPageView: [
        ...
        KAMPYLE_DATA.resetAllFormsLanguagesCode,
      ],
    };
  ...
  function handleInviteApplicationUpdated(eventName, data) {
    data = data || {};
    var shouldShowInvite = (KAMPYLE_DATA.getFormState(data.formId) || {})
      .inviteShown;
    if (
      shouldShowInvite &&
      MDIGITAL.FORM_STATE.isInviteLoaded(data.formId) &&
      isJsonDataLoaded(data.formId, true) &&
      ...
      hasApplicationLoadingFailed(data.formId, true) ||
      isAnyOtherFormAlreadyShown()
    ) {
  ...
  function isFormAlreadyShown(formId) {
    var shouldSkipShowingForm =
      KAMPYLE_DATA.isSkipInvitation(formId) &&
      isAnyOtherFormAlreadyShown(formId);
  ...
  function hasApplicationLoadingFailed(formId, isInvite) {
    ...
    return (
      currentMemoryData === false ||
      (!isInvite && formState && formState.loaded === false) ||
      (isInvite && formState && formState.inviteLoaded === false)
    );
  }
  function isJsonDataLoaded(formId, isInvite) {
    ...
    return (
      !KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.FORM_TEMPLATE_DATA_SEPERATION
      ) || KAMPYLE_DATA.getMemoryData(jsonDataKey)
    );
  }
  function shouldSkipFetchingFormData(formId, isInvite, formLanguage) {
    var isFetchingDataInProgressKey =
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.FETCHING_DATA_IN_PROGRESS +
      formId +
      "_" +
      formLanguage;
    var isFetchingDataInProgress = KAMPYLE_DATA.getMemoryData(
      isFetchingDataInProgressKey
    );
    var dataSeparationProvision = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.FORM_TEMPLATE_DATA_SEPERATION
    );
    return (
      !dataSeparationProvision ||
      isJsonDataLoaded(formId, false) ||
      hasApplicationLoadingFailed(formId, isInvite) ||
  ...
  function handleFormLoadingFailed(eventName, data) {
    if (
      data.failureCause ===
      KAMPYLE_CONSTANT.FORM_LOADING_FAILURE_CAUSE.APPLICATION_FAILED
    ) {
      ...
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_COULD_NOT_LOAD_FORM_APPLICATION,
        eventData
      );
    }
    setTimeout(function () {
  ...
  function handleInviteVersion(formId) {
    ...
    KAMPYLE_DATA.setMemoryData(
      "inviteVersion",
      inviteData && inviteData.mobileDesign ? 1 : -1
    );
  }
  function prepareJsonData(eventName, data) {
    var isInvite = eventName === "neb_loadInvitation";
    var formLanguage = KAMPYLE_DATA.getMemoryData(
      KAMPYLE_CONSTANT.LANG_MEMORY_DATA_KEY_PREFIX + data.formId
    );
    var fetchingDataInProgressKey =
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.FETCHING_DATA_IN_PROGRESS +
      data.formId;
    ...
    if (
      KAMPYLE_FUNC.shouldSkipFetchingFormData(
        data.formId,
        isInvite,
        formLanguage
      )
  ...
  function shouldRenderForm(data) {
    ...
    var isFormLoadedSuccessfully =
      isFormLoaded(data.formId) && isJsonDataLoaded(data.formId, false);
    var shouldShowForm =
      MDIGITAL.FORM_STATE.shouldFormBeShown(data.formId) &&
      !isFormAlreadyShown(data.formId);
  ...
  function onJsonFetchingFailed(data, isInvite, error) {
    ...
    if (!isInvite) {
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.NEB_FORM_LOADING_HAS_FAILED,
        {
          formId: data.formId,
    ...
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_COULD_NOT_FETCH_DATA,
      eventData
    );
  }
  function handleFormStateUpdated(data) {
    ...
    var shouldTriggerFormReady =
      (isPopup && isFormShown) || (!isPopup && shouldTriggerShowForm);
  ...
  function subscribeToPageEvents() {
    var eventMapping = {
      ...
      neb_showInvitation: [
        setInviteState,
        KAMPYLE_UTILS.setCurrentScrollPosition,
        setFocusedElement,
        focusOnInviteIframe,
      ...
      neb_inviteDeclined: [
        ...
        KAMPYLE_UTILS.handleAfterFormPosition,
      ],
      ...
      neb_submitFeedbackSuccess: [handleFeedbackSubmitted],
  ...
  function createSession() {
    ...
    try {
      isDirectNavigation =
        document.referrer === "" || window.parent.document.referrer === "";
    } catch (e) {
      var data = {
        errorMessage: e.message,
        errorStack: e.stack,
        additionalInfo:
          "createSession- failed to access document.referrer, probably CORS problem when the customer's iframe is hosted in a website with different domain.",
      };
      referrerDifferentThanHost = false;
      MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEvent(
        KAMPYLE_CONSTANT.MDIGITAL_WARNING,
        data
      );
    } finally {
      if (
        !kampyleUserSession ||
        isDirectNavigation ||
        referrerDifferentThanHost
      ) {
        var sessionData = {
          user_session: kampyleUserSession,
          direct_navigation: isDirectNavigation,
          different_referrer: referrerDifferentThanHost,
        };
        if (checkProvision(KAMPYLE_CONSTANT.PROVISIONS.CREATE_SESSION)) {
          if (
            window.MDIGITAL.ANALYTICS &&
            KAMPYLE_FUNC.checkProvision(
              KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DIGITAL_ANALYTICS
            )
          ) {
            MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEvent(
              "MDigital_createSession",
              { additionalData: sessionData }
            );
          }
        }
        KAMPYLE_DATA.setMemoryData("isSyncedIdentifierData", false);
        KAMPYLE_DATA.setData(
          "kampyleUserSession",
        ...
        KAMPYLE_DATA.deleteData(
          KAMPYLE_CONSTANT.SESSION_DATA_FIELDS.IS_SURVEY_SUBMITTED_IN_SESSION
        );
        if (
          !checkProvision(
            KAMPYLE_CONSTANT.PROVISIONS
              .DISABLE_USER_PERCENTILE_AT_CREATE_SESSION
          )
        ) {
          KAMPYLE_TARGETING.setUserPercentile();
        }
        if (
          window.MDIGITAL.ANALYTICS &&
          KAMPYLE_FUNC.checkProvision(
          ...
          MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEvent("neb_sessionStart", {
            additionalData: sessionData,
          });
        }
      }
  ...
  function isReferrerDifferentThanHost() {
    ...
    if (propertySharedSubdomain) {
      return (
        !KAMPYLE_UTILS.isHostMatchesSubdomain(
          referrerDomain,
          propertySharedSubdomain
        ) ||
        !KAMPYLE_UTILS.isHostMatchesSubdomain(
  ...
  function handleInviteLoaded(eventName, data) {
    ...
    if (!KAMPYLE_DATA.getMemoryData("inviteVersion")) {
      var inviteVersion = KAMPYLE_UTILS.getNestedPropertyValue(
        data,
        "inviteVersion"
      );
  ...
  function sendInviteInitialData(eventName, data) {
    ...
    var messageData = {
      ...
      customCssUpload: KAMPYLE_DATA.isFeatureEnabled(
        KAMPYLE_CONSTANT.PROPERTY_SETTINGS.CUSTOM_CSS_UPLOAD
      ),
    };
    if (
      KAMPYLE_FUNC.checkProvision(
      ...
      var callbackFunction = function (jsonData) {
        ...
        messageData.formLanguage =
          MDIGITAL.LOCALIZATION.getFormLanguage(data.formId) || null;
        KAMPYLE_UTILS.sendMessageToIframe("kampyleInvite", {
          action: "setExtraData",
          data: messageData,
        });
  ...
  function setInviteState(eventName, data) {
    KAMPYLE_UTILS.sendMessageToIframe("kampyleInvite", {
      action: KAMPYLE_CONSTANT.EVENTS.MDIGITAL_VALIDATE_ORIGIN,
    });
    data = data || {};
  ...
  function showPreviewForm() {
    ...
    if (
      !!formUrl &&
      KAMPYLE_UTILS.validateKampyleDomain(decodeURIComponent(formUrl))
    ) {
      ...
      var currentWebsiteId = decodeURIComponent(
        window.getUrlParam("websiteId")
      );
      if (!!currentWebsiteId) {
      ...
      var iframe = KAMPYLE_UTILS.createIframe(
        formUrl,
        iframeWidth,
        iframeHeight,
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.PREVIEW.IFRAME_ID + formId
      );
  ...
  function updateIdentifierDataIfNecessary(identifierData) {
    updateIdentifierDataIntegerIfNecessary(
      identifierData.DECLINED_DATE,
      "DECLINED_DATE"
    );
    updateIdentifierDataIntegerIfNecessary(
      identifierData.SUBMITTED_DATE,
      "SUBMITTED_DATE"
    );
  }
  function updateIdentifierDataIntegerIfNecessary(
    identifierDataValue,
    userStateString
  ) {
    ...
    if (
      (identifierDataValue !== null && localStorageDate === null) ||
      identifierDataValue > localStorageDate
    ) {
      KAMPYLE_DATA.setData(userStateString, identifierDataValue);
    } else if (
      (localStorageDate !== null && identifierDataValue === null) ||
      identifierDataValue < localStorageDate
    ) {
  ...
  function syncIdentifierData(userIdentifierVal, callback) {
    var propertyId =
      MDIGITAL.CONFIGURATION.getOnsiteConfiguration().websiteId || null;
    var urlPrefix = KAMPYLE_DATA.getServerBasedTargetingPrefix();
    var envPrefix = KAMPYLE_DATA.getServerBasedTargetingEnv();
    var url =
      urlPrefix +
      "/api/targeting/property/" +
      envPrefix +
      propertyId +
      "/identity/" +
      userIdentifierVal;
    var onSuccess = KAMPYLE_FUNC.onSyncIdentifierDataSuccess.bind(
      null,
      callback
    );
  ...
  function onSyncIdentifierDataSuccess(callback, data) {
    if (!data || data.error) {
      MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEventDispatcherError({
        errorMessage: "Invalid identifier data. Data: " + JSON.stringify(data),
      });
    } else {
  ...
  function onSyncIdentifierDataFailure(callback, error) {
    MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEventDispatcherError({
      errorMessage:
        "Failed to fetch identifier data. Error: " + JSON.stringify(error),
    });
    typeof callback === "function" && callback();
  }
  function shouldSyncIdentifier(newUserIdentifierVal) {
    var prevUserIdentifierVal = KAMPYLE_DATA.getMemoryData("identifierValue");
    var isSyncedIdentifierData =
      KAMPYLE_DATA.getMemoryData("isSyncedIdentifierData") === true;
    var isSBTOn = !!KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.SERVER_BASED_TARGETING
    );
    var isIdentifiedUser = !!newUserIdentifierVal;
    return (
      isIdentifiedUser &&
      isSBTOn &&
      (!isSyncedIdentifierData ||
  ...
  function startCheckingTargeting() {
    ...
    if (window.KAMPYLE_ONSITE_STATE !== "manual") {
      var shouldSkipInviteTargeting = KAMPYLE_DATA.isFeatureEnabled(
        KAMPYLE_CONSTANT.PROPERTY_SETTINGS.MANUALLY_INVITE_TARGETING
      );
  ...
  function increasePageViewCounter() {
    var sessionPageCounter =
      KAMPYLE_DATA.getData("kampyleSessionPageCounter") || 0;
  ...
  function handleFormClosed(eventName, data) {
    ...
    if (
      ...
      (!KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.EXEMPT_FORMS_FROM_QUARANTINE
      ) ||
        KAMPYLE_DATA.isFormOfType(
          data.formId,
          KAMPYLE_CONSTANT.FORM_TYPES.INVITATION
        ))
    ) {
      KAMPYLE_FUNC.setQuarantineDeclinedDate(
        KAMPYLE_CONSTANT.EVENTS
          .MDIGITAL_UPDATE_TARGETING_WITH_DECLINED_DATE_FORM_CLOSED
      );
    }
  ...
  function finishFormClosed(formId) {
    ...
    var delay =
      KAMPYLE_DATA.getDisplayType(formData) === "animation" ? 1000 : 0;
    if (buttonFormId) {
      setTimeout(function () {
  ...
  function updateTargeting(stateType, data) {
    if (
      !!checkProvision(KAMPYLE_CONSTANT.PROVISIONS.SERVER_BASED_TARGETING) &&
      !!KAMPYLE_DATA.getMemoryData("identifierValue")
    ) {
      var dataJson = {};
      ...
      var url =
        ...
        "/api/targeting/property/" +
        envPrefix +
        MDIGITAL.CONFIGURATION.getOnsiteConfiguration().websiteId +
        "/identity/" +
        encodeURIComponent(KAMPYLE_DATA.getMemoryData("identifierValue"));
      ...
      KAMPYLE_UTILS.httpRequest(
        KAMPYLE_CONSTANT.HTTP_METHODS.POST,
        url,
        dataJson
      );
    }
  ...
  function exemptQuarantineSubmittedDateByFormType(formId) {
    switch (KAMPYLE_DATA.getFormTriggerType(formId)) {
      case KAMPYLE_CONSTANT.FORM_TYPES.BUTTON:
        return KAMPYLE_DATA.isFeatureEnabled(
          KAMPYLE_CONSTANT.FEATURE_FLAGS.EXEMPT_BUTTON_FORMS_FROM_QUARANTINE
        );
      case KAMPYLE_CONSTANT.FORM_TYPES.CODE:
        return KAMPYLE_DATA.isFeatureEnabled(
          KAMPYLE_CONSTANT.FEATURE_FLAGS.EXEMPT_CODE_FORMS_FROM_QUARANTINE
        );
      case KAMPYLE_CONSTANT.FORM_TYPES.EMBEDDED:
        return KAMPYLE_DATA.isFeatureEnabled(
          KAMPYLE_CONSTANT.FEATURE_FLAGS.EXEMPT_EMBEDDED_FORMS_FROM_QUARANTINE
        );
      default:
        return false;
    }
  ...
  function handleFeedbackSubmitted(eventMapping, data) {
    if (
      eventMapping === "neb_feedbackSubmitted" &&
      !KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.DISABLE_EXTRA_FEEDBACK_SUBMIT_EVENT
      )
    ) {
      return;
    }
    if (
      ...
      !exemptQuarantineSubmittedDateByFormType(data.formId)
    ) {
  ...
  function loadScreenCaptureApi(eventName, data) {
    data = data || {};
    var isScreenCaptureAsComponentEnabled = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.USER_SNAP_AS_COMPONENT
    );
    var shouldNotCallScreenCaptureJs =
      isScreenCaptureAsComponentEnabled &&
      !isScreenCaptureAsComponentExistAndSupported(data.formId);
    if (
      data.isScreenCaptureEnabled !== false &&
      !shouldNotCallScreenCaptureJs
    ) {
      KAMPYLE_SCREEN_CAPTURE.init(function () {
        KAMPYLE_EVENT_DISPATCHER.trigger("neb_screenCaptureLoaded", {
          formId: data.formId,
        });
      });
    }
  }
  function isScreenCaptureAsComponentExistAndSupported(formId) {
    var displayType = KAMPYLE_UTILS.getNestedPropertyValue(
      KAMPYLE_DATA.getFormData(formId),
      "formDisplay.displayType"
    );
    var isScreenCaptureSupported =
      displayType !== "embedded" && !KAMPYLE_DATA.isMobile();
    if (!isScreenCaptureSupported) {
      return false;
    }
    var formPages = KAMPYLE_DATA.getMemoryJsonNestedPropertyValue(
      formId,
      "pages",
      false
    );
    var isFormV2 = KAMPYLE_DATA.isV2Form(formId);
    var componentsName = isFormV2 ? "components" : "dynamicData";
    var componentType = isFormV2 ? "type" : "component";
    return formPages.some(function (page) {
      return page[componentsName].some(function (component) {
  ...
  function handleScreenCaptureApiLoaded(eventName, data) {
    ...
    switch (KAMPYLE_DATA.getDisplayType(formData)) {
      case KAMPYLE_CONSTANT.DISPLAY_TYPES.LIGHTBOX:
        iframePrefix =
          KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.IFRAME_ID_PREFIX;
        break;
      case KAMPYLE_CONSTANT.DISPLAY_TYPES.ANIMATION:
        iframePrefix = KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.IFRAME_ID;
        break;
      default:
        iframePrefix = KAMPYLE_CONSTANT.IFRAME_SETTINGS.PREVIEW.IFRAME_ID;
    }
  ...
  function checkProvision(name) {
    ...
    return (
      ...
      !!dataObj &&
      !!dataObj.provisions &&
      (!!dataObj.provisions[name] || dataObj.provisions[name] === "true")
    );
  }
  ...
  function checkDevice() {
    if (!KAMPYLE_DATA.getMemoryData("kampyleDevice")) {
      KAMPYLE_DATA.setMemoryData(
        "kampyleDevice",
        KAMPYLE_UTILS.getDeviceTypeByUserAgent(
          navigator.userAgent,
  ...
  function sendFormExtraData(eventName, data) {
    ...
    switch (formDisplayType) {
      case "animation":
        currIframe = document.getElementById(
          KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.IFRAME_ID + data.formId
        );
        break;
      case "lightbox":
        currIframe = document.getElementById(
    ...
    if (targetWindow && targetWindow.postMessage) {
      var messageData = getCustomParamsMessage(data.formId);
      targetWindow.postMessage(messageData, origin);
      var isExtensionEnabled = KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.EXTENSION_ENABLED
      );
      if (isExtensionEnabled) {
        var authorizationMessage = KAMPYLE_MESSAGE.getAuthorizationMessage();
        targetWindow.postMessage(authorizationMessage, origin);
      }
      messageData =
        '{"action": "setFormId","formId" : ' +
        data.formId +
        ', "__isDigitalAction": true}';
      targetWindow.postMessage(messageData, origin);
      messageData = { action: "setExtraData", __isDigitalAction: true };
      messageData.clientHints =
        KAMPYLE_UTILS.safeJsonParse(
          KAMPYLE_CONSTANT.PLACEHOLDERS.USER_AGENT_CLIENT_HINTS_LIST
        ) || [];
      messageData.provisions = getAllProvisions();
      messageData.integrations = KAMPYLE_INTEGRATION.getIntegrationData();
      messageData.websiteId =
        MDIGITAL.CONFIGURATION.getOnsiteConfiguration().websiteId || null;
      ...
      (messageData.customCssUpload = KAMPYLE_DATA.isFeatureEnabled(
        KAMPYLE_CONSTANT.PROPERTY_SETTINGS.CUSTOM_CSS_UPLOAD
      )),
        (messageData = addScreenCaptureData(messageData));
      messageData.isScreenCaptureEnabled =
        KAMPYLE_SCREEN_CAPTURE.isScreenCaptureEnabled(data.formId);
      messageData.provisions.screenCapture =
        checkProvision("screenCapture") ||
        checkProvision("screenCaptureEnabled");
      ...
      Object.assign(messageData, {
        ...
        globalResourcesUrl: onsiteConfiguration.globalResourcesUrl,
        versionedResourcesUrl: formData.versionedResourcesUrl,
        textAreaLimit: onsiteConfiguration.textAreaLimit,
      });
      if (
        KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.SERVER_BASED_TARGETING
  ...
  function getCustomParamsMessage(formId) {
    if (MDIGITAL.CUSTOM_PARAMETERS.hasCustomParams(formId)) {
      var customParams = MDIGITAL.CUSTOM_PARAMETERS.formatCustomParams(formId);
      return (
        '{"action": "customParamsLoaded","customParams" : ' +
  ...
  function isAnyOtherFormAlreadyShown(
    currentFormId,
    shouldIgnoreSkipInvitation
  ) {
    var allFormStates = KAMPYLE_DATA.getMemoryData("formsState") || {};
    for (var formId in allFormStates) {
      if (allFormStates.hasOwnProperty(formId)) {
        var shouldIgnoreForm =
          !MDIGITAL.FORM_STATE.isFormShown(formId) &&
          ((currentFormId && currentFormId == formId) ||
            (shouldIgnoreSkipInvitation &&
              KAMPYLE_DATA.isSkipInvitation(formId)));
        if (
          (allFormStates[formId].shown ||
            allFormStates[formId].shouldBeShown) &&
          !KAMPYLE_DATA.isEmbeddedDisplayType(formId) &&
          !shouldIgnoreForm
        ) {
          return true;
        }
      ...
      switch (currParamType.toLowerCase()) {
        case "datetime":
          if (typeof currParamVal === "string") {
            ...
            !(currParamVal instanceof Date) &&
            (!currParamVal.getTime || isNaN(currParamVal.getTime()))
          ) {
            currParamVal = null;
          } else {
  ...
  function isPreloadConfigured() {
    ...
    return (
      !!data &&
      data.isFormsPreloaded !== "false" &&
      data.isFormsPreloaded !== false
    );
  }
  function addToSessionCount() {
    var kampyleUserSessionsCount = KAMPYLE_DATA.getData(
  ...
  function getTimeInSession() {
    var sessionStartTimestamp = Number(
      KAMPYLE_DATA.getData("kampyleUserSession")
    );
    return !!sessionStartTimestamp
      ? Math.floor(
          KAMPYLE_UTILS.getTimeDiff(
            sessionStartTimestamp,
            KAMPYLE_CONSTANT.TIME_FORMATS.SECONDS
          )
        )
      : 0;
  }
  ...
  function getLastDeclineSubmit(key) {
    ...
    return !!date
      ? KAMPYLE_UTILS.getTimeDiff(date, KAMPYLE_CONSTANT.TIME_FORMATS.DAYS)
      : null;
  }
  function prepareInviteTriggeringData(eventName, data) {
    var deviceName = (
      KAMPYLE_DATA.getMemoryData("kampyleDevice") ||
      KAMPYLE_CONSTANT.DEVICES.DESKTOP
    ).toLowerCase();
    var formData = KAMPYLE_DATA.getFormData(data.formId);
    var inviteTriggeringData = {
      ...
      excluded_urls: KAMPYLE_UTILS.getNestedPropertyValue(
        formData,
        "urlRules.exclusionUrl.rule"
      ),
      included_urls: KAMPYLE_UTILS.getNestedPropertyValue(
        formData,
        "urlRules.inclusionUrl.rule"
      ),
      number_of_visits:
        Number(KAMPYLE_DATA.getData("kampyleUserSessionsCount")) || 1,
    ...
    KAMPYLE_DATA.setMemoryData(
      "invitationTriggeringData",
      inviteTriggeringData
    );
  }
  function isContentPayloadEnabled() {
    return (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.EVENT_INCLUDE_FEEDBACK_CONTENT
      ) ||
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.EXPOSE_SELECTION_QUESTIONS
      ) ||
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.EXPOSE_VERBATIM_QUESTIONS
      )
    );
  }
  return {
    ...
    getCustomParamsMessage: getCustomParamsMessage,
  };
})();
window.KAMPYLE_DATA = (function () {
  ...
  function getNestedConfigurationProperty(propertyName) {
    var configuration = KAMPYLE_DATA.getMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.DATA_JSON
    );
  ...
  function initOnsiteDefaultState(formIdToReset) {
    var shouldResetAllForms = isNaN(formIdToReset) || !formIdToReset;
    var formsState;
    if (shouldResetAllForms) {
      formsState = {};
      ...
      for (var i = 0; i < len; i++) {
        curr = allForms[i];
        if (curr) {
          formsState[curr.formId] = getDefaultFormState();
        }
      }
      setMemoryData("formsState", formsState);
    } else {
      formsState = KAMPYLE_DATA.getMemoryData("formsState");
      formsState[formIdToReset] = getDefaultFormState();
      setMemoryData("formsState", formsState);
    }
    setMemoryData("hostingElementsHeights", {});
    KAMPYLE_DATA.setMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.IS_BUTTON_CONTAINER_CREATED,
      false
    );
  ...
  function clearAllTimeoutHandlers() {
    var allHandlers = getMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.TIMEOUT_HANDLERS
    );
  ...
  function setTimeoutHandler(key, timeoutHandler) {
    setMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.TIMEOUT_HANDLERS + "." + key,
      timeoutHandler
    );
  }
  ...
  function setDOMEventHandler(key, eventName, eventHandler, targetElement) {
    ...
    setMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.DOM_EVENT_HANDLERS + "." + key,
      dataValue
    );
  }
  ...
  function clearAllDOMEventsHandlers() {
    var handlers =
      getMemoryData(KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.DOM_EVENT_HANDLERS) ||
      {};
  ...
  function setAuthorizationToken(jwt) {
    setMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.EX_AUTHORIZATION_TOKEN,
      jwt
    );
  }
  ...
  function getData(dataName) {
    try {
      if (KAMPYLE_DATA.getPropertySharedSubdomain()) {
        return KAMPYLE_UTILS.kampyleGetCookie(
          KAMPYLE_UTILS.TOP_LEVEL_DOMAIN_COOKIE_PREFIX + dataName
        );
      }
  ...
  function setData(dataName, dataValue, expriration) {
    ...
    KAMPYLE_UTILS.kampyleSetCookie(
      dataName,
      dataValue,
      expriration || getCookieExpiration(),
      KAMPYLE_DATA.getPropertySharedSubdomain()
    );
    return true;
  }
  var deleteData = function (dataName) {
    var propertySharedSubdomain = KAMPYLE_DATA.getPropertySharedSubdomain();
    if (propertySharedSubdomain) {
      KAMPYLE_UTILS.kampyleDeleteCookie(
        KAMPYLE_UTILS.TOP_LEVEL_DOMAIN_COOKIE_PREFIX + dataName,
        propertySharedSubdomain
      );
      return;
    ...
    if (
      !KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.DISABLE_COOKIE_STORAGE
      )
    ) {
  ...
  function setFormState(formId, data) {
    ...
    for (var prop in data) {
      if (
        typeof allFormState[formId][prop] === "object" &&
        allFormState[formId][prop] !== null
      ) {
  ...
  function isSkipInvitation(formId) {
    var formData = getFormData(formId) || {};
    var isSkipInvitation = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      "inviteData.skipInvitation"
    );
  ...
  function isCustomHtmlAndFixedSizeForm(formId) {
    ...
    var hasProvision = KAMPYLE_FUNC.checkProvision(
      "dynamicModalSizeInCustomHtml"
    );
    return (
      customHtmlForm && (!hasProvision || (hasProvision && isFixedSize(formId)))
    );
  }
  function getDisplayType(formData) {
    ...
    if (kampyleDevice !== KAMPYLE_CONSTANT.DEVICES.DESKTOP) {
      return displayType === KAMPYLE_CONSTANT.DISPLAY_TYPES.EMBEDDED
        ? KAMPYLE_CONSTANT.DISPLAY_TYPES.EMBEDDED
        : KAMPYLE_CONSTANT.DISPLAY_TYPES.LIGHTBOX;
    } else {
  ...
  function isCustomHtmlForm(formId) {
    ...
    return (
      formData.isCustomHtmlEnabled === true ||
      formData.isCustomHtmlEnabled === "true" ||
      isCustomHtml === true ||
      isCustomHtml === "true"
    );
  ...
  function getCookieExpiration() {
    return (
      KAMPYLE_UTILS.getNestedPropertyValue(dataJson, "cookieExpirationTime") ||
      KAMPYLE_CONSTANT.COOKIE_EXPIRATION
    );
  }
  function getTargetingData(formId, targetingName) {
    ...
    return KAMPYLE_UTILS.getNestedPropertyValue(
      selectedForm,
      "onSiteData." + targetingRealName
    );
  }
  ...
  function isMobileDynamicSizing(formId) {
    ...
    var requiredVersion =
      KAMPYLE_CONSTANT.FEATURE_VERSIONS.MOBILE_DYNAMIC_SIZING;
    formId = formId || KAMPYLE_DATA.getMemoryData("currentFormId");
    var isCustomHtml = KAMPYLE_DATA.isCustomHtmlForm(formId);
    var isProvisioned =
      (isCustomHtml &&
        KAMPYLE_FUNC.checkProvision("mobileWebDynamicFormSize")) ||
      (!isCustomHtml &&
        KAMPYLE_FUNC.checkProvision("mobileWebDynamicFormSizeInFormBuilder"));
    return (
      isProvisioned &&
      device &&
      device !== "desktop" &&
      !KAMPYLE_DATA.isV2Form(formId) &&
      KAMPYLE_DATA.getFormVersion(formId) >= requiredVersion
    );
  }
  function isMobile() {
    var device = KAMPYLE_DATA.getMemoryData("kampyleDevice");
  ...
  function getMemoryJsonKey(formId, isInvite, dataKey) {
    ...
    if (!dataKey) {
      jsonDataKey =
        (isInvite
          ? KAMPYLE_CONSTANT.INVITE_JSON_DATA_KEY_PREFIX
          : KAMPYLE_CONSTANT.FORM_JSON_DATA_KEY_PREFIX) + formId;
    } else {
  ...
  function getMemoryJsonNestedPropertyValue(formId, propertyName, isInvite) {
    ...
    return data != null
      ? KAMPYLE_UTILS.getNestedPropertyValue(data, propertyName)
      : null;
  }
  function localizeUrl(formId, url) {
    var langCode =
      MDIGITAL.LOCALIZATION.getFormLanguage(formId) ||
      KAMPYLE_CONSTANT.LOCALIZED_FORM_DEFAULT_REPLACE_VALUE;
  ...
  function getDataJsonUrl(formId, isInviteJson) {
    var formData = KAMPYLE_DATA.getFormData(formId);
    var jsonDataUrl = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      isInviteJson ? "inviteData.inviteDataUrl" : "formDataUrl"
    );
    if (!jsonDataUrl) {
      return null;
    }
  ...
  function fetchFormJsonData(obj, isSecondCall) {
    var jsonDataMemoryKey = KAMPYLE_DATA.getMemoryJsonKey(
      obj.formId,
      obj.isInvite
    );
    ...
    if (jsonData) {
      obj.fetchingDataInProgressKey &&
        KAMPYLE_DATA.setMemoryData(obj.fetchingDataInProgressKey, false);
    ...
    KAMPYLE_UTILS.httpRequest(
      ...
      function (data) {
        if (isSecondCall) {
          var warnMessage =
            "second http request have been made to the url: " + url;
        ...
        if (!data) {
          var errorMessage =
            "Invalid form json data. Data: " + JSON.stringify(data);
          MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEventDispatcherError({
      ...
      function (error) {
        MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEventDispatcherError({
          errorMessage:
            "Failed to fetch JSON data. Error: " + JSON.stringify(error),
        });
  ...
  function getFormTriggerType(formId) {
    var formType = KAMPYLE_UTILS.getNestedPropertyValue(
      KAMPYLE_DATA.getFormData(formId),
      "formType"
    );
  ...
  function getFormIframeId(formId) {
    ...
    if (formData.formType === KAMPYLE_CONSTANT.FORM_TYPES.EMBEDDED) {
      return (
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.EMBEDDED.IFRAME_ID_PREFIX + formId
      );
    } else {
      var displayType = getDisplayType(formData);
      if (displayType === KAMPYLE_CONSTANT.DISPLAY_TYPES.LIGHTBOX) {
        return (
          KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.IFRAME_ID_PREFIX + formId
        );
  ...
  function isV2Form(formId) {
    var formData = KAMPYLE_DATA.getFormData(formId);
    if (!formId || !formData || !formData.formDataUrl) {
      return;
    }
    return formData.formDataUrl.indexOf("formDataV2") > -1;
  }
  return {
    init: init,
...
window.KAMPYLE_TARGETING = (function (window, document) {
  ...
  function isDataValid(days, cookieName) {
    ...
    return (
      checkingDate + msInOneDay * days > KAMPYLE_UTILS.getCurrentTimestamp()
    );
  }
  function handleButton(eventName, data) {
    ...
    var kampyleDevice = KAMPYLE_DATA.getMemoryData("kampyleDevice");
    if (
      kampyleDevice === KAMPYLE_CONSTANT.DEVICES.MOBILE ||
      kampyleDevice === KAMPYLE_CONSTANT.DEVICES.TABLET
    ) {
      if (KAMPYLE_FUNC.isPreloadConfigured()) {
        KAMPYLE_EVENT_DISPATCHER.trigger(
          KAMPYLE_CONSTANT.EVENTS.NEB_LOAD_FORM,
          { formId: data.formId }
        );
      }
    }
  ...
  function isShowInvite(formData) {
    ...
    if (
      !formData ||
      KAMPYLE_DATA.getMemoryData("isInvitationShouldBeIgnored")
    ) {
      return false;
    }
    var isSkipInvitation = KAMPYLE_UTILS.getNestedPropertyValue(
    ...
    var shouldDisableIntercept =
      KAMPYLE_TARGETING.shouldDisableIntercept(formData);
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_BEHAVIORAL_TARGETING_PHASE_ENDED,
      formData
    );
    ...
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_QUARANTINED_SURVEY_PHASE_ENDED,
      formData
    );
    ...
    KAMPYLE_UTILS.isTruthy(isSkipInvitation)
      ? handleSkippedInvitaion(formData)
      : KAMPYLE_TARGETING.handleNotSkippedInvitaion(formData);
    return true;
  }
  function isQuarantinePhasePassed(formData) {
    ...
    var isInviteMoreThanOncePerSession = KAMPYLE_UTILS.isTrue(
      KAMPYLE_UTILS.getNestedPropertyValue(
        formData,
        "onSiteData.kampyleInvitePerSession.inviteOncePerSession"
      )
    );
    ...
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
      {
        formId: formData.formId,
        targetingRule:
          KAMPYLE_CONSTANT.TARGETING_RULES
            .INVITE_IF_ANY_INVITE_ALREADY_SHOWN_PER_SESSION,
    ...
    if (
      ...
      isSurveySubmittedInSession =
        !isInviteMoreThanOncePerSession &&
        KAMPYLE_UTILS.isTrue(
          KAMPYLE_DATA.getData(
            KAMPYLE_CONSTANT.SESSION_DATA_FIELDS.IS_SURVEY_SUBMITTED_IN_SESSION
          )
        );
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
        {
          ...
          formTriggerType: KAMPYLE_DATA.getFormTriggerType(formData.formId),
          targetingPhase:
            KAMPYLE_CONSTANT.TARGETING_PHASES.QUARANTINE_TARGETING,
  ...
  function isUbtFailed(formData) {
    var isUbtCheckFailed =
      ...
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.DO_NOT_SURVEY_ON_UBT_NETWORK_FAIL
      ) &&
  ...
  function isAlreadySubmitted(formData) {
    ...
    var submittedDays = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      "onSiteData.submitted.days"
    );
    var submittedDateText =
      KAMPYLE_TARGETING.getTextByQuarentine("SUBMITTED_DATE");
    submittedDays &&
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
        {
  ...
  function isAlreadyDeclined(formData) {
    ...
    var declinedDateText =
      KAMPYLE_TARGETING.getTextByQuarentine("DECLINED_DATE");
    var declinedDays = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      "onSiteData.declined.days"
    );
    declinedDays &&
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
  ...
  function shouldDisableIntercept(formData) {
    ...
    var disableInterceptRule = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      "onSiteData.disableInterceptRule"
    );
    if (disableInterceptRule) {
      if (typeof disableInterceptRule === "string") {
        disableInterceptRule = JSON.parse(disableInterceptRule);
      }
      shouldDisableIntercept = KAMPYLE_TARGETING.evaluateRule(
        disableInterceptRule,
        formData
      );
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
        {
          ...
          formTriggerType: KAMPYLE_DATA.getFormTriggerType(formData.formId),
          targetingPhase:
            KAMPYLE_CONSTANT.TARGETING_PHASES.BEHAVIORAL_TARGETING,
  ...
  function handleNotSkippedInvitaion(formData) {
    if (
      KAMPYLE_DATA.getDisplayType(formData) ===
      KAMPYLE_CONSTANT.DISPLAY_TYPES.POPUP
    ) {
      ...
      if (KAMPYLE_FUNC.isFormLoaded(formData.formId)) {
        ...
        KAMPYLE_DATA.setMemoryData(
          KAMPYLE_TARGETING.generateShouldShowInviteParamName(formData.formId),
          true
        );
        KAMPYLE_EVENT_DISPATCHER.trigger(
          KAMPYLE_CONSTANT.EVENTS.NEB_LOAD_FORM,
      ...
      KAMPYLE_UTILS.getNestedPropertyValue(
        ...
        KAMPYLE_EVENT_DISPATCHER.trigger(
          KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
          {
            ...
            expected: KAMPYLE_UTILS.getNestedPropertyValue(
              form,
              "urlRules.exclusionUrl.rule"
            ),
  ...
  function isGenericRuleValid(data) {
    if (data && data.onSiteData && data.onSiteData.genericRule) {
      if (typeof data.onSiteData.genericRule === "string") {
        if (
          KAMPYLE_FUNC.checkProvision(
            KAMPYLE_CONSTANT.PROVISIONS.GREATER_SMALLER_THAN_DECODE
          )
        ) {
          data.onSiteData.genericRule = KAMPYLE_UTILS.htmlDecode(
            data.onSiteData.genericRule
          );
        }
  ...
  function checkAllowedOperator(paramType, operator) {
    var paramTypeToOperator = {
      ...
      datetime: [
        KAMPYLE_CONSTANT.OPERATORS.EQUALS,
        KAMPYLE_CONSTANT.OPERATORS.DOES_NOT_EQUAL,
        KAMPYLE_CONSTANT.OPERATORS.LATER_THAN,
        KAMPYLE_CONSTANT.OPERATORS.EARLIER_THAN,
        KAMPYLE_CONSTANT.OPERATORS.HAS_VALUE,
      ],
      number: [
        KAMPYLE_CONSTANT.OPERATORS.EQUALS,
        KAMPYLE_CONSTANT.OPERATORS.DOES_NOT_EQUAL,
        KAMPYLE_CONSTANT.OPERATORS.GREATER_THAN,
        KAMPYLE_CONSTANT.OPERATORS.SMALLER_THAN,
        KAMPYLE_CONSTANT.OPERATORS.HAS_VALUE,
      ],
    };
    ...
    if (
      !paramType ||
      !operator ||
      !paramTypeToOperator[paramType] ||
      paramTypeToOperator[paramType].indexOf(operator) === -1
    ) {
      return false;
    } else {
      return true;
  ...
  function evaluateCriteria(criteria, formData) {
    ...
    if (
      ...
      criteria.value === null ||
      allowedTypes.indexOf(criteria.fieldOrigin) === -1
    ) {
      evaluation = false;
    } else {
      switch (criteria.fieldOrigin) {
        case "customParam":
          var customParamObj = MDIGITAL.CUSTOM_PARAMETERS.getCustomParamById(
            criteria.fieldId,
            formData
          );
          if (customParamObj && customParamObj.source_name) {
            ...
            isAllowedOperator = checkAllowedOperator(
              customParamObj.type,
              criteria.condition
            );
          }
          if (paramValue !== undefined && isAllowedOperator) {
            evaluation = KAMPYLE_UTILS.kampyleCompareByOperator(
              paramValue,
              secondParamValue,
              condition,
              customParamObj.type
  ...
  function evaluateRule(toEvaluate, formData) {
    ...
    if (
      toEvaluate &&
      toEvaluate.type === "criteriaGroup" &&
      toEvaluate.childrenCriterias &&
      toEvaluate.childrenCriterias.length &&
      toEvaluate.conjunction
    ) {
      for (
        var i = 0;
        i < toEvaluate.childrenCriterias.length && !stopFlag;
        i++
      ) {
        tmpEval = KAMPYLE_TARGETING.evaluateRule(
          toEvaluate.childrenCriterias[i],
          formData
        );
  ...
  function isNumSessions(data) {
    ...
    var numberOfRepeats = KAMPYLE_UTILS.getNestedPropertyValue(
      data,
      "onSiteData.numberOfVisits.numberOfRepeats"
    );
    var compareString = KAMPYLE_UTILS.getNestedPropertyValue(
      data,
      "onSiteData.numberOfVisits.compareString"
    );
    if (!!kampyleUserSession) {
      if (numberOfRepeats === null || compareString === null) {
        numberOfVisitsResult = true;
      } else {
        numberOfVisitsResult = KAMPYLE_UTILS.kampyleCompareByOperator(
          parseInt(kampyleUserSession),
  ...
  function checkPagesInSession(sessionPageObj) {
    ...
    return KAMPYLE_UTILS.kampyleCompareByOperator(
      pageCount,
      parseInt(sessionPageObj.numberOfRepeats),
      sessionPageObj.compareString
    );
  }
  ...
  function checkAbandon(formData) {
    var isAbandonmentPassed =
      isUrlIncluded(formData) && !isUrlExcluded(formData);
  ...
  function abandon(abandonObj, formData) {
    ...
    var height =
      !!abandonObj.height && abandonObj.height !== "0"
        ? abandonObj.height + "px"
        : heightDef + "px";
  ...
  function getEventQuery(eventObj) {
    ...
    switch (eventObj.selectorType) {
      ...
      case "objectId":
        ...
        break;
      default:
        KAMPYLE_UTILS.showWarning(
          "Invalid selector type:" + eventObj.selectorType
        );
        return null;
    }
    ...
    for (var i = 0; i < selectedElements.length; i++) {
      var eventHandler = function () {
        var timeout = setTimeout(function () {
          KAMPYLE_DATA.setMemoryData(
            "targetingState.kampylePageElementEvent",
            true
          );
          KAMPYLE_EVENT_DISPATCHER.trigger(
            KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
            {
              ...
              expected:
                eventsObj.eventName +
                " for element with " +
                eventsObj.selectorType +
                "=" +
                eventsObj.selectorValue,
              actual: true,
            }
          );
          if (isShowInvite() === false) {
      ...
      KAMPYLE_DATA.setDOMEventHandler(
        "triggerByEvent_" + i,
        eventsObj.eventName,
        eventHandler,
        selectedElements[i]
  ...
  function sendTimeOnPageEventData(formData) {
    var timeOnPage = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      "onSiteData.kampyleTimeOnPage.time"
    );
    timeOnPage &&
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
  ...
  function timeOnPage(time, formData) {
    ...
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_FIX_FOR_SPA_TIME_ON_PAGE
      )
    ) {
  ...
  function initDynamicTargeting(formData) {
    ...
    var eventsContract = KAMPYLE_DATA.getTargetingData(
      formData.formId,
      "pageEvents"
    );
    if (eventsContract) {
  ...
  function initEmbeddedTargeting() {
    ...
    for (var i = allForms.length - 1; i >= 0; i--) {
      if (allForms[i].formType === KAMPYLE_CONSTANT.FORM_TYPES.EMBEDDED) {
        ...
        if (canShow) {
          var quarantineTargetingPassed = !KAMPYLE_TARGETING.checkQuarantine(
            allForms[i]
          );
          var sessionTargetingPassed =
            KAMPYLE_TARGETING.validateCommonSessionTargeting(
              allForms[i].formId
            );
          var isUbtInitializedSuccessfully = !isUbtFailed(allForms[i]);
          if (
            isUbtInitializedSuccessfully &&
            quarantineTargetingPassed &&
            sessionTargetingPassed
          ) {
            KAMPYLE_EVENT_DISPATCHER.trigger(
              KAMPYLE_CONSTANT.EVENTS.NEB_LOAD_FORM,
  ...
  function handleIdentifierData(userIdentifierVal, eventName, data) {
    if (KAMPYLE_FUNC.shouldSyncIdentifier(userIdentifierVal)) {
      ...
      KAMPYLE_FUNC.syncIdentifierData(
        encodeURIComponent(userIdentifierVal),
        initFormsTargeting.bind(this, arguments[0], arguments[1])
      );
    } else {
  ...
  function handleFormsTargeting(eventName, data) {
    ...
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_UBT_IDENTIFIER_HASH
      )
    ) {
  ...
  function initFormsTargeting(eventName, data) {
    ...
    for (var i = allForms.length - 1; i >= 0; i--) {
      if (
        allForms[i].formType === KAMPYLE_CONSTANT.FORM_TYPES.BUTTON &&
        !data.skipButtonTargeting &&
        KAMPYLE_DATA.getMemoryData("isButtonShown") === false
      ) {
        canShow = KAMPYLE_TARGETING.validateCommonTargetingOnForm(allForms[i]);
        if (canShow === true) {
        ...
        canShow =
          KAMPYLE_TARGETING.validateCommonTargetingOnForm(allForms[i]) &&
          KAMPYLE_TARGETING.validateCommonSessionTargeting(allForms[i].formId);
        if (canShow === true) {
          KAMPYLE_TARGETING.initDynamicTargeting(allForms[i]);
        }
      }
      if (
        KAMPYLE_DATA.getMemoryData("isInviteActivated") === true &&
        KAMPYLE_DATA.getMemoryData("isButtonShown") === true
      ) {
        break;
      }
    }
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_PAGE_TARGETING_PHASE_ENDED
    );
  }
  ...
  function validateCommonSessionTargeting(formId) {
    ...
    if (
      KAMPYLE_UTILS.getNestedPropertyValue(
        data,
        "onSiteData.kampyleInvitePercentageUsers.prec"
      )
    ) {
      var formPercentageUsersResult = checkPercentageUsers(
        data.onSiteData.kampyleInvitePercentageUsers
      );
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
        {
    ...
    var numberOfPagesResult =
      !!data.onSiteData.numberOfPagesViewed &&
      checkPagesInSession(data.onSiteData.numberOfPagesViewed) === false;
    KAMPYLE_UTILS.getNestedPropertyValue(
      data,
      "onSiteData.numberOfPagesViewed.numberOfRepeats"
    ) &&
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_TARGETING_RULE_EVALUATED,
        {
...
window.KAMPYLE_ANIMATION = (function () {
  ...
  function listenToResizeAnimation(eventName, data) {
    var formData = KAMPYLE_DATA.getFormData(data.formId);
    var isMobile = KAMPYLE_DATA.isMobile();
    var displayDirection = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      "formDisplay.displayDirection"
    );
    var shouldAdjustToZoom =
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ZOOM_WCAG_REFLOW
      ) && KAMPYLE_UTILS.getNestedPropertyValue(formData, "urlVersion");
    if (
      shouldAdjustToZoom &&
      !isMobile &&
      !!displayDirection &&
      displayDirection.indexOf("center") > -1
    ) {
  ...
  function debouncedOnResize(formData) {
    var onResizeAnimationContainerBound =
      KAMPYLE_ANIMATION.resizeAnimationContainer.bind(null, formData);
    var debounced = KAMPYLE_UTILS.debounce(
      onResizeAnimationContainerBound,
      100,
      true
    );
    KAMPYLE_ANIMATION.debouncedOnResize = debounced;
    return debounced;
  }
  function resizeAnimationContainer(formData) {
    var animationContainer = document.getElementById(
      "KampyleAnimationContainer"
    );
    KAMPYLE_UTILS.setElementStyle(animationContainer, {
      top: getMiddleHeight(formData.formHeight),
    });
  }
  function animateWithInit(eventName, data) {
    ...
    setTimeout(function () {
      ...
      focusAnimatedIframeSafari(data.formId);
    }, 10);
  }
  function focusAnimatedIframeSafari(formId) {
    if (!KAMPYLE_UTILS.isSafari()) return;
    var targetIframeId = KAMPYLE_DATA.getFormIframeId(formId);
    var targetIframe = document.getElementById(targetIframeId);
    KAMPYLE_UTILS.focusElement(targetIframe);
  }
  function shouldIgnoreRenderAnimationForm(
    formData,
    animationContainer,
    formId
  ) {
    return (
      !formData ||
      !animationContainer ||
      !isAnimationForm(formData) ||
      KAMPYLE_FUNC.hasApplicationLoadingFailed(formId, false)
    );
  }
  function animateIn(eventName, data) {
    ...
    var formData = KAMPYLE_DATA.getFormData(data.formId);
    var animationContainer = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.CONTAINER_ID
    );
    if (
      shouldIgnoreRenderAnimationForm(formData, animationContainer, data.formId)
    ) {
      return false;
    }
    KAMPYLE_DATA.setMemoryData("isInvitationShouldBeIgnored", true);
    var formIframe = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.IFRAME_ID + data.formId
    );
    var v1IframeStyle = {
      display: "block",
      backgroundColor: formData.designSettings.formBackgroundColor || "white",
    };
    var v2IframeStyle = {
      display: "block",
    ...
    if (
      ...
      v2IframeStyle.maxWidth = "100vw";
      v2IframeStyle.maxHeight = "100vh";
    }
    ...
    switch (displayDirection) {
      ...
      default:
        KAMPYLE_UTILS.showWarning(
          "Invalid animation type - " + displayDirection
        );
        return false;
    }
    if (formIframe) {
  ...
  function refreshFormIframe(eventName, data) {
    setTimeout(function () {
      var formData = KAMPYLE_DATA.getFormData(data.formId);
      var currIframe = document.getElementById(
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.IFRAME_ID + data.formId
      );
      if (!!currIframe) {
        currIframe.remove();
        KAMPYLE_UTILS.removeEventHandler(
          window,
          "resize",
          KAMPYLE_ANIMATION.debouncedOnResize
        );
        KAMPYLE_DATA.initOnsiteDefaultState(data.formId);
      }
  ...
  function getMiddleHeight(formHeight) {
    return (
      String(Math.max(0, (window.innerHeight - parseInt(formHeight, 10)) / 2)) +
      "px"
    );
  }
  ...
  function setAnimationContainerInitialPosition(formData, animationContainer) {
    ...
    switch (getDisplayDirection(formData)) {
      ...
      default:
        KAMPYLE_UTILS.showWarning(
          "Invalid animation type - " + formData.formDisplayDirection
        );
        return false;
    }
  }
  ...
  function getOrCreateAnimationContainer(shouldAdjustToZoom) {
    var container = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.CONTAINER_ID
    );
    if (!container) {
      var formSpanPlacer = KAMPYLE_UTILS.getOrCreateWrapperContainer(
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.WRAPPER_ID
      );
  ...
  function handleAnimationIframe(eventName, data) {
    ...
    var animationContainer =
      KAMPYLE_ANIMATION.getOrCreateAnimationContainer(shouldAdjustToZoom);
    var formIframe = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.IFRAME_ID + data.formId
    );
    if (!formIframe) {
      ...
      if (shouldAdjustToZoom) {
        (animationIframeStyle.maxWidth = "100vw"),
          (animationIframeStyle.maxHeight = "100vh");
      }
      var isStyleImportant = false;
      KAMPYLE_VIEW.createFormIframe(
        data.formId,
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.IFRAME_ID,
        animationContainer,
        animationIframeStyle,
        isStyleImportant
      );
    }
    var spinnerContainer = animationContainer.querySelector(
      "#" + KAMPYLE_CONSTANT.SPINNER_SETTINGS.CONTAINER_ID
    );
    if (!spinnerContainer) {
      spinnerContainer = MDIGITAL_ELEMENT_BUILDER.createSpinnerElement();
      animationContainer.appendChild(spinnerContainer);
      KAMPYLE_UTILS.setSpinnerStyle(
        spinnerContainer,
        KAMPYLE_CONSTANT.SPINNER_SETTINGS.SIZE.LARGE
      );
    }
  }
  function initAnimationPosition(eventName, data) {
    ...
    if (
      ...
      !formData ||
      !isAnimationForm(formData) ||
      (isSkipInvitation && KAMPYLE_FUNC.isAnyOtherFormAlreadyShown(data.formId))
    ) {
      return false;
    }
    if (
  ...
  return {
    ...
    listenToResizeAnimation: listenToResizeAnimation,
    resizeAnimationContainer: resizeAnimationContainer,
    debouncedOnResize: debouncedOnResize,
  };
})();
window.KAMPYLE_VIEW = (function (window, document) {
  ...
  function handleInviteReady(eventName, data) {
    ...
    if (newMobileInvite()) {
      restoreHostingDOM();
      if (inviteContainer) {
        var isMobileWebBannerFixEnabled = KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.MOBILE_WEB_BANNER_FIX
        );
        KAMPYLE_UTILS.setElementStyle(
          inviteContainer,
          {
            ...
            right: "",
            background: "none",
            visibility: isMobileWebBannerFixEnabled ? "hidden" : "",
            display: isMobileWebBannerFixEnabled ? "" : "none",
          },
          true
        );
      }
  ...
  function showInviteIfNeeded(eventName, params) {
    var shouldShowParamName =
      KAMPYLE_TARGETING.generateShouldShowInviteParamName(params.formId);
  ...
  function updateInviteIframeTitle(eventName, data) {
    ...
    var browser = KAMPYLE_UTILS.getNestedPropertyValue(
      KAMPYLE_UTILS.getBrowser(),
      "name"
    );
  ...
  function handleInvitationSkippedForms(eventName, data) {
    ...
    KAMPYLE_UTILS.setElementStyle(
      document.getElementById("kampyleFormContainer"),
      { visibility: "visible" },
      true
    );
  ...
  function init() {
    var eventMapping = {
      ...
      neb_loadForm: [
        handleLightBoxIframe,
        showSpinner,
        applyMobileDynamicStyles,
      ],
      ...
      neb_inviteAccepted: [
        hideInvite,
        hideButton,
        removeListenersForHideInviteWhenVirtualKeyboardIsOpen,
      ],
    ...
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ADDITIONAL_BUTTON_STYLES
      )
    ) {
    ...
    if (!KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.CSP_RULES)) {
      ...
      KAMPYLE_UTILS.addStyle(
        ".wcagOutline:focus{outline: 1px dashed #595959 !important;outline-offset: 2px !important;transition: none !important;}"
      );
    }
    KAMPYLE_UTILS.setEventHandler(window, "resize", function (e) {
      updateIframeStyle();
    });
    KAMPYLE_DATA.setMemoryData("isOriginalHostingDom", true);
  }
  ...
  function listenToResize(eventName, data) {
    var formData = KAMPYLE_DATA.getFormData(data.formId);
    var shouldAdjustToZoom =
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ZOOM_WCAG_REFLOW
  ...
  function debouncedOnResize(formData) {
    var shouldBeImmediate = !KAMPYLE_DATA.isMobile();
    var onResizeWithFormData = KAMPYLE_VIEW.onResizeButton.bind(null, formData);
    var debounced = KAMPYLE_UTILS.debounce(
      onResizeWithFormData,
      100,
      shouldBeImmediate
    );
    KAMPYLE_VIEW.debouncedOnResize = debounced;
    return debounced;
  }
  function onResizeButton(formData) {
    var z = KAMPYLE_UTILS.getZoomedIn();
    var currentZoom = z.zoom;
    var wasZoomUnchanged =
      currentZoom &&
      currentZoom === KAMPYLE_DATA.getMemoryData("kampylePageZoom");
    if (wasZoomUnchanged) {
      return;
    }
    KAMPYLE_DATA.setMemoryData("kampylePageZoom", currentZoom);
    var btnStaticData = {
      buttonVerticalOffset:
        KAMPYLE_UTILS.getNestedPropertyValue(
          formData,
          "buttonData.verticalOffset"
        ) || 0,
      buttonType:
        KAMPYLE_UTILS.getNestedPropertyValue(formData, "buttonData.type") || "",
      buttonPosition: getButtonDirection(formData),
    };
    if (btnStaticData.buttonType === "custom") {
      var imgElement = document.querySelector("#nebula_div_btn > img");
      var shouldAdjustToZoom = true;
      handleLoadCustomBtnEvent(imgElement, btnStaticData, shouldAdjustToZoom);
    } else {
      resizeButton(btnStaticData, formData);
      setVerticalPosition("windowResize", btnStaticData);
    }
  }
  function unlistenToResize() {
    KAMPYLE_UTILS.removeEventHandler(
      window,
      "resize",
      KAMPYLE_VIEW.debouncedOnResize
    );
  }
  ...
  function getCornerMargin(formData) {
    formData = formData || {};
    var isInvite =
      KAMPYLE_UTILS.getNestedPropertyValue(formData, "formType") ===
      "invitation";
    var margin;
    if (!isInvite) {
      var buttonType = KAMPYLE_UTILS.getNestedPropertyValue(
        formData,
        "buttonData.type"
      );
      switch (buttonType) {
        ...
        case "custom":
          margin =
            KAMPYLE_UTILS.getNestedPropertyValue(
              formData,
              "buttonData.cornerMargin"
            ) || DEFAULTS.CORNER_MARGIN;
          break;
        default:
  ...
  function getButtonDirection(formData) {
    formData = formData || {};
    return (
      KAMPYLE_UTILS.getNestedPropertyValue(formData, "buttonData.position") ||
      "right"
    );
  }
  ...
  function setVerticalPosition(eventName, data) {
    var offset = 0;
    var buttonWidth = 35;
    var position = "";
    var buttonType = "";
    var formData =
      (data && data.formId && KAMPYLE_DATA.getFormData(data.formId)) || {};
    if (eventName === "windowResize") {
      offset = data.buttonVerticalOffset;
      position = data.buttonPosition;
      buttonType = data.buttonType;
    } else {
      var buttonData = KAMPYLE_UTILS.getNestedPropertyValue(
        formData,
        "buttonData"
      );
      offset =
        KAMPYLE_UTILS.getNestedPropertyValue(buttonData, "verticalOffset") || 0;
      position = buttonData.position;
      buttonType = KAMPYLE_UTILS.getNestedPropertyValue(buttonData, "type");
    }
    ...
    if (!isNaN(offset)) {
      ...
      if (nebButton && nebButton.style) {
        if (shouldAdjustToZoom) {
          var z = KAMPYLE_UTILS.getZoomedIn();
          offset = Number(offset) / z.zoom;
          if (z.isZoomedIn) {
            buttonWidth = buttonWidth * (z.constraint / z.zoom);
          }
        }
        if (position.indexOf("bottom") > -1) {
          ...
          nebButton.style.bottom =
            window.innerHeight * 0.55 - buttonWidth + offset + "px";
        }
      }
    }
  }
  function setZIndex(eventName, data) {
    var formData = KAMPYLE_DATA.getFormData(data.formId);
    var zIndex = KAMPYLE_UTILS.getNestedPropertyValue(
      formData,
      "buttonData.zIndex"
    );
  ...
  function showVisibility(eventName, data) {
    ...
    var formIframe = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.IFRAME_ID_PREFIX + currentFormId
    );
  ...
  function getWCAGClass() {
    return KAMPYLE_DATA.isFeatureEnabled(KAMPYLE_CONSTANT.FEATURE_FLAGS.WCAG)
      ? " wcagOutline "
      : " noOutline ";
  }
  function hideVisibility() {
    ...
    var noDataContainer = document.getElementById(
      "mdigital-no-json-modal_" + currentFormId
    );
  ...
  function hideForm(eventName, data) {
    ...
    if (kampyleFormContainer !== null) {
      var currIframe = document.getElementById(
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.IFRAME_ID_PREFIX +
          currentFormId
      );
      if (!!currIframe) {
        KAMPYLE_DATA.initOnsiteDefaultState(currentFormId);
        if (currIframe && currIframe.remove) {
          currIframe.remove();
        }
        KAMPYLE_UTILS.setElementStyle(
  ...
  function formatTriangularStyle(formData) {
    ...
    var isDynamicButtonSizeEnabled = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DYNAMIC_FEEDBACK_BUTTON_SIZE
    );
    var btnSize = 81;
    var fontSize = 14;
    var lineHeightTop = 53,
      lineHeightBottom = 108;
    var buttonStyles = {},
      height,
      buttonPadding,
      txtWidth;
    ...
    if (isDynamicButtonSizeEnabled) {
      var buttonTextSize = getTriangleTextSizeCalculation(formData);
      height = buttonTextSize.height;
      txtWidth = buttonTextSize.width;
      buttonStyles = getDynamicButtonStyles(buttonData.position);
      btnSize = height * 2;
      lineHeightTop = lineHeightBottom = "1.5";
      buttonPadding = buttonTextSize.padding;
    }
    if (isZoomSupported) {
      var z = KAMPYLE_UTILS.getZoomedIn();
      if (z.isZoomedIn) {
        btnSize = String(btnSize * (z.constraint / z.zoom));
        fontSize = String(fontSize * (z.constraint / z.zoom));
        if (isDynamicButtonSizeEnabled) {
          height = String(height * (z.constraint / z.zoom));
          buttonPadding = String(buttonPadding * (z.constraint / z.zoom));
          txtWidth = String(txtWidth * (z.constraint / z.zoom));
        } else {
          lineHeightTop = String(lineHeightTop * (z.constraint / z.zoom));
          lineHeightBottom = String(lineHeightBottom * (z.constraint / z.zoom));
        }
      }
    }
    btnSize = btnSize + "px";
    ...
    if (!isDynamicButtonSizeEnabled) {
      lineHeightTop = lineHeightTop + "px";
      lineHeightBottom = lineHeightBottom + "px";
    }
    var triangularStyle = [
      isDynamicButtonSizeEnabled
        ? ""
        : "button#nebula_div_btn { height: auto }",
      ...
      isDynamicButtonSizeEnabled
        ? "width:" +
          ...
          ";clip-path:" +
          buttonStyles.buttonPath +
          ";"
        : "background-color:transparent;",
      'font-family:"Open Sans",sans-serif;cursor:pointer;position:fixed;z-index:99999990',
      "}",
      ...
      isDynamicButtonSizeEnabled
        ? "height: auto;width:" +
          txtWidth +
          "px;margin: 0 auto;" +
          buttonStyles.paddingDirection +
          ":" +
          buttonPadding +
          "px;"
        : "height:" + btnSize + ";width:" + btnSize + ";",
      "text-align:center;z-index:1000;color:" +
        buttonData.textColor +
        ";font-size:" +
        fontSize +
        ";font-weight:normal;",
      "}",
      ".kampyle_feedback-button .kampyle_button-text.kampyle_bottom{",
      "line-height:" + lineHeightBottom + ";",
      "}",
      ".kampyle_feedback-button .kampyle_button-text.kampyle_top {",
      ...
      "top:0;border-top:" + btnSize + " solid;",
      "}",
      ...
      "}",
      isDynamicButtonSizeEnabled
        ? ".kampyle_shape_left { height:100%;width:50%;clip-path:" +
          buttonStyles.textPath.left +
          ";float:left;shape-outside:" +
          buttonStyles.textPath.left +
          "; }"
        : "",
      isDynamicButtonSizeEnabled
        ? ".kampyle_shape_right { height:100%;width:50%;clip-path:" +
          buttonStyles.textPath.right +
          ";float:right;shape-outside:" +
          buttonStyles.textPath.right +
          "; }"
        : "",
    ].join(" ");
  ...
  function getVerticalButtonWidth(buttonText) {
    var textWidth = KAMPYLE_VIEW.getVerticalButtonTextWidth(buttonText);
    var verticalPaddingWidth = 40;
    return textWidth + verticalPaddingWidth;
  }
  function formatVerticalStyle(formData) {
    var buttonData = formData.buttonData || {};
    var textScaling = "";
    if (MDIGITAL.LOCALIZATION.isFormLanguageRtl(formData)) {
      textScaling =
    ...
    if (
      ...
      width = getVerticalButtonWidth(buttonText);
    }
    var side = (-width + height) / 2;
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ZOOM_WCAG_REFLOW
      ) &&
      ...
      var z = KAMPYLE_UTILS.getZoomedIn();
      if (z.isZoomedIn) {
        width = width * (z.constraint / z.zoom);
        height = height * (z.constraint / z.zoom);
        fontSize = fontSize * (z.constraint / z.zoom);
        side = side * (z.constraint / z.zoom);
      }
    }
    var VpHeight = window.innerHeight;
    var buttonPosition = VpHeight * 0.55 - width;
    var verticalStyle = [
      "button#nebula_div_btn { height: auto }",
      ".kampyle_vertical_button {",
      'background-color:transparent;font-family:"Open Sans",sans-serif;cursor:pointer;position:fixed;bottom:' +
        buttonPosition +
        "px;z-index:99999990;height:" +
        height +
        "px;min-height:" +
        height +
        "px;max-height:" +
        ...
        "px;min-width:" +
        width +
        "px;-ms-transform:rotate(90deg);-webkit-transform:rotate(90deg);transform:rotate(90deg)",
      "}",
      ...
      "color:" +
        ...
        "px;line-height:" +
        height +
        "px;text-align:center;font-weight:normal;" +
        textScaling,
      "}",
  ...
  function getTriangleTextSizeCalculation(formData) {
    var buttonText = KAMPYLE_VIEW.getButtonText(formData);
    var shouldReverseText = formData.buttonData.position.includes("bottom");
    if (shouldReverseText) {
      buttonText = buttonText.split("").reverse().join("");
    }
    var buttonStyles = getDynamicButtonStyles("topRight");
    var buttonEl = document.createElement("div");
    KAMPYLE_UTILS.addAttributesOnElement(buttonEl, {
      id: "nebula_div_btn",
      class: "kampyle_feedback-button",
    });
    KAMPYLE_UTILS.setElementStyle(buttonEl, {
      visibility: "hidden",
      position: "fixed",
      border: "none",
      width: KAMPYLE_CONSTANT.BUTTON.MAX_WIDTH + "px",
      height: KAMPYLE_CONSTANT.BUTTON.MAX_WIDTH / 2 + "px",
    });
    var buttonTxt = document.createElement("div");
    KAMPYLE_UTILS.addAttributesOnElement(buttonTxt, {
      class: "kampyle_button-text",
    });
    KAMPYLE_UTILS.setElementStyle(buttonTxt, {
    ...
    var buttonTxtSpan = document.createElement("span");
    buttonTxtSpan.textContent = buttonText;
    buttonTxt.appendChild(buttonTxtSpan);
    var shapeLeft = document.createElement("div");
    KAMPYLE_UTILS.setElementStyle(shapeLeft, {
      height: "100%",
      width: "50%",
      float: "left",
      clipPath: buttonStyles.textPath.left,
      shapeOutside: buttonStyles.textPath.left,
    });
    var shapeRight = document.createElement("div");
    KAMPYLE_UTILS.setElementStyle(shapeRight, {
      height: "100%",
      width: "50%",
      float: "right",
      clipPath: buttonStyles.textPath.right,
      shapeOutside: buttonStyles.textPath.right,
    });
    buttonEl.appendChild(shapeLeft);
    buttonEl.appendChild(shapeRight);
    buttonEl.appendChild(buttonTxt);
    document.body.appendChild(buttonEl);
    var size = {
      width: buttonTxtSpan.offsetWidth + 1,
      height: buttonTxt.offsetHeight + 1,
    };
    buttonEl.removeChild(shapeLeft);
    buttonEl.removeChild(shapeRight);
    buttonTxt.style.width = size.width + "px";
    var textHeightWithoutShapesGap = buttonTxt.offsetHeight;
    size.height = Math.max(size.height, KAMPYLE_CONSTANT.BUTTON.MIN_HEIGHT);
    size.padding = size.height - textHeightWithoutShapesGap;
    document.body.removeChild(buttonEl);
    return size;
  }
  function getTriangleButtonStyle(formData) {
    var buttonText = KAMPYLE_VIEW.getButtonText(formData);
    var classDirection = getClassDirectonByPosition(
      formData.buttonData.position
    );
    var kampyleTriangle = '<div class="kampyle_triangle"></div>',
      kampyleShape = "";
    var backgroundStyle = "background: none;";
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DYNAMIC_FEEDBACK_BUTTON_SIZE
      )
      ...
      kampyleShape =
        '<div class="kampyle_shape_left"></div><div class="kampyle_shape_right"></div>';
      backgroundStyle = "";
      kampyleTriangle = "";
    }
    return (
      '<button id="nebula_div_btn" style="border: none; ' +
      backgroundStyle +
      '" alt="' +
      buttonText +
  ...
  function displayGenericFormBtn(formData) {
    var btnBuilders = {
      vertical: { html: getVerticalButtonStyle, style: formatVerticalStyle },
      triangular: {
    ...
    if (!btnBuilders) {
      KAMPYLE_UTILS.showWarning(
        "Invalid button type " + formData.buttonData.type
      );
      return "";
    }
  ...
  function displayCustomBtn(formData) {
    var divStyle =
      "z-index: 99999990;position: fixed;cursor: pointer;line-height:1px; padding:0;";
    var buttonText = KAMPYLE_VIEW.getButtonText(formData);
    var imageBtnStyles = [
      ".nebula_image_button {",
      "width: auto;",
    ...
    return (
      '<button id="nebula_div_btn" class="nebula_image_button ' +
      getWCAGClass() +
      '" style="' +
      divStyle +
      ';border: none" tabindex="0"><img alt="' +
      buttonText +
      '" src="' +
      formData.buttonData.buttonUrl +
      '" /></button>'
    );
  }
  ...
  function resetFormState(formId) {
    var isInvite =
      KAMPYLE_DATA.getFormTriggerType(formId) ===
      KAMPYLE_CONSTANT.FORM_TYPES.INVITATION;
    var formJsonMemoryKey = KAMPYLE_DATA.getMemoryJsonKey(formId, isInvite);
    var progressFetchingKey = KAMPYLE_DATA.getMemoryJsonKey(
      formId,
      isInvite,
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.FETCHING_DATA_IN_PROGRESS
    );
    ...
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.NEB_RESET_FORM_STATE,
      { formId: formId }
    );
  }
  function getErrorFetchedModal(formId) {
    ...
    var errorMessageTitle =
      KAMPYLE_CONSTANT.TEXTS.DEFAULT_JSON_ERROR_MESSAGES.title;
    var errorMessageBody =
      KAMPYLE_CONSTANT.TEXTS.DEFAULT_JSON_ERROR_MESSAGES.body;
    var errorMessageClose =
      KAMPYLE_CONSTANT.TEXTS.DEFAULT_JSON_ERROR_MESSAGES.close;
    errorMessageTitle = MDIGITAL.PROPERTY_MESSAGES.getMessageByTypeAndKey(
      KAMPYLE_CONSTANT.PROPERTY_MESSAGES.NO_JSON_MESSAGE.TYPE,
      KAMPYLE_CONSTANT.PROPERTY_MESSAGES.NO_JSON_MESSAGE.KEYS.TITLE
    );
    errorMessageBody = MDIGITAL.PROPERTY_MESSAGES.getMessageByTypeAndKey(
      KAMPYLE_CONSTANT.PROPERTY_MESSAGES.NO_JSON_MESSAGE.TYPE,
      KAMPYLE_CONSTANT.PROPERTY_MESSAGES.NO_JSON_MESSAGE.KEYS.BODY
    );
    errorMessageClose = MDIGITAL.PROPERTY_MESSAGES.getMessageByTypeAndKey(
      KAMPYLE_CONSTANT.PROPERTY_MESSAGES.NO_JSON_MESSAGE.TYPE,
      KAMPYLE_CONSTANT.PROPERTY_MESSAGES.NO_JSON_MESSAGE.KEYS.CLOSE
    );
    errorModalLang =
      MDIGITAL.PROPERTY_MESSAGES.getPropertyMessageLanguageCode();
    var NO_JSON_MODAL_ID = "mdigital-no-json-modal_" + formId;
    return MDIGITAL_ELEMENT_BUILDER.getErrorModal(
      NO_JSON_MODAL_ID,
      errorModalLang,
      errorMessageTitle,
      errorMessageBody,
  ...
  function handleButtonClicked(eventName, data) {
    ...
    if (displayType === "lightbox") {
      var kampyleFormContainer = document.getElementById(
        "kampyleFormContainer"
      );
      if (
        !kampyleFormContainer ||
        KAMPYLE_FUNC.hasApplicationLoadingFailed(data.formId, false) ||
        KAMPYLE_FUNC.isAnyOtherFormAlreadyShown(data.formId, true)
      ) {
        return false;
      }
      KAMPYLE_UTILS.setElementStyle(
  ...
  function getStyleDirectionByPosition(position, btnSize) {
    var styleDirection = {};
    switch (position) {
      case "bottomLeft":
        styleDirection = {
          "border-bottom-width": btnSize,
          "border-right-width": btnSize,
        };
        break;
      case "bottomRight":
        styleDirection = {
          "border-bottom-width": btnSize,
          "border-left-width": btnSize,
        };
        break;
      case "topLeft":
        styleDirection = {
          "border-top-width": btnSize,
          "border-right-width": btnSize,
        };
        break;
      case "topRight":
  ...
  function shouldShowErrMsgOnForm(formId) {
    ...
    return (
      MDIGITAL.FORM_STATE.shouldFormBeShown(formId) &&
      (formTriggerType === KAMPYLE_CONSTANT.FORM_TYPES.BUTTON ||
        formTriggerType === KAMPYLE_CONSTANT.FORM_TYPES.CODE)
    );
  }
  function handleCloseButtonInErroModalKeydown(e, formId) {
    if (
      e.keyCode === KAMPYLE_CONSTANT.KEY_CODES.SPACE ||
      e.keyCode === KAMPYLE_CONSTANT.KEY_CODES.ENTER
    ) {
  ...
  function onErrorModalKeyPressed(e, formId) {
    ...
    var closeButtonEl = document.querySelector(
      "#mdigital-no-json-modal_" + formId + " #close-no-json-modal"
    );
  ...
  function showFormCouldNotFechtedErrorMsg(eventName, data) {
    if (
      KAMPYLE_FUNC.isAnyOtherFormAlreadyShown(data.formId) ||
      !KAMPYLE_VIEW.shouldShowErrMsgOnForm(data.formId)
    ) {
      return;
    }
    ...
    if (document.getElementById(NO_JSON_MODAL_ID)) {
      ...
      var closeBtnElem = document.querySelector(
        "#" + NO_JSON_MODAL_ID + " #close-no-json-modal"
      );
      closeBtnElem.onclick = onErrModalClose.bind(null, data.formId);
      if (
        KAMPYLE_DATA.isFeatureEnabled(KAMPYLE_CONSTANT.FEATURE_FLAGS.WCAG) ||
        KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.NEW_MODAL_DIALOGS
        )
      ) {
    ...
    KAMPYLE_UTILS.triggerCustomEvent(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_NO_JSON_MODAL_SHOWN,
      { formId: data.formId }
    );
    ...
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_NO_JSON_MODAL_SHOWN,
      { additionalInfo: payload, formId: data.formId }
    );
  }
  function showFeedbackButtonVisually() {
    ...
    KAMPYLE_UTILS.setElementStyle(
      button,
      getVisualHideOrShowStyles(false),
      false
    );
  ...
  function hideFeedbackButtonVisually() {
    ...
    KAMPYLE_UTILS.setElementStyle(
      button,
      getVisualHideOrShowStyles(true),
      false
    );
  ...
  function showButton(eventName, data) {
    if (
      ...
      document.getElementById(
        KAMPYLE_CONSTANT.VIEW.FEEDBACK_BUTTON_CONTAINER_ID
      ) != null &&
  ...
  function triggerDisplayEvent(evetName, data) {
    ...
    KAMPYLE_EVENT_DISPATCHER.trigger("neb_button_displayed", {
      ...
      position:
        ...
        "",
      formClosed:
        KAMPYLE_UTILS.getNestedPropertyValue(data, "formClosed") || false,
    });
  }
  function getVisualHideOrShowStyles(isHide) {
    var cachedButtonPosition =
      KAMPYLE_DATA.getMemoryData(
        KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.FEEDBACK_BUTTON_POSITION_STYLE
      ) || {};
    var transformValue = "";
    var distance = isHide ? "9999px" : 0;
    switch (cachedButtonPosition) {
      case "left":
        transformValue = "rotate(90deg) translateY(" + distance + ")";
        break;
      case "right":
        transformValue = "rotate(90deg) translateY(-" + distance + ")";
        break;
      ...
      case "topRight":
        if (
          ...
          transformValue = distance
            ? "translateX(" + distance + ")"
            : getDynamicButtonStyles(cachedButtonPosition).transform;
          break;
        }
  ...
  function handleLoadCustomBtnEvent(
    imgElement,
    btnStaticData,
    shouldAdjustToZoom
  ) {
    if (
      Number(imgElement.naturalWidth) > 0 &&
      Number(imgElement.naturalHeight) > 0
    ) {
      var buttonEl = document.getElementById("nebula_div_btn");
      var imgData = {
        el: imgElement,
        width: imgElement.naturalWidth,
        height: imgElement.naturalHeight,
      };
      styleCustomBtn(buttonEl, imgData, btnStaticData, shouldAdjustToZoom);
      KAMPYLE_UTILS.removeEventHandler(
        imgElement,
        "load",
  ...
  function createButton(eventName, data) {
    ...
    if (!KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.CSP_RULES)) {
      switch (buttonType.toLowerCase()) {
        ...
        case "triangular":
          ...
          break;
        case "custom":
          btnHtml = displayCustomBtn(formData);
          break;
        default:
    ...
    if (buttonType.toLowerCase() === "custom") {
      var shouldAdjustToZoom =
        KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ZOOM_WCAG_REFLOW
        ) && KAMPYLE_UTILS.getNestedPropertyValue(formData, "urlVersion");
      var imgElement = document.querySelector("#nebula_div_btn > img");
      this.boundHandleLoadCustomBtnEvent = handleLoadCustomBtnEvent.bind(
        this,
        imgElement,
        btnStaticData,
        shouldAdjustToZoom
    ...
    if (
      ...
      var positionStyle = btnStaticData.buttonPosition;
      if (buttonType.toLowerCase() === "custom") {
        positionStyle =
          btnStaticData.buttonPosition.toLowerCase().indexOf("right") > -1
            ? "customRight"
            : "customLeft";
      }
      KAMPYLE_DATA.setMemoryData(
    ...
    var isDesktop =
      ...
      KAMPYLE_CONSTANT.DEVICES.DESKTOP;
    if (isDesktop) {
      this.boundHandleLoadFormEvent = handleLoadFormEvent.bind(
        this,
        data,
        spanContainer
      );
      KAMPYLE_UTILS.setEventHandler(
        spanContainer,
        "mouseover",
        this.boundHandleLoadFormEvent
      );
      var buttonProximityFunc = function (event) {
        var formState = KAMPYLE_DATA.getFormState(data.formId);
        var skipOnInviteLoadedAndShown =
          KAMPYLE_FUNC.checkProvision(
            KAMPYLE_CONSTANT.PROVISIONS.ENABLE_INVITATION_SPINNER_FIX
          ) && KAMPYLE_DATA.getMemoryData("inviteLoadedAndShown");
        if (
          !formState ||
          formState.loading ||
          formState.loaded ||
          skipOnInviteLoadedAndShown ||
          KAMPYLE_DATA.getMemoryData(
            KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.IS_BUTTON_CONTAINER_CREATED
          )
        ) {
    ...
    spanContainer.onkeyup = function (event) {
      switch (event.keyCode) {
        case KAMPYLE_CONSTANT.KEY_CODES.TAB:
          if (!MDIGITAL.FORM_STATE.isFormLoaded(data.formId)) {
            KAMPYLE_EVENT_DISPATCHER.trigger("neb_loadForm", {
              formId: data.formId,
            });
          }
  ...
  function setIframeOrigin(eventName, data) {
    if (data.origin != undefined) {
      var formIframe = document.getElementById(
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.IFRAME_ID_PREFIX + data.formId
      );
  ...
  function styleVerticalBtnToZoom(z, buttonEl, buttonTxt) {
    var rightVertical = document.querySelector(
      ".kampyle_vertical_button.kampyle_right"
    );
    var leftVertical = document.querySelector(
      ".kampyle_vertical_button.kampyle_left"
    );
    var buttonBg = document.querySelector(".kampyle_button");
    var constraint = z.constraint;
    var height = 35;
    var width = 125;
    var fontSize = 14;
    if (
      ...
      width = getVerticalButtonWidth(buttonTxt.textContent);
    }
    var side = (-width + height) / 2;
    width = z.isZoomedIn
      ? String(width * (constraint / z.zoom)) + "px"
      : width + "px";
    height = z.isZoomedIn
      ? String(height * (constraint / z.zoom)) + "px"
      : height + "px";
    side = z.isZoomedIn
      ? String(side * (constraint / z.zoom)) + "px"
      : side + "px";
    fontSize = z.isZoomedIn
      ? String(fontSize * (constraint / z.zoom)) + "px"
      : fontSize + "px";
    KAMPYLE_UTILS.setElementStyle(
      buttonEl,
      {
    ...
    KAMPYLE_UTILS.setElementStyle(rightVertical, { right: side }, false);
    KAMPYLE_UTILS.setElementStyle(leftVertical, { left: side }, false);
    KAMPYLE_UTILS.setElementStyle(
      buttonBg,
      {
        width: width,
    ...
    KAMPYLE_UTILS.setElementStyle(
      buttonTxt,
      { lineHeight: height, fontSize: fontSize },
      false
    );
  }
  function styleTriangularBtnToZoom(z, buttonEl, buttonTxt, formData) {
    var isDynamicButtonSizeEnabled = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DYNAMIC_FEEDBACK_BUTTON_SIZE
    );
    var width = 81,
    ...
    var buttonStyles = {},
      ...
      lineHeight,
      buttonPadding;
    var buttonPosition = getButtonDirection(formData);
    if (isDynamicButtonSizeEnabled) {
      var buttonTextSize =
        KAMPYLE_VIEW.getTriangleTextSizeCalculation(formData);
      buttonStyles = KAMPYLE_VIEW.getDynamicButtonStyles(buttonPosition);
      height = buttonTextSize.height;
      width = buttonTextSize.height * 2;
      buttonPadding = buttonTextSize.padding;
      txtWidth = buttonTextSize.width;
      lineHeight = 1.5;
      if (z.isZoomedIn) {
        var multiplier = constraint / z.zoom;
        txtWidth = txtWidth * multiplier;
        height = height * multiplier;
        buttonPadding = buttonPadding * multiplier;
      }
    } else {
      var lineHeightTop = 53;
      var lineHeightBottom = 108;
      lineHeight =
        buttonPosition.indexOf("top") === 0 ? lineHeightTop : lineHeightBottom;
      lineHeight = z.isZoomedIn
        ? String(lineHeight * (constraint / z.zoom)) + "px"
        : lineHeight + "px";
    }
    var btnSize = z.isZoomedIn
      ? String(width * (constraint / z.zoom)) + "px"
      : width + "px";
    fontSize = z.isZoomedIn
      ? String(fontSize * (constraint / z.zoom)) + "px"
    ...
    if (isDynamicButtonSizeEnabled) {
      ...
      buttonTxtStyle[buttonStyles.paddingDirection] = buttonPadding + "px";
      KAMPYLE_UTILS.setElementStyle(buttonTxt, buttonTxtStyle, false);
    } else {
      KAMPYLE_UTILS.setElementStyle(
        buttonEl,
        { lineHeight: lineHeight },
      ...
      var triangleBackground = document.querySelector(".kampyle_triangle");
      var responsiveBorderWidth = getStyleDirectionByPosition(
        buttonPosition,
        btnSize
      );
      KAMPYLE_UTILS.setElementStyle(
  ...
  function getVerticalButtonTextWidth(textContent) {
    var el = document.createElement("div");
    el.style.position = "absolute";
    el.style.visibility = "hidden";
    el.style.width = "auto";
    el.style.font = '14px "Open Sans", sans-serif';
  ...
  function getDynamicButtonStyles(buttonPosition) {
    var clipPathRight, clipPathLeft, transform, paddingDirection, buttonPath;
    switch (buttonPosition) {
      case "topLeft":
        buttonPath = "polygon(0 100%, 50% 0%, 100% 100%)";
        clipPathRight = "polygon(0% 0%, 100% 100%, 100% 0%)";
        clipPathLeft = "polygon(0% 100%, 0% 0%, 100% 0%)";
        transform = "rotate(-45deg) translate(-18%, -58%)";
        paddingDirection = "padding-top";
        break;
      case "topRight":
        buttonPath = "polygon(0 100%, 50% 0%, 100% 100%)";
        clipPathRight = "polygon(0% 0%, 100% 100%, 100% 0%)";
        ...
        break;
      case "bottomLeft":
        buttonPath = "polygon(0 0%, 50% 100%, 100% 0%)";
        clipPathRight = "polygon(0% 100%, 100% 0%, 100% 100%)";
        clipPathLeft = "polygon(0% 100%, 0% 0%, 100% 100%)";
        transform = "rotate(45deg) translate(-18%, 58%)";
        paddingDirection = "padding-bottom";
        break;
      case "bottomRight":
        buttonPath = "polygon(0 0%, 50% 100%, 100% 0%)";
        clipPathRight = "polygon(0% 100%, 100% 0%, 100% 100%)";
    ...
    return {
      textPath: { right: clipPathRight, left: clipPathLeft },
      buttonPath: buttonPath,
      transform: transform,
      paddingDirection: paddingDirection,
    };
  }
  function getBottomPositionCustomBtn(buttonPosition, imgHeight, offset) {
    var vertical = {};
    if (buttonPosition.indexOf("bottom") > -1) {
      vertical = { bottom: 0, marginBottom: offset + "px", top: "unset" };
    } else if (buttonPosition.indexOf("top") > -1) {
      vertical = { top: 0, marginTop: offset * -1 + "px", bottom: "unset" };
    } else {
      vertical = {
        bottom: window.innerHeight * 0.5 - imgHeight + "px",
  ...
  function styleCustomBtn(
    ...
    var multiplier = shouldAdjustToZoom
      ? 1 / KAMPYLE_UTILS.getZoomedIn().zoom
      : 1;
    var imgElement = imgData
      ? imgData.el
      : document.querySelector("#nebula_div_btn>img");
    var imgNaturalWidth = imgData ? imgData.width : imgElement.naturalWidth;
    var imgNaturalHeight = imgData ? imgData.height : imgElement.naturalHeight;
    var imgWidth = String(imgNaturalWidth * multiplier);
    var imgHeight = String(imgNaturalHeight * multiplier);
    var offset = btnStaticData.buttonVerticalOffset;
    offset = isNaN(offset) ? 0 : Number(offset) * multiplier;
    var verticalPositioning = getBottomPositionCustomBtn(
    ...
    KAMPYLE_UTILS.setElementStyle(buttonEl, verticalPositioning, true);
    KAMPYLE_UTILS.setElementStyle(
      imgElement,
  ...
  function resizeButton(btnStaticData, formData) {
    var buttonEl = document.getElementById("nebula_div_btn");
    if (buttonEl === null) {
      return;
    }
    var buttonTxtEl = document.querySelector(".kampyle_button-text");
    var browserZoom = KAMPYLE_UTILS.getZoomedIn();
    if (btnStaticData.buttonType === "vertical") {
      styleVerticalBtnToZoom(
        browserZoom,
        buttonEl,
        buttonTxtEl,
        btnStaticData.buttonPosition
  ...
  function updateIframeStyle(formData) {
    var currentFormId =
      KAMPYLE_DATA.getMemoryData("currentFormId") ||
      KAMPYLE_DATA.getShownFormId();
    if (
      document.getElementById("kampyleFormContainer") === null ||
      !currentFormId
    ) {
      return;
    }
  ...
  function updateDynamicIframeStyle(formData) {
    var spanContainer = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.WRAPPER_ID
    );
    if (spanContainer) {
  ...
  function openPopup(formUrl, formId, formWidth, formHeight, eventName) {
    ...
    var popupSettings =
      "width=" + formWidth + ", height=" + formHeight + ",scrollbars=yes";
    var popupHandler = window.open(formUrl, popupPageName, popupSettings);
    var isPopupBlocked =
      !popupHandler ||
      popupHandler.closed ||
      typeof popupHandler.closed == "undefined";
    KAMPYLE_DATA.setMemoryData("popupHandler", popupHandler);
    setTimeout(function () {
      if (isPopupBlocked) {
        if (eventName !== "neb_skipInvitation") {
          alert(
            "The survey was blocked, please disable popup blocker and refresh the page."
          );
        }
  ...
  function showForm(eventName, data) {
    ...
    var formData = KAMPYLE_DATA.getFormData(data.formId);
    if (
      KAMPYLE_VIEW.ignoreShowForm(formData) ||
      KAMPYLE_FUNC.hasApplicationLoadingFailed(data.formId, false) ||
      KAMPYLE_FUNC.isAnyOtherFormAlreadyShown(data.formId, true)
    ) {
      return false;
    ...
    var shouldOptimizeSkipInvitation =
      isSkipInvitationEvent &&
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.SKIP_INVITATION_OPTIMIZATION
      );
    if (displayType === "lightbox") {
  ...
  function changeInviteHeight(eventName, data) {
    ...
    var isInviteShown = (KAMPYLE_DATA.getFormState(data.formId) || {})
      .inviteShown;
  ...
  function setMetaViewport(optional) {
    ...
    var isWCAGUserScalable = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.FORM_TEMPLATES_WCAG
    )
      ? "width=device-width, initial-scale=1.0"
      : "width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no";
    var kampyleMeta = document.getElementById("kampyleMetaViewport");
    var viewportAmendContentAttributeOnlyProvision =
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_VIEWPORT_LOGIC_FIX
      );
    var savedViewportContent =
      KAMPYLE_DATA.getMemoryData("hostedViewportContent") || [];
    if (!savedViewportContent.length) {
      var hostedViewportElements = document.getElementsByName("viewport");
      var isHostedViewportElementExists =
        hostedViewportElements && hostedViewportElements.length > 0;
      if (isHostedViewportElementExists) {
        for (var i = hostedViewportElements.length - 1; i >= 0; i--) {
          var viewportContent =
            hostedViewportElements[i].getAttribute("content");
          if (viewportContent) {
            if (viewportAmendContentAttributeOnlyProvision) {
              savedViewportContent.push({
                el: hostedViewportElements[i],
                content: viewportContent,
              });
            } else {
              savedViewportContent.push(hostedViewportElements[i]);
            }
          }
        }
      }
    }
    var shouldSetMetaContentInsteadOfElement =
      viewportAmendContentAttributeOnlyProvision &&
      !!savedViewportContent.length &&
      !kampyleMeta;
      ...
      case "add":
        if (savedViewportContent && savedViewportContent.length) {
          for (var i = savedViewportContent.length - 1; i >= 0; i--) {
            if (shouldSetMetaContentInsteadOfElement) {
              if (savedViewportContent[i].el) {
                savedViewportContent[i].el.setAttribute(
                  "content",
                  isWCAGUserScalable
                );
              }
            } else if (!savedViewportContent[i].el) {
              if (
                savedViewportContent[i].hasAttribute("id") &&
                savedViewportContent[i].id === "kampyleMetaViewport"
              ) {
                continue;
              } else {
          ...
          for (var i = savedViewportContent.length - 1; i >= 0; i--) {
            if (shouldSetMetaContentInsteadOfElement) {
              if (savedViewportContent[i].el) {
                savedViewportContent[i].el.setAttribute(
                  "content",
                  savedViewportContent[i].content
                );
              }
            } else if (!savedViewportContent[i].el) {
              if (
                ...
                document
                  ...
                  .appendChild(savedViewportContent[i]);
              }
            }
          }
        }
  ...
  function newMobileInvite() {
    return (
      Number(KAMPYLE_DATA.getMemoryData("inviteVersion")) > 0 &&
      KAMPYLE_DATA.isNewMobileInvitation()
    );
  }
  ...
  function showInvite(eventName, data) {
    ...
    var inviteTop = "25%";
    var inviteTransform = "none";
    if (
      !inviteData ||
      !MDIGITAL.FORM_STATE.isInviteLoaded(data.formId) ||
      !KAMPYLE_FUNC.isJsonDataLoaded(data.formId, true)
    ) {
      return false;
    }
    var shouldAdjustToZoom = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ZOOM_WCAG_REFLOW
    );
    var kampyleDevice = KAMPYLE_DATA.getMemoryData("kampyleDevice");
    var inviteMaxWidth = "unset";
    var inviteMaxHeight = "unset";
    if (
      kampyleDevice === KAMPYLE_CONSTANT.DEVICES.MOBILE ||
      kampyleDevice === KAMPYLE_CONSTANT.DEVICES.TABLET
    ) {
      ...
      inviteMaxWidth = "100vw";
      inviteMaxHeight = "100vh";
      inviteTop = 0;
    } else if (shouldAdjustToZoom) {
      inviteMaxWidth = "100vw";
      inviteMaxHeight = "100vh";
      inviteTop = "50%";
      inviteTransform = "translateY(-50%)";
    }
    ...
    if (
      inviteContainer &&
      KAMPYLE_DATA.getMemoryData("formIsShown") === false
    ) {
      KAMPYLE_UTILS.setElementStyle(
        inviteContainer,
        {
          display: "block",
          visibility: KAMPYLE_DATA.isNewMobileInvitation()
            ? "hidden"
            : "visible",
        },
        true
      ...
      if (inviteModal) {
        KAMPYLE_UTILS.setElementStyle(
          inviteModal,
          {
            ...
            "max-width": inviteMaxWidth,
            height: inviteData.inviteHeight + "px",
            "max-height": inviteMaxHeight,
            display: "block",
  ...
  function prepareHostingDOM() {
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_SPA_OVERFLOW_FIX
      ) &&
      !KAMPYLE_DATA.getMemoryData("isOriginalHostingDom")
    ) {
      return;
    }
    KAMPYLE_DATA.setMemoryData("isOriginalHostingDom", false);
    if (KAMPYLE_DATA.isMobileDynamicSizing()) {
      KAMPYLE_VIEW.manipulateAriaHiddens();
      KAMPYLE_DATA.setMemoryData(
        "hostedBodyOverflowXAttr",
        document.body.style.overflowX
      );
      document.body.style.overflowX = "hidden";
    } else {
      KAMPYLE_DATA.setMemoryData(
        "hostedBodyPositionAttr",
        document.body.style.position
      );
      if (
        KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.ENABLE_OVERFLOW)
      ) {
        return false;
      } else {
        KAMPYLE_DATA.setMemoryData(
          "hostedBodyOverflowAttr",
  ...
  function restoreHostingDOM() {
    ...
    if (KAMPYLE_DATA.isMobileDynamicSizing()) {
      ...
      document.body.style.overflowX =
        KAMPYLE_DATA.getMemoryData("hostedBodyOverflowXAttr") || "";
    } else {
      if (
        !KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.ENABLE_OVERFLOW
        )
      ) {
        document.body.style.overflow =
          KAMPYLE_DATA.getMemoryData("hostedBodyOverflowAttr") || "";
      }
      document.body.style.position =
        KAMPYLE_DATA.getMemoryData("hostedBodyPositionAttr") || "";
    }
  }
  function createFormContainer() {
    if (!KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.CSP_RULES)) {
      ...
      html +=
        '   <div id="kampyleFormModal" style="z-index:99999999 !important; -webkit-overflow-scrolling: touch !important;"></div>';
      html += " </div>";
      html += "</div>";
      ...
      if (
        KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.ENABLE_WCAG_PREPEND_SURVEY
        )
      ) {
  ...
  function handleLightBoxIframe(eventName, data) {
    ...
    var timeoutKey =
      KAMPYLE_CONSTANT.TIMEOUT_HANDLERS.FORM_APP_STATUS + "_" + data.formId;
    ...
    if (!formIframe) {
      ...
      if (
        !KAMPYLE_DATA.isCustomHtmlForm(data.formId) &&
        !KAMPYLE_DATA.isMobileDynamicSizing(formData.formId)
      ) {
        iframeStyleObject.position = "fixed";
      }
      var isStyleImportant = true;
      var lightboxContainer = document.getElementById(
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.CONTAINER_ID_PREFIX
      );
      KAMPYLE_VIEW.createFormIframe(
        data.formId,
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.IFRAME_ID_PREFIX,
  ...
  function createFormIframe(
    ...
    formIframe.setAttribute(
      "title",
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.IFRAME_TITLE
    );
  ...
  function applyMobileDynamicStyles(eventName, data) {
    ...
    if (
      !(formId && kFormContainer && KAMPYLE_DATA.isMobileDynamicSizing(formId))
    ) {
      return false;
    }
    KAMPYLE_UTILS.setElementStyle(
  ...
  function reactToIframeHeightChange(eventName, eventData) {
    if (KAMPYLE_DATA.isMobileDynamicSizing((eventData || {}).formId)) {
      ...
      var formHeight = KAMPYLE_UTILS.getNestedPropertyValue(
        eventData,
        "newHeight"
      );
  ...
  function restoreAriaHidden() {
    var storeValues =
      KAMPYLE_DATA.getMemoryData("initialAriaHiddenValues") || [];
    for (var i = 0; i < storeValues.length; i++) {
      if (
        !(storeValues[i].element && storeValues[i].element instanceof Element)
      ) {
        continue;
      }
      if (storeValues[i].prevAriaHiddenValue === null) {
        ...
        storeValues[i].element.setAttribute(
          "aria-hidden",
          storeValues[i].prevAriaHiddenValue
        );
      }
    }
  ...
  function createInviteIframe(eventName, eventData) {
    ...
    var inviteSpanPlacer = KAMPYLE_UTILS.getOrCreateWrapperContainer(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.INVITATION.WRAPPER_ID
    );
    if (!KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.CSP_RULES)) {
      var inviteContainerStyles =
        "top:0 !important;left:0 !important;width: 100% !important;";
      inviteContainerStyles +=
        "height: 100% !important;position: fixed !important;visibility:hidden !important;";
      inviteContainerStyles +=
        "display:none !important;background-color: rgba(22,22,22,0.5) !important;";
      inviteContainerStyles +=
        "z-index:99999999 !important;overflow:auto !important";
      ...
      var spinnerContainer = MDIGITAL_ELEMENT_BUILDER.createSpinnerElement();
      inviteSpanPlacer
        .querySelector("#kampyleInviteContainer")
        .appendChild(spinnerContainer);
      KAMPYLE_UTILS.setSpinnerStyle(
        spinnerContainer,
        KAMPYLE_CONSTANT.SPINNER_SETTINGS.SIZE.LARGE
    ...
    inviteIframe.title =
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.INVITATION.IFRAME_TITLE;
    inviteIframe.tabIndex = -1;
    var inviteHeight = data.inviteData.inviteHeight;
    var inviteWidth = data.inviteData.inviteWidth;
    var kampyleDevice = KAMPYLE_DATA.getMemoryData("kampyleDevice");
    var isWithReflowSupport = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ZOOM_WCAG_REFLOW
    );
    var applyFullscreenStyle =
      kampyleDevice === KAMPYLE_CONSTANT.DEVICES.MOBILE ||
      kampyleDevice === KAMPYLE_CONSTANT.DEVICES.TABLET;
    var inviteMaxWidth =
      isWithReflowSupport || applyFullscreenStyle ? "100vw" : "unset";
    var inviteMaxHeight =
      isWithReflowSupport || applyFullscreenStyle ? "100vh" : "unset";
    var inviteModal = document.getElementById("kampyleInviteModal");
    if (applyFullscreenStyle) {
      inviteHeight = "100%";
      inviteWidth = "100%";
      if (!!inviteModal) {
        KAMPYLE_UTILS.setElementStyle(
    ...
    if (!!inviteModal) {
      KAMPYLE_UTILS.setElementStyle(
        inviteModal,
        { maxWidth: inviteMaxWidth, maxHeight: inviteMaxHeight },
        true
      );
    }
    KAMPYLE_UTILS.setElementStyle(
      inviteIframe,
      {
        height: inviteHeight,
        width: inviteWidth,
        maxWidth: inviteMaxWidth,
        maxHeight: inviteMaxHeight,
        display: "none",
        border: "0",
      },
      true
    ...
    iframeSrc +=
      "&domainsListRelativePath=" +
      KAMPYLE_COMMON.getDomainsListRelativePath(iframeSrc);
  ...
  function removeAllShownElements(eventName, data) {
    ...
    var lightboxWrapper = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.WRAPPER_ID
    );
    var invitationWrapper = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.INVITATION.WRAPPER_ID
    );
    var animationWrapper = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.ANIMATION.WRAPPER_ID
    );
    var abandon = document.getElementById("kampyle_abandon_zone");
    KAMPYLE_VIEW.removeButton(eventName, data);
  ...
  function getButtonText(formData) {
    if (typeof MDIGITAL.LOCALIZATION !== "undefined") {
  ...
  function shouldAddScroll(formHeight) {
    ...
    return (
      (bodyPosition === "fixed" || bodyPosition === "absolute") &&
      KAMPYLE_UTILS.getDocumentSize().height <= formHeight
    );
  }
  return {
    ...
    unlistenToResize: unlistenToResize,
    ...
    getVerticalButtonTextWidth: getVerticalButtonTextWidth,
    init: init,
    ...
    getDynamicButtonStyles: getDynamicButtonStyles,
    getVerticalButtonWidth: getVerticalButtonWidth,
    getTriangleTextSizeCalculation: getTriangleTextSizeCalculation,
  };
})(window, document);
window.KAMPYLE_MESSAGE = (function () {
  var extensionActions = [
    "updateProfileFromExtension",
    "clearProfileFromExtension",
  ];
  function init() {
    KAMPYLE_UTILS.setEventHandler(window, "message", handleMessage);
  }
  function isAllowedSource(source) {
    var iframeConsts = KAMPYLE_CONSTANT.IFRAME_SETTINGS;
    var iframeIds = [
      iframeConsts.EMBEDDED.IFRAME_ID_PREFIX,
      iframeConsts.LIGHTBOX.IFRAME_ID_PREFIX,
      iframeConsts.INVITATION.IFRAME_ID,
      iframeConsts.ANIMATION.IFRAME_ID,
      iframeConsts.PREVIEW.IFRAME_ID,
    ];
    for (var i = 0; i < iframeIds.length; i++) {
      var iframeElements = document.querySelectorAll(
        'iframe[id^="' + iframeIds[i] + '"]'
      );
      for (var idx = 0; idx < iframeElements.length; idx++) {
        if (source === iframeElements[idx].contentWindow) {
          return true;
        }
      }
    }
    var popupHandler = KAMPYLE_DATA.getMemoryData("popupHandler");
    if (popupHandler && popupHandler === source) {
      return true;
    }
    return false;
  }
  function validateMessageEvent(e) {
    ...
    var isSourceCheckEnabled = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ONSITE_POST_MESSAGE_VERIFICATION
    );
    if (
      !e ||
      !e.origin ||
      !e.data ||
      (isSourceCheckEnabled && (!e.source || !isAllowedSource(e.source)))
    ) {
      return false;
    }
    ...
    if (
      isExtensionEnabled &&
      parsedData &&
      extensionActions.includes(parsedData.action)
    ) {
      return true;
    }
    var formData = KAMPYLE_DATA.getFormData(parsedData.formId);
    if (
      formData &&
      formData.formHtmlUrl &&
      KAMPYLE_UTILS.getParsedUrl(formData.formHtmlUrl).origin !== e.origin
    ) {
      return false;
    }
    ...
    return (
      KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.ON_PREM_HYBRID) ||
      e.origin === baseUrl
    );
  }
  function getUUID(data) {
    return KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.BACKEND_GENERATED_UUID
    )
      ? data.feedbackUUID
      : data.uuid;
  }
  function handleMessage(e) {
    ...
    var feedbackCorrelationUUID = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.BACKEND_GENERATED_UUID
    )
      ? data.feedbackCorrelationUUID
      : data.uuid;
    switch (data.action) {
      case "inviteAccepted":
      ...
      case "formSubmitted":
        var payload = {
          formId: data.formId,
          feedbackUUID: getUUID(data),
          feedbackCorrelationUUID: feedbackCorrelationUUID,
        };
      ...
      case "submitFeedbackSuccess":
        ...
        KAMPYLE_EVENT_DISPATCHER.trigger("neb_submitFeedbackSuccess", payload);
        break;
      case "formClose":
        var payload = {
          formId: data.formId,
      ...
      case "triggerScreenCapture":
        var payload = {
          ...
          feedbackCorrelationUUID: feedbackCorrelationUUID,
          screenCaptureUuid: KAMPYLE_FUNC.checkProvision(
            KAMPYLE_CONSTANT.PROVISIONS.USER_SNAP_AS_COMPONENT
          )
            ? data.screenCaptureUuid
            : null,
        };
      ...
      case "routeToForm":
        KAMPYLE_EVENT_DISPATCHER.trigger(
          "neb_routeToForm",
          { formId: data.destinationForm, sourceFormId: data.formId },
          null,
          { isStrictMode: true }
        );
        break;
      ...
      case "MDigital_Invalid_Origin":
        ...
        var isPopup =
          ...
          KAMPYLE_CONSTANT.DISPLAY_TYPES.POPUP;
        var isInvitation =
          KAMPYLE_DATA.getFormTriggerType(data.formId) ===
          KAMPYLE_CONSTANT.FORM_TYPES.INVITATION;
        var isSkipInvitation = KAMPYLE_UTILS.getNestedPropertyValue(
          KAMPYLE_DATA.getFormData(data.formId),
          "inviteData.skipInvitation"
        );
        if (
          !isPopup &&
          isInvitation &&
          MDIGITAL.FORM_STATE.isFormShown(data.formId)
        ) {
        ...
        KAMPYLE_DATA.setMemoryData(
          KAMPYLE_TARGETING.generateShouldShowInviteParamName(data.formId),
          false
        );
        if (isSkipInvitation) {
          KAMPYLE_DATA.setMemoryData("isInvitationShouldBeIgnored", true);
        ...
        break;
      case "updateProfileFromExtension":
        window.md_ex_profile = data && data.payload ? data.payload : null;
        if (window.md_ex_profile) {
          var cpMessage = KAMPYLE_FUNC.getCustomParamsMessage(data.formId);
          updateMessageForAllLoadedForms(cpMessage);
          if (window.md_ex_profile.authorizationToken) {
            KAMPYLE_DATA.setAuthorizationToken(
              window.md_ex_profile.authorizationToken
            );
            var authorizationMessage =
              KAMPYLE_MESSAGE.getAuthorizationMessage();
            updateMessageForAllLoadedForms(authorizationMessage);
          }
        }
        if (
          window.md_ex_profile &&
          typeof window.md_ex_profile === "object" &&
          Object.keys(window.md_ex_profile).length
        ) {
          KAMPYLE_DATA.initOnsiteDefaultState();
          KAMPYLE_FUNC.startCheckingTargeting();
        }
        break;
      case "clearProfileFromExtension":
        window.md_ex_profile = null;
        KAMPYLE_DATA.setAuthorizationToken(null);
        break;
      default:
        if (
          !KAMPYLE_FUNC.checkProvision(
            KAMPYLE_CONSTANT.PROVISIONS.ON_PREM_HYBRID
          )
        ) {
  ...
  function getAuthorizationMessage() {
    var message = {
      ...
      authorizationToken: KAMPYLE_DATA.getAuthorizationToken(),
      __isDigitalAction: true,
    };
    return JSON.stringify(message);
  }
  function updateMessageForAllLoadedForms(messageData) {
    var formsState = KAMPYLE_DATA.getMemoryData("formsState");
    for (var formId in formsState) {
      if (formsState[formId] && formsState[formId].loaded) {
        var formData = KAMPYLE_DATA.getFormData(formId);
        if (
          !formData ||
          !formData.formDisplay ||
          !formData.formDisplay.displayType
        ) {
          continue;
        }
        if (formData.formDisplay.displayType === "popup") {
          var targetWindow = KAMPYLE_DATA.getMemoryData("popupHandler");
          targetWindow.postMessage(messageData, "*");
        } else {
          KAMPYLE_UTILS.sendMessageToIframe(
            KAMPYLE_DATA.getFormIframeId(formId),
            messageData
          );
        }
      }
  ...
  return {
    init: init,
    updateMessageForAllLoadedForms: updateMessageForAllLoadedForms,
    getAuthorizationMessage: getAuthorizationMessage,
...
window.KAMPYLE_UTILS = {
  ...
  startsWith: function (originalString, searchString, position) {
    return (
      originalString.substr(position || 0, searchString.length) === searchString
    );
  },
  isIos: function () {
  ...
  isiPadOS13: function () {
    return (
      KAMPYLE_UTILS.isSafari() &&
      !KAMPYLE_UTILS.isiPhone() &&
  ...
  isDebugMode: function () {
    ...
    return (
      [
        KAMPYLE_CONSTANT.REGIONS.DEV,
        KAMPYLE_CONSTANT.REGIONS.QA,
        KAMPYLE_CONSTANT.REGIONS.COLO_CRT_QA1,
      ].indexOf(region) > -1
    );
  },
  ...
  addStyle: function (cssStyle, referencedDocument) {
    if (!KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.CSP_RULES)) {
      ...
      if (style === null) {
        ...
        var head =
          referencedDocument.head ||
          referencedDocument.getElementsByTagName("head")[0];
      ...
      if (style.styleSheet) {
        ...
        KAMPYLE_UTILS.setAnimationRule(
          KAMPYLE_CONSTANT.CSS_SETTINGS.ANIMATION_RULE_SPIN_NAME,
          "{0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); }}"
        );
      }
    } else {
      console.error(
        "Should not use this method if the provision csp rules is on"
      );
    }
  },
    ...
    return {
      width:
        ...
        (body.getBoundingClientRect
          ? body.getBoundingClientRect().left || 0
          : 0),
      height:
        height +
        (body.getBoundingClientRect
  ...
  getTimeDiff: function (time, format) {
    var datetime =
      typeof time !== "undefined" ? time : "2014-01-01 01:02:03.123456";
  ...
  setAnimationRule: function (ruleName, rule, iframe) {
    var allStyleSheets =
      (iframe && iframe.styleSheets && iframe.styleSheets) ||
      (!iframe && window.document.styleSheets);
    var mDigitalAnimationStylesheet = null;
    for (var i = 0; i < allStyleSheets.length; i++) {
      if (
        allStyleSheets[i].title ==
        KAMPYLE_CONSTANT.CSS_SETTINGS.STYLE_SHEET_ANIMATION_NAME
      ) {
    ...
    if (!mDigitalAnimationStylesheet) {
      ...
      if (iframe) {
        ...
        mDigitalAnimationStylesheet =
          iframe.styleSheets[iframe.styleSheets.length - 1];
      } else {
  ...
  replaceAll: function (str, find, replace) {
    return str.replace(
      new RegExp(KAMPYLE_UTILS.escapeRegExp(find), "g"),
      replace
    );
  },
  sendMessageToIframe: function (iframeId, message) {
    ...
    if (targetWindow && targetWindow.postMessage) {
      if (message) {
        message.__isDigitalAction = true;
      }
      targetWindow.postMessage(JSON.stringify(message), origin);
    }
  },
  getZoomedIn: function () {
    var browserZoom = { zoom: 1, constraint: 1, isZoomedIn: false };
    var browser = "";
    var windowDPR = 1;
    var isHighDPI = false;
    try {
      if (KAMPYLE_DATA.isMobile()) {
        browserZoom.zoom = window.visualViewport.scale;
      } else {
        browser = KAMPYLE_UTILS.getBrowser();
        windowDPR = window.devicePixelRatio;
        if (browser.name === KAMPYLE_CONSTANT.BROWSERS.SAFARI) {
          isHighDPI = windowDPR === 2;
          browserZoom.zoom = window.outerWidth / window.innerWidth;
        } else if (browser.name === KAMPYLE_CONSTANT.BROWSERS.FIREFOX) {
          if (windowDPR > 5) {
            isHighDPI = true;
          } else {
            isHighDPI = window.matchMedia(
          ...
          if (window.screen && window.screen.pixelDepth) {
            isHighDPI = window.screen.pixelDepth > 24;
          } else {
            isHighDPI =
              window.outerWidth - window.innerWidth * (windowDPR / 2) <= 60;
          }
          browserZoom.zoom = isHighDPI ? windowDPR * 0.5 : windowDPR;
        }
        browserZoom.constraint = 2;
      }
    } catch (e) {
      return browserZoom;
    }
    browserZoom.isZoomedIn = browserZoom.zoom >= 2.4;
    return browserZoom;
  },
  getBrowser: function () {
  ...
  kampyleGetUserId: function () {
    ...
    if (!userid) {
      ...
      if (
        KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.ALTERNATIVE_UUID
        )
      ) {
        KAMPYLE_DATA.setData(
          KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.MDIGITAL_ALTERNATIVE_UUID,
          KAMPYLE_UTILS.generateUUID()
        );
      }
    }
    return userid;
  ...
  kampyleSetCookie: function (
    ...
    var domain = propertySharedSubdomain
      ? ";domain=" + propertySharedSubdomain + ";"
      : "";
    var cookieFlag = KAMPYLE_UTILS.getCookieFlagByProtocol();
    cookieName = propertySharedSubdomain
      ? KAMPYLE_UTILS.TOP_LEVEL_DOMAIN_COOKIE_PREFIX + cookieName
      : cookieName;
    document.cookie =
      cookieName +
      "=" +
  ...
  getDomainFromHost: function (host) {
    ...
    var lastDomainCharacter =
      portStartIndex === -1 ? host.length : portStartIndex;
  ...
  getAllKampyleData: function () {
    ...
    if (
      KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.ALTERNATIVE_UUID)
    ) {
      kampyleDataNames.push(
        KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.MDIGITAL_ALTERNATIVE_UUID
      );
    }
    if (
  ...
  kampyleDeleteCookie: function (cookieName, propertySharedSubdomain) {
    ...
    document.cookie =
      cookieName + "= ; " + cookieFlag + expires + ";path=/" + domain;
  },
  ...
  kampyleCompareByOperator: function (source, toCompare, operator, varType) {
    ...
    switch (operator) {
      case KAMPYLE_CONSTANT.OPERATORS.EQUALS:
        if (varType === "datetime") {
          ans = KAMPYLE_UTILS.kampyleCompareTimestamps(
            source,
            toCompare,
            operator
          );
  ...
  getNestedPropertyValue: function (obj, propString, fallback) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var propOnObjPrototypeChain =
        tmpObj !== null && typeof tmpObj === "object" && currProp in tmpObj;
      if (
        !!tmpObj &&
        (Object.prototype.hasOwnProperty.call(tmpObj, currProp) ||
          propOnObjPrototypeChain ||
          tmpObj[currProp])
      ) {
  ...
  validateKampyleDomain: function (url) {
    var urlRegex =
      /(^https?:\/\/[A-Za-z0-9\-]+\.kampyle\.com(\/[A-Za-z0-9\-\._~:\/\?#\[\]@!$&'\(\)\*\+,;\=]*)?)/;
  ...
  objectPropertyObserver: function (obj, pathToProperty, callback) {
    ...
    function timoutCallback() {
      var propertyValue = KAMPYLE_UTILS.getNestedPropertyValue(
        obj,
        pathToProperty
      );
  ...
  httpRequest: function (method, url, jsonData, onSuccess, onError, config) {
    var counter = 0;
    var numOfTimes =
      (config || {}).numOfTimes || KAMPYLE_CONSTANT.NETWORK.NUM_OF_RETRIES;
    var timeout =
      (config || {}).timeout || KAMPYLE_CONSTANT.NETWORK.REQUEST_TIMEOUT;
    ...
    function makeRequest() {
      ...
      if (method === "POST") {
        KAMPYLE_EVENT_DISPATCHER.trigger(
          KAMPYLE_CONSTANT.ANALYTICS_HTTP_EVENTS_BY_METHOD.BEFORE.POST,
      ...
      xhr.onerror = function (error) {
        var onErrorRequestTotalTime = KAMPYLE_UTILS.getTimeDiff(
          requestStartDate,
          KAMPYLE_CONSTANT.TIME_FORMATS.SECONDS
        );
        var onErrorData = {
        ...
        handleError(
          onErrorData,
          KAMPYLE_CONSTANT.ANALYTICS_HTTP_EVENTS_BY_METHOD.ERROR[method],
          { httpRequestData: onErrorData }
        );
      };
      xhr.ontimeout = function (error) {
        var ontimeoutRequestTotalTime = KAMPYLE_UTILS.getTimeDiff(
          requestStartDate,
          KAMPYLE_CONSTANT.TIME_FORMATS.SECONDS
        );
        var onTimeoutData = {
          ...
          networkConnectionDetails:
            KAMPYLE_UTILS.extractAllObjectNonFunctionsProperties(
        ...
        handleError(
          onTimeoutData,
          KAMPYLE_CONSTANT.ANALYTICS_HTTP_EVENTS_BY_METHOD.TIMEOUT[method],
          { httpRequestData: onTimeoutData }
        );
      };
      xhr.onload = function () {
        var onloadRequestTotalTime = KAMPYLE_UTILS.getTimeDiff(
          requestStartDate,
          KAMPYLE_CONSTANT.TIME_FORMATS.SECONDS
        );
        ...
        if (xhr.status === 200) {
          ...
          KAMPYLE_EVENT_DISPATCHER.trigger(
            KAMPYLE_CONSTANT.ANALYTICS_HTTP_EVENTS_BY_METHOD.AFTER[method],
          ...
          handleError(
            afterRequestData,
            KAMPYLE_CONSTANT.ANALYTICS_HTTP_EVENTS_BY_METHOD.ERROR_STATUS_CODE[
              method
            ],
      ...
      jsonData
        ? xhr.send(JSON.stringify(beforeRequestData.postData))
        : xhr.send();
    }
    makeRequest();
  },
  ...
  getDeviceTypeByUserAgent: function (agent, userAgentData) {
    ...
    if (userAgentData && userAgentData.platform) {
      return userAgentData.mobile
        ? MOBILE
        : userAgentData.platform.toLowerCase() === "android"
        ? TABLET
        : DESKTOP;
    }
          ...
          for (p = 0; p < props.length; p++) {
            ...
            if (typeof q === OBJ_TYPE && q.length > 0) {
              if (q.length == 2) {
                ...
                device[q[0]] = match
                  ? q[3].call(device, match.replace(q[1], q[2]))
                  : undefined;
              }
            } else {
  ...
  safeJsonParse: function (str, msgOnFail) {
    try {
      ...
      KAMPYLE_UTILS.showWarning(
        msgOnFail
          ? msgOnFail
          : "Failed to parse JSON, error message: " + (e || {}).message
      );
  ...
  getScrollPosition: function () {
    ...
    var x = isPageOffsetSupported
      ? window.pageXOffset
      : isCSS1Compat
      ? document.documentElement.scrollLeft
      : document.body.scrollLeft;
    var y = isPageOffsetSupported
      ? window.pageYOffset
      : isCSS1Compat
  ...
  generateUUID: function () {
    ...
    var UUID = "xxxx-xxxx-xxxx-xxxx-xxxx-xxxx-xxxx-xxxx".replace(
      /[x]/g,
      function () {
  ...
  getRetryTimeout: function () {
    ...
    return (
      KAMPYLE_UTILS.getNestedPropertyValue(dataJson, "retryTimout") ||
      KAMPYLE_CONSTANT.NETWORK.RETRY_TIMEOUT
    );
  },
  ...
  sha512: function (str) {
    ...
    if (
      typeof digest.then === "function" &&
      typeof digest.catch === "function"
    ) {
      return digest
        .then(function (hash) {
  ...
  isModalFormType: function (formId) {
    ...
    var validFormTypes =
      Boolean(formData) &&
      Boolean(displayType) &&
      formData.formType !== KAMPYLE_CONSTANT.FORM_TYPES.EMBEDDED &&
      displayType !== KAMPYLE_CONSTANT.DISPLAY_TYPES.POPUP;
    return KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.NEW_MODAL_DIALOGS
    )
      ? validFormTypes &&
          displayType !== KAMPYLE_CONSTANT.DISPLAY_TYPES.ANIMATION
      : validFormTypes;
  },
  isFormInPreviewContext: function (formId) {
    ...
    return (
      formState != null &&
      (formState.triggerType == "Preview" ||
        formState.triggerType == "DraftPreview")
    );
  },
  setSpinnerStyle: function (spinnerContainerElement, spinnerSize, iframe) {
    if (!(spinnerContainerElement || spinnerSize)) return;
    var spinerSizes = {
      small: { widthHeight: 15, border: 3 },
      large: { widthHeight: 40, border: 4 },
    };
    var halfSpinnerSize =
      spinerSizes[spinnerSize]["widthHeight"] / 2 +
      spinerSizes[spinnerSize]["border"];
    var spinner = spinnerContainerElement.querySelector(
      "#" + KAMPYLE_CONSTANT.SPINNER_SETTINGS.ID
    );
  ...
  getOrCreateWrapperContainer: function (wrapperId) {
    return (
      document.getElementById(wrapperId) ||
      Object.assign(document.createElement("span"), { id: wrapperId })
    );
  },
};
window.KAMPYLE_EVENT_DISPATCHER = (function (window, document) {
  ...
  function getTriggerMessage(settings) {
    var messageTemplate =
      KAMPYLE_CONSTANT.LOG_MESSAGES.TRIGGER_MESSAGE_TEMPLATE;
    messageTemplate = messageTemplate
      ...
      .replace(
        "{{OPTIONS}}",
        KAMPYLE_UTILS.getStrigifiedData(settings.options)
      );
    if (settings.errorMessage) {
      messageTemplate = messageTemplate.replace(
        "{{STATUS}}",
        KAMPYLE_CONSTANT.LOG_STATUSES.FAILED
      );
      messageTemplate +=
        KAMPYLE_CONSTANT.LOG_MESSAGES.ERROR_LOG_TEMPLATE.replace(
          "{{ERROR_MESSAGE}}",
      ...
      messageTemplate = messageTemplate.replace(
        "{{STATUS}}",
        KAMPYLE_CONSTANT.LOG_STATUSES.TRIGGERED
      );
    }
  ...
  var trigger = function (eventName, data, context, options) {
    var subscribers = KAMPYLE_UTILS.getNestedPropertyValue(
      eventSubscriptions,
      eventName
    );
    var i, iMax;
  ...
  var triggerError = function (errorData) {
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.NEB_EVENT_DISPATCHER_ERROR,
      errorData
    );
  };
...
window.MDIGITAL.LOCALIZATION = (function () {
  function getLangCodeFromCP(formNode) {
    ...
    var customParamId = KAMPYLE_UTILS.getNestedPropertyValue(
      formNode,
      "formLocalizationSettings.customParam"
    );
    var formData = KAMPYLE_DATA.getFormData(formId) || {};
    var customParam =
      MDIGITAL.CUSTOM_PARAMETERS.getCustomParamById(customParamId, formData) ||
      {};
    return (
      MDIGITAL.CUSTOM_PARAMETERS.fetchCPValue(customParam) ||
      KAMPYLE_UTILS.getNestedPropertyValue(
        formNode,
        "formLocalizationSettings.defaultLanguage"
      )
    );
  }
  function getLangCode(formNode) {
    var shouldUseCustomParam = KAMPYLE_UTILS.isTruthy(
      KAMPYLE_UTILS.getNestedPropertyValue(
        formNode,
        "formLocalizationSettings.useCustomParam"
      )
    );
    return shouldUseCustomParam
      ? MDIGITAL.LOCALIZATION.getLangCodeFromCP(formNode)
      : KAMPYLE_UTILS.getLangCodeFromNavAPI();
  }
  function getLangaugeFallback(formNode, languageCode) {
    var availableLanguages = KAMPYLE_UTILS.getNestedPropertyValue(
      formNode,
      "availableLanguages"
    );
  ...
  function getLocalizedButtonText(formNode) {
    var langCodeMemKey =
      KAMPYLE_CONSTANT.LANG_MEMORY_DATA_KEY_PREFIX + formNode.formId;
    ...
    var textLocalization =
      KAMPYLE_UTILS.getNestedPropertyValue(
        formNode,
        "buttonData.textLocalization"
      ) || {};
  ...
  function isFormLanguageRtl(formNode) {
    ...
    return (
      buttonData[languageCode] && buttonData[languageCode].isRtl === "true"
    );
  }
  function getFormLanguage(formId) {
    ...
    return (
      KAMPYLE_DATA.getMemoryData(
        KAMPYLE_CONSTANT.LANG_MEMORY_DATA_KEY_PREFIX + formId
      ) || detectAndSetFormLanguage(formData)
    );
  }
  return {
...
window.MDIGITAL.EMBEDDED = (function (window, document) {
  ...
  function removeEmbeddedIFrames() {
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.CLEAR_EMBEDDED_ON_UPDATE_PAGE
      )
    ) {
      var allForms = KAMPYLE_DATA.getAllForms();
      for (var i = 0; i < allForms.length; i++) {
  ...
  function createEmbeddedIframe(eventName, data) {
    ...
    if (hostingElement) {
      var hostingElementHeight = parseInt(
        getComputedStyle(hostingElement).height
      );
      var hostingElementPaddingY =
        parseInt(getComputedStyle(hostingElement).paddingTop) +
        parseInt(getComputedStyle(hostingElement).paddingBottom);
    ...
    KAMPYLE_DATA.setTimeoutHandler(
      KAMPYLE_CONSTANT.TIMEOUT_HANDLERS.EMBEDDED_STATUS + "_" + data.formId,
      timeoutHandler
    );
    var formIframe = getOrCreateIframe(data.formId);
    var formData = KAMPYLE_DATA.getFormData(data.formId);
    formIframe.src = KAMPYLE_COMMON.createFormUrl(
      formData.formHtmlUrl,
      data.formId
    );
  ...
  function showForm(eventName, data) {
    if (
      !KAMPYLE_DATA.isEmbeddedDisplayType(data.formId) ||
      KAMPYLE_FUNC.hasApplicationLoadingFailed(data.formId)
    ) {
      return false;
    }
    var currIframe = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.EMBEDDED.IFRAME_ID_PREFIX + data.formId
    );
    if (!!currIframe) {
  ...
  function getHostingElement(formId) {
    ...
    return document.getElementById(
      KAMPYLE_UTILS.getNestedPropertyValue(
        formData,
        "embeddedData.pageElementId"
      )
    );
  }
  function shouldCustomizeScrolling(formId) {
    var isPreventIFrameScrollFlickeringEnabled = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.PREVENT_IFRAME_SCROLLBAR_FLICKERING
    );
    var isFormV2 = KAMPYLE_DATA.isV2Form(formId);
    var isCustomHTML = KAMPYLE_DATA.isCustomHtmlForm(formId);
  ...
  function calculateFormSize(event, data) {
    ...
    KAMPYLE_UTILS.sendMessageToIframe(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.EMBEDDED.IFRAME_ID_PREFIX + data.formId,
      { action: "setFormStyle", data: messageData }
    );
    return messageData;
  }
  function calculateHeights(hostingElement, formData, data) {
    var isStatic =
      formData.isFixedSize === "true" || formData.isFixedSize === true;
    ...
    hostHeight = isNaN(hostHeight)
      ? parseInt(hostingElement.style.height || 0)
      : hostHeight;
    var hostMaxHeight = isNaN(parseInt(hostingElementComputedStyle.maxHeight))
      ? hostHeight || 0
      : parseInt(hostingElementComputedStyle.maxHeight);
    var hostMinHeight = isNaN(parseInt(hostingElementComputedStyle.minHeight))
      ? 0
      : parseInt(hostingElementComputedStyle.minHeight);
  ...
  function calculateWidth(hostingElement, formData) {
    ...
    return Math.min(
      formWidth,
      KAMPYLE_UTILS.getElementSize(hostingElement).width
    );
  }
  function handleFormClosed(eventName, data) {
  ...
  function addHostingElementHeight(elementId, height) {
    var elementsHeightsObj = KAMPYLE_DATA.getMemoryData(
      "hostingElementsHeights"
    );
...
window.MDIGITAL.FORM_STATE = (function (window, document) {
  function init() {
    var eventMapping = {
      MDigital_targetingRuleEvaluated: [updateTargetingState],
  ...
  function isFormFullyLoaded(formId) {
    var isSeparate = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.FORM_TEMPLATE_DATA_SEPERATION
    );
    var formState = KAMPYLE_DATA.getFormState(formId);
    if (!isSeparate) {
...
window.MDIGITAL_ELEMENT_BUILDER = (function () {
  function createScreenCaptureHeader() {
    ...
    linkElem.href =
      KAMPYLE_CONSTANT.PLACEHOLDERS.URL_PREFIX + KAMPYLE_CONSTANT.SRC.OPEN_SANS;
  ...
  function getScreenCaptureDescription(formData) {
    if (formData.screenCaptureSettings) {
      return (
        ...
        formData.screenCaptureSettings.instructionsScreenCapture ||
        KAMPYLE_CONSTANT.TEXTS.SCREEN_CAPTURE.DEFAULT_DESCRIPTION
      );
    }
  ...
  function createScreenCaptureFooter(formattedText, formattedStyles) {
    ...
    KAMPYLE_UTILS.addAttributesOnElement(cancelBtn, {
      class: "sc-widget-footer-cancel top-tip",
      title: formattedText.cancelScreenCaptureTooltip,
    });
    ...
    KAMPYLE_UTILS.setElementStyle(
      ...
      Object.assign(
        { outline: "none" },
        formattedStyles.widgetFooterCancel,
        formattedStyles.widgetFooterBtn
      )
    );
    var cancelBtnText = document.createTextNode(
      formattedText.cancelScreenCaptureLabel
    );
    cancelBtn.appendChild(cancelBtnText);
    var submitBtn = document.createElement("button");
    KAMPYLE_UTILS.addAttributesOnElement(submitBtn, {
      class: "sc-widget-footer-capture top-tip",
      title: formattedText.submitScreenCaptureTooltip,
    });
    submitBtn.addEventListener(
      "click",
      KAMPYLE_SCREEN_CAPTURE.onSubmitScreenCaptureClick
    );
    KAMPYLE_UTILS.setElementStyle(
      submitBtn,
      Object.assign(
        { outline: "none" },
        formattedStyles.widgetFooterCapture,
        formattedStyles.widgetFooterBtn
      )
    );
    ...
    spanElem.appendChild(
      document.createTextNode(formattedText.submitScreenCaptureLabel)
    );
  ...
  function createScreenCaptureIframe(htmlElem, scContainer) {
    ...
    scIframe.height =
      docIframe.getElementById("sc-widget") &&
      docIframe.getElementById("sc-widget").scrollHeight + 20;
  ...
  function createScreenCaptureBody(formData, divFooter, formattedStyles) {
    ...
    KAMPYLE_UTILS.setElementStyle(bodyElem, {
      margin: "0px",
      overflow: "hidden",
    });
    ...
    var headerForWidget =
      formData.screenCaptureSettings &&
      formData.screenCaptureSettings.titleInstructionsScreenCapture
        ? formData.screenCaptureSettings.titleInstructionsScreenCapture
        : KAMPYLE_CONSTANT.TEXTS.SCREEN_CAPTURE.DEFAULT_TITLE;
    var h2Text = document.createTextNode(headerForWidget);
    h2Elem.appendChild(h2Text);
    var pElem = document.createElement("p");
    pElem.className = "scParagraph";
    KAMPYLE_UTILS.setElementStyle(pElem, {
      fontSize: "12px",
      marginTop: "10px",
    });
  ...
  function createScreenCaptureInstructionWidget(formData, scContainer) {
    var formattedText =
      KAMPYLE_SCREEN_CAPTURE.overrideDefaultScreenCaptureTexts(
        formData.screenCaptureSettings
      ) || {};
    var formattedStyles =
      KAMPYLE_SCREEN_CAPTURE.overrideDefaultScreenCaptureStyles(
        formData.screenCaptureSettings
      ) || {};
    var headElem = createScreenCaptureHeader();
    var footerElem = createScreenCaptureFooter(formattedText, formattedStyles);
    var bodyElem = createScreenCaptureBody(
      formData,
      footerElem,
      formattedStyles
    );
  ...
  function createFormContainer() {
    ...
    KAMPYLE_UTILS.setElementStyle(
      divElem2,
      { zIndex: "99999999", webkitOverflowScrolling: "touch" },
      true
    );
  ...
  function createInviteIframeContainer(data) {
    ...
    KAMPYLE_UTILS.setElementStyle(
      divContainer,
      {
        top: "0",
        left: "0",
        width: shouldAdjustToZoom ? "100vw" : "100%",
        height: shouldAdjustToZoom ? "100vh" : "100%",
        position: "fixed",
        visibility: "hidden",
        display: "none",
        backgroundColor: "rgba(22,22,22,0.5)",
        zIndex: "99999999",
        overflow: shouldAdjustToZoom ? "hidden" : "auto",
      },
      true
    );
    var spinnerContainer = createSpinnerElement();
    ...
    KAMPYLE_UTILS.setElementStyle(divElem1, {
      ...
      top: shouldAdjustToZoom ? "50%" : "25%",
      transform: shouldAdjustToZoom ? "translateY(-50%)" : "none",
      maxHeight: shouldAdjustToZoom ? "100vh" : "unset",
      maxWidth: shouldAdjustToZoom ? "100vw" : "unset",
    });
    divContainer.appendChild(divElem1);
    ...
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_PREPEND_SURVEY
      )
    ) {
      document.body.prepend(inviteSpanPlacer);
    } else {
  ...
  function getErrorModal(
    ...
    var errorModalAttributes = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.NEW_MODAL_DIALOGS
    )
      ? { id: noJsonModalId }
      : {
    ...
    KAMPYLE_UTILS.setElementStyle(errorModal, {
      ...
      margin:
        KAMPYLE_DATA.getMemoryData("context") === "preview"
          ? "100px auto 2% auto"
          : "5% auto 2% auto",
    ...
    var closeButton = getErrorModalCloseButton(
      errorMessageClose,
      errorModalLang
    );
  ...
  function getVerticalButton(formData) {
    ...
    var multiplier = 1;
    if (shouldAdjustToZoom) {
      var zoomMetrics = KAMPYLE_UTILS.getZoomedIn();
      KAMPYLE_DATA.setMemoryData("kampylePageZoom", zoomMetrics.zoom);
      multiplier = zoomMetrics.isZoomedIn ? getMultiplier() : 1;
    }
    var width = 125;
    var height = 35;
    var fontSize = 14 * multiplier;
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DYNAMIC_FEEDBACK_BUTTON_SIZE
      )
    ) {
      width = KAMPYLE_VIEW.getVerticalButtonWidth(buttonText);
    }
    var side = ((-width + height) / 2) * multiplier;
    ...
    if (KAMPYLE_DATA.isFeatureEnabled(KAMPYLE_CONSTANT.FEATURE_FLAGS.WCAG)) {
      ...
      KAMPYLE_UTILS.setElementStyle(buttonElement, { outline: "none" }, false);
    }
    buttonElement.style[formData.buttonData.position] = side + "px";
    var divElement = document.createElement("div");
    divElement.setAttribute("class", "kampyle_button");
    var styleForRadius =
      formData.buttonData.position === "left" ? "3px 3px 0 0" : "0 0 3px 3px";
    KAMPYLE_UTILS.setElementStyle(
      divElement,
      {
        width: width + "px",
    ...
    KAMPYLE_UTILS.setElementStyle(divElement2, { fontWeight: "normal" }, false);
    ...
    if (
      ...
      addHoverStyleToFeedbackButton(
        buttonElement,
        {
          ...
          textElementStyle: { color: formData.buttonData.textColor },
        },
        {
          bgElementStyle: {
            backgroundColor:
              formData.buttonData.backgroundHoverColor ||
  ...
  function addHoverStyleToFeedbackButton(
    parentElement,
    defaultStyles,
    onHoverStyles
  ) {
    if (
      ...
      defaultStyles &&
      onHoverStyles &&
      typeof parentElement.addEventListener === "function"
    ) {
      parentElement.addEventListener("mouseenter", function () {
        ...
        KAMPYLE_UTILS.setElementStyle(
          div2_text,
          onHoverStyles.textElementStyle
        );
      });
  ...
  function getPositionEnum(formData, btnSize) {
    return {
      topRight: {
        ...
        div1Style: {
          top: 0,
          right: 0,
          borderTop: btnSize + " solid " + formData.buttonData.backgroundColor,
          borderLeft: btnSize + " solid transparent",
        },
        div2Style: {
      ...
      topLeft: {
        ...
        div1Style: {
          ...
          left: 0,
          borderTop: btnSize + " solid " + formData.buttonData.backgroundColor,
          borderRight: btnSize + " solid transparent",
        },
        div2Style: {
          "-ms-transform": "rotate(-45deg)",
          "-webkit-transform": "rotate(-45deg)",
      ...
      bottomRight: {
        ...
        div1Style: {
          ...
          right: 0,
          borderBottom:
            btnSize + " solid " + formData.buttonData.backgroundColor,
          borderLeft: btnSize + " solid transparent",
        },
        div2Style: {
          "-ms-transform": "rotate(-45deg)",
  ...
  function getTriangularButton(formData) {
    ...
    var width = 81;
    var fontSize = 14;
    var lineHeight, height, buttonPadding, txtWidth;
    var buttonText = KAMPYLE_VIEW.getButtonText(formData);
    ...
    var multiplier =
      shouldAdjustToZoom && KAMPYLE_UTILS.getZoomedIn().isZoomedIn
        ? getMultiplier()
        : 1;
    var buttonStyles = {};
    if (isDynamicButtonSizeEnabled) {
      var buttonTextSize =
        KAMPYLE_VIEW.getTriangleTextSizeCalculation(formData);
      height = buttonTextSize.height * multiplier;
      txtWidth = buttonTextSize.width * multiplier;
      buttonStyles = KAMPYLE_VIEW.getDynamicButtonStyles(position);
      width = buttonTextSize.height * 2;
      buttonPadding = buttonTextSize.padding * multiplier;
      lineHeight = 1.5;
    } else {
      lineHeight =
    ...
    var btnSize = width * multiplier + "px";
    var STYLE_BY_POSITION = getPositionEnum(formData, btnSize);
    var buttonElement = document.createElement("button");
    ...
    if (isDynamicButtonSizeEnabled) {
      buttonTxtStyle[buttonStyles.paddingDirection] = buttonPadding + "px";
      Object.assign(buttonTxtStyle, {
        width: txtWidth + "px",
        height: "auto",
        margin: "0 auto",
      });
    } else {
      Object.assign(buttonTxtStyle, STYLE_BY_POSITION[position].div2Style, {
        height: btnSize,
        width: btnSize,
        zIndex: "1000",
      });
    ...
    KAMPYLE_UTILS.setElementStyle(
      buttonTxt,
      Object.assign({ fontSize: fontSize * multiplier + "px" }, buttonTxtStyle)
    );
    if (isDynamicButtonSizeEnabled) {
      KAMPYLE_UTILS.setElementStyle(
        buttonElement,
        {
          clipPath: buttonStyles.buttonPath,
      ...
      var shapes = createDynamicButtonShapes(buttonStyles);
      buttonElement.appendChild(shapes.leftShape);
      buttonElement.appendChild(shapes.rightShape);
    }
    var textNode = document.createTextNode(buttonText);
    buttonTxt.appendChild(textNode);
    if (!isDynamicButtonSizeEnabled) {
      var triangleBgStyle = STYLE_BY_POSITION[position].div1Style;
      var triangleBgElement = document.createElement("div");
      triangleBgElement.setAttribute("class", "kampyle_triangle");
      KAMPYLE_UTILS.setElementStyle(
        triangleBgElement,
        Object.assign(
      ...
      buttonElement.appendChild(triangleBgElement);
    }
  ...
  function getCustomButton(formData) {
    ...
    KAMPYLE_UTILS.setElementStyle(
      buttonElement,
      Object.assign({ padding: 0 }, styleForBtn)
    );
    if (KAMPYLE_DATA.isFeatureEnabled(KAMPYLE_CONSTANT.FEATURE_FLAGS.WCAG)) {
      buttonElement.addEventListener("focusin", function (event) {
        KAMPYLE_UTILS.setElementStyle(
  ...
  function getErrorModalCloseButton(text, langCode) {
    ...
    KAMPYLE_UTILS.addAttributesOnElement(closeButton, {
      "aria-hidden": KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.NEW_MODAL_DIALOGS
      )
        ? false
        : true,
      tabIndex: KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.NEW_MODAL_DIALOGS
      )
        ? 0
        : -1,
    ...
    if (KAMPYLE_UTILS.isDesktop()) {
      KAMPYLE_UTILS.addHoverEffectOnElement(
        closeButton,
        { opacity: 0.7 },
        { opacity: 1 }
      );
  ...
  function createDynamicButtonShapes(buttonStyles) {
    var leftShape = document.createElement("div");
    leftShape.className = "kampyle_shape_left";
    var rightShape = document.createElement("div");
    rightShape.className = "kampyle_shape_right";
    KAMPYLE_UTILS.setElementStyle(
      leftShape,
      {
        clipPath: buttonStyles.textPath.left,
    ...
    return { leftShape: leftShape, rightShape: rightShape };
  }
  return {
...
window.MDIGITAL.PROPERTY_MESSAGES = (function () {
  ...
  function getLanguageCodeByCustomParam() {
    var propertyMessageData = getPropertyMessageTranslation();
    var customParamObject = KAMPYLE_UTILS.getNestedPropertyValue(
      propertyMessageData,
      "localizationSettings.customParam"
    );
    return (
  ...
  function getPropertyMessageLanguageCode() {
    var propertyMessageData = getPropertyMessageTranslation();
    var shouldUseCustomParam = KAMPYLE_UTILS.getNestedPropertyValue(
      propertyMessageData,
      "localizationSettings.useCustomParam"
    );
  ...
  function getMessageByTypeAndKey(type, key) {
    var propertyMessageData =
      MDIGITAL.PROPERTY_MESSAGES.getPropertyMessageTranslation().messagesData ||
      {};
    var propertyMessageLangCode = KAMPYLE_UTILS.safeToLower(
      MDIGITAL.PROPERTY_MESSAGES.getPropertyMessageLanguageCode()
    );
    var messageObjectByLanguage =
      propertyMessageData[type] &&
      propertyMessageData[type][propertyMessageLangCode];
    if (messageObjectByLanguage && messageObjectByLanguage[key] != undefined) {
      return messageObjectByLanguage[key];
    } else {
      var defaultLanguageCode = KAMPYLE_UTILS.safeToLower(
        MDIGITAL.PROPERTY_MESSAGES.getDefaultLanguageFromPropertyData()
      );
      var messageObjectByDefaultLanguage =
        propertyMessageData[type] &&
        propertyMessageData[type][defaultLanguageCode];
      if (
        messageObjectByDefaultLanguage &&
        messageObjectByDefaultLanguage[key] != undefined
      ) {
...
window.MDIGITAL = window.MDIGITAL || {};
window.MDIGITAL.ANALYTICS = (function (window, document) {
  var rateLimitCounterMap = {};
  var eventMdDataKeyReplacementMap = {
    additional_data: "additionalData",
    additionalInfo: function (val) {
      return typeof val === "string" ? val : "additionalData";
    },
    error_logs: "errorLogs",
    source_function: "sourceFunction",
    formId: "destinationForm",
  };
  var eventDataKeyReplacementMap = {
    errorMessage: "message",
    uuid: "feedbackUuid",
  };
  var COMMON_FIELDS;
  var allDigitalEvents = {};
  var initialEvents = {
    neb_eventDispatcherError: {
      eventId: "nebula_eventDispatcherError",
      eventDataKeys: ["errorMessage", "errorStack"],
      eventMdDataKeys: ["additionalInfo"],
    },
    MDigital_warning: {
      eventId: "MDigital_warning",
      eventDataKeys: ["message"],
    },
    MDigital_couldNotFetchData: {
      eventId: "MDigital_couldNotFetchData",
      eventMdDataKeys: ["errorData"],
    },
    ...
    neb_afterHttpGetRequestOnError: {
      ...
      eventMdDataKeys: ["httpRequestData"],
    },
    ...
    MDigital_directGenericInjection: {
      eventId: "mdigital_directGenericInjection",
    },
  };
  var nonConfigurationDependentEvents = {
    neb_pageView: {
      eventId: "nebula_page_view",
      disableProvision:
        KAMPYLE_CONSTANT.PROVISIONS.DISABLE_ANANLYTICS_NEW_PAGE_EVENTS,
    },
    ...
    neb_submitFeedbackSuccess: { eventId: "nebula_submitFeedbackSuccess" },
    ...
    MDigital_pageTargetingPhaseEnded: {
      eventId: KAMPYLE_CONSTANT.EVENTS.MDIGITAL_PAGE_TARGETING_PHASE_ENDED,
      provision: KAMPYLE_CONSTANT.PROVISIONS.TARGETING_EVENTS_REPORTING,
      formatDataFunc: formatPageTargetingData,
      eventMdDataKeys: ["additionalData"],
      rateLimit: 1,
    },
    MDigital_behavioralTargetingPhaseEnded: {
      eventId:
        KAMPYLE_CONSTANT.EVENTS.MDIGITAL_BEHAVIORAL_TARGETING_PHASE_ENDED,
      provision: KAMPYLE_CONSTANT.PROVISIONS.TARGETING_EVENTS_REPORTING,
      formatDataFunc: formatBehavioralTargetingData,
      eventMdDataKeys: ["additionalData"],
      rateLimit: 1,
    },
    MDigital_quarantinedSurveyPhaseEnded: {
      eventId: KAMPYLE_CONSTANT.EVENTS.MDIGITAL_QUARANTINED_SURVEY_PHASE_ENDED,
      provision: KAMPYLE_CONSTANT.PROVISIONS.TARGETING_EVENTS_REPORTING,
      formatDataFunc: formatQuarantineTargetingData,
      eventMdDataKeys: ["additionalData"],
      rateLimit: 1,
    },
  };
  var configurationDependentEvents = {
    neb_sessionStart: {
      ...
      provision: KAMPYLE_CONSTANT.PROVISIONS.ENABLE_ANALYTICS_SESSION_EVENTS,
    },
    neb_load_form_preload_proximity: {
      eventId: "nebula_load_form_preload_proximity",
      provision: KAMPYLE_CONSTANT.PROVISIONS.analyticsNewEvents,
    },
    neb_button_displayed: {
      eventId: "nebula_button_displayed",
      provision: KAMPYLE_CONSTANT.PROVISIONS.analyticsNewEvents,
      eventMdDataKeys: ["position"],
    },
    ...
    MDigital_createSession: {
      eventId: KAMPYLE_CONSTANT.EVENTS.CREATE_SESSION,
      provision: KAMPYLE_CONSTANT.PROVISIONS.CREATE_SESSION,
      eventMdDataKeys: ["additionalData"],
    },
  };
  if (
    !KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DYNAMIC_ANALYTICS_EVENTS
    )
  ) {
    Object.assign(
      allDigitalEvents,
      initialEvents,
      nonConfigurationDependentEvents
    );
  }
  function initRateLimiting(specificEvents) {
    var events = specificEvents || allDigitalEvents;
    Object.keys(events).forEach(function (ev) {
      var eventObj = events[ev];
  ...
  function getDataToSend(dataFormatted) {
    var feedbackUUID = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.BACKEND_GENERATED_UUID
    )
      ? dataFormatted.feedbackUUID
      : dataFormatted.feedbackCorrelationUUID;
    if (!dataFormatted.accountId) {
      KAMPYLE_EVENT_DISPATCHER.appendDefaultEventParams(dataFormatted);
    }
    var dataToSend = {
      accountId: dataFormatted.accountId,
      propertyId: dataFormatted.websiteId,
      ...
      mdData: dataFormatted.mdData || {},
      userId: dataFormatted.userId || null,
      sessionId: dataFormatted.sessionId || null,
      cookieSize: dataFormatted.cookieSize || null,
      sdkVersion: window.MDIGITAL.version || "0.0.0.0",
      historyLength: window.history.length,
      isUserIdentified: !!KAMPYLE_DATA.getMemoryData("identifierValue"),
      feedbackCorrelationUuid: KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.BACKEND_GENERATED_UUID
      )
        ? dataFormatted.feedbackCorrelationUUID
        : null,
      clientEventUuid: KAMPYLE_UTILS.kampyleCreateUUID(),
    };
    if (dataFormatted.formId) {
      dataToSend.webFormLang =
        MDIGITAL.LOCALIZATION.getFormLanguage(dataFormatted.formId) || null;
    }
    addEventDataKeys(dataFormatted, dataToSend);
    if (dataFormatted.mdTargetingData) {
      dataToSend.mdData.mdTargetingData = dataFormatted.mdTargetingData;
    }
    return dataToSend;
  }
  function addEventDataKeys(dataFormatted, dataToSend) {
    var digitalEvent = allDigitalEvents[dataFormatted.eventName];
    var eventDataKeys = digitalEvent ? digitalEvent.eventDataKeys : null;
    if (eventDataKeys) {
      for (var i = 0; i < eventDataKeys.length; i++) {
        var eventDataKeyName =
          eventDataKeyReplacementMap[eventDataKeys[i]] || eventDataKeys[i];
        dataToSend[eventDataKeyName] = dataFormatted[eventDataKeys[i]] || null;
      }
    }
  }
  function sendDigitalAnalytics(dataFormatted) {
    if (!dataFormatted || !window.MDIGITAL.md_tracker) {
      return false;
    }
    var dataToSend = getDataToSend(dataFormatted);
    MDIGITAL.ANALYTICS.makeAnalyticsCalls(dataFormatted.eventId, dataToSend);
    return true;
  }
  function formatData(eventName, oData) {
    ...
    var result = Object.assign({}, oData);
    for (var prop in COMMON_FIELDS) {
      result[prop] = COMMON_FIELDS[prop];
    }
    for (var eventProp in allDigitalEvents[eventName]) {
      result[eventProp] = allDigitalEvents[eventName][eventProp];
    }
    var digitalData = KAMPYLE_UTILS.getAllKampyleData();
    result.mdData = {
      submittedDate: digitalData.SUBMITTED_DATE,
      declinedDate: digitalData.DECLINED_DATE,
      lastInvitationView: digitalData.LAST_INVITATION_VIEW,
      mdUserPercentile: digitalData.kampyleUserPercentile,
      mdInvitePresented: digitalData.kampyleInvitePresented,
    };
    appendMemoryDataByEventName(eventName, result);
    appendMdDataByEventName(eventName, result.mdData, result);
    result.sessionId = digitalData.kampyleUserSession;
    result.cookieSize = KAMPYLE_UTILS.getByteSize(document.cookie);
    result.pageURL = location.href;
    result.eventName = eventName;
    return result;
  }
  var triggerDigitalAnalyticsEvent = function (eventName, oData) {
    var eventObj = allDigitalEvents[eventName];
    if (
      !(eventObj && isEventProvisioned(eventObj)) ||
      isRateLimitExceeded(eventObj)
    ) {
      return false;
    }
    var shouldFormatEventData =
      eventObj.formatDataFunc && typeof eventObj.formatDataFunc === "function";
    if (shouldFormatEventData) {
      oData = eventObj.formatDataFunc.call(this, oData);
    }
    var oDataFormatted = formatData(eventName, oData);
    MDIGITAL.ANALYTICS.sendDigitalAnalytics(oDataFormatted);
  };
  function isEventProvisioned(eventObj) {
    var isDisabled =
      eventObj.disableProvision &&
      KAMPYLE_FUNC.checkProvision(eventObj.disableProvision);
    return (
      !isDisabled &&
      (!eventObj.provision || !!KAMPYLE_FUNC.checkProvision(eventObj.provision))
    );
  }
  ...
  function isRateLimitExceeded(eventObj) {
    ...
    return (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ANALYTICS_EVNETS_LIMIT
      ) &&
      typeof counter === "number" &&
      counter >= eventObj.rateLimit
    );
  }
  ...
  function triggerDigitalAnalyticsEventDispatcherError(eventData) {
    if (!eventData || !window.MDIGITAL.md_tracker) {
      return false;
    }
  ...
  function makeAnalyticsCalls(eventId, dataToSend) {
    try {
      window.MDIGITAL.md_tracker.trackEvent(eventId, dataToSend);
      getEventCallsCounter(eventId) !== null &&
        increaseEventCallsCounter(eventId);
    } catch (e) {
      console.warn(e);
    }
  }
  function initTracker() {
    try {
      ...
      try {
        if (!window.MDIGITAL.md_tracker && window.MDIGITAL.TRACKER) {
          window.MDIGITAL.TRACKER.loadBaseTracker();
        }
        var tracker_config = {
          api_host:
            KAMPYLE_CONSTANT.PLACEHOLDERS.DEFAULT_ANALYTICS_SUBMIT_EVENTS_URL,
          topic: KAMPYLE_CONSTANT.PLACEHOLDERS.DEFAULT_MEC_INTEGRATION_ID,
        };
        window.MDIGITAL.md_tracker.init(tracker_config);
        if (window.MDIGITAL.TRACKER) {
          window.MDIGITAL.TRACKER.loadFullTracker();
        }
        if (
          KAMPYLE_FUNC.checkProvision(
            KAMPYLE_CONSTANT.PROVISIONS.ANALYTICS_EVNETS_LIMIT
          ...
          initRateLimiting(initialEvents);
          KAMPYLE_EVENT_DISPATCHER.subscribeOnce(
            KAMPYLE_CONSTANT.EVENTS.MDIGITAL_SDK_PAGE_VIEW,
            initRateLimiting
          );
        }
      } catch (e) {
      ...
      if (
        ...
        var configEventsList = JSON.parse(
          KAMPYLE_CONSTANT.PLACEHOLDERS.DIGITAL_ANALYTICS_STATIC_EVENTS_LIST ||
            "[]"
        );
        initialEvents = filterEventsByConfig(initialEvents, configEventsList);
        Object.assign(allDigitalEvents, initialEvents);
      } else {
        KAMPYLE_UTILS.initEventSubscriptions(
          nonConfigurationDependentEvents,
          triggerDigitalAnalyticsEvent
        );
      }
      KAMPYLE_UTILS.initEventSubscriptions(
        initialEvents,
  ...
  function filterEventsByConfig(allEventsList, configEventsList) {
    var filteredEventsByConfig = {};
    for (var eventKey in allEventsList) {
      if (
        configEventsList &&
        configEventsList.includes(allEventsList[eventKey].eventId)
      ) {
        filteredEventsByConfig[eventKey] = allEventsList[eventKey];
      }
    }
    return filteredEventsByConfig;
  }
  function updateTrackerAnalyticsUrl() {
    var onsiteData = MDIGITAL.CONFIGURATION.getOnsiteConfiguration();
    if (onsiteData && onsiteData.analyticsSubmitEventsUrl) {
      var tracker_config = {
        api_host: onsiteData.analyticsSubmitEventsUrl,
        topic: onsiteData.mecIntegrationId,
      };
      window.MDIGITAL.md_tracker.set_config(tracker_config);
    } else {
      console.error(new Error("No Digital analytics url was found!"));
    }
  }
  function registerConfigurationDependentEvents() {
    var dynamicEvents = {};
    if (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DYNAMIC_ANALYTICS_EVENTS
      ...
      var onsiteData = MDIGITAL.CONFIGURATION.getOnsiteConfiguration();
      var configEventsList = onsiteData.digitalAnalyticsDynamicEventsList;
      Object.assign(
        dynamicEvents,
        configurationDependentEvents,
        nonConfigurationDependentEvents
      );
      dynamicEvents = filterEventsByConfig(dynamicEvents, configEventsList);
    } else {
      Object.assign(dynamicEvents, configurationDependentEvents);
    }
    Object.assign(allDigitalEvents, dynamicEvents);
    initRateLimiting(dynamicEvents);
    KAMPYLE_UTILS.initEventSubscriptions(
      dynamicEvents,
      triggerDigitalAnalyticsEvent
    );
  }
  function appendMemoryDataByEventName(eventName, eventJsonData) {
    eventJsonData = eventJsonData || {};
    var memoryDataKey = allDigitalEvents[eventName].memoryDataKey;
    var memoryData = KAMPYLE_DATA.getMemoryData(memoryDataKey);
    if (memoryData) {
      eventJsonData.mdData = eventJsonData.mdData || {};
      eventJsonData.mdData[memoryDataKey] = memoryData;
    }
    return eventJsonData;
  }
  function appendMdDataByEventName(eventName, mdDataObj, eventJsonData) {
    eventJsonData = eventJsonData || {};
    var eventMdDataKeys = allDigitalEvents[eventName].eventMdDataKeys;
    if (eventMdDataKeys) {
      for (var i = 0; i < eventMdDataKeys.length; i++) {
        var eventMdDataKey = eventMdDataKeys[i];
        var eventData = eventJsonData[eventMdDataKey];
        var eventMdDataKeyName =
          eventMdDataKeyReplacementMap[eventMdDataKey] || eventMdDataKey;
        if (typeof eventMdDataKeyName === "function") {
          eventMdDataKeyName = eventMdDataKeyName(eventData);
        }
  ...
  function getFinalConditionResult(accumulator, current) {
    return accumulator && current.isPassed;
  }
  function getFormTargetingData(form, targetingPhase) {
    form = form || {};
    var formState = KAMPYLE_DATA.getFormState(form.formId);
    if (!formState || !formState.targeting) {
      MDIGITAL.LOGGER.error(
        "Digital Analytics - getFormTargetingData: failed to build " +
          targetingPhase +
          " phase data for form id: " +
          form.id
      );
      return false;
    }
    var conditions = formState.targeting[targetingPhase];
    var dataToSend = {
      formId: form.formId,
      ...
      evaluationResult:
        conditions === null
          ? false
          : (conditions || []).reduce(getFinalConditionResult, true),
      conditions: conditions,
    };
    return dataToSend;
  }
  function formatPageTargetingData() {
    var dataToSend = [];
    var allForms = KAMPYLE_DATA.getAllForms() || [];
    allForms.forEach(function (form) {
      dataToSend.push(
        getFormTargetingData(
          form,
          KAMPYLE_CONSTANT.TARGETING_PHASES.PAGE_TARGETING
        )
      );
    });
    return { mdTargetingData: dataToSend };
  }
  function formatBehavioralTargetingData(formData) {
    return {
      mdTargetingData: getFormTargetingData(
        formData,
        KAMPYLE_CONSTANT.TARGETING_PHASES.BEHAVIORAL_TARGETING
      ),
    };
  }
  function formatQuarantineTargetingData(formData) {
    var form =
      getFormTargetingData(
        formData,
        KAMPYLE_CONSTANT.TARGETING_PHASES.QUARANTINE_TARGETING
      ) || {};
    form.userIdentifierValue = KAMPYLE_DATA.getMemoryData("identifierValue");
    return { mdTargetingData: form };
  }
  return {
    getDataToSend: getDataToSend,
    sendDigitalAnalytics: sendDigitalAnalytics,
    formatData: formatData,
    appendDataByEventName: appendMdDataByEventName,
    appendMemoryDataByEventName: appendMemoryDataByEventName,
    addEventDataKeys: addEventDataKeys,
    makeAnalyticsCalls: makeAnalyticsCalls,
    ...
    updateTrackerAnalyticsUrl: updateTrackerAnalyticsUrl,
    registerConfigurationDependentEvents: registerConfigurationDependentEvents,
    triggerDigitalAnalyticsEvent: triggerDigitalAnalyticsEvent,
    triggerDigitalAnalyticsEventDispatcherError:
      triggerDigitalAnalyticsEventDispatcherError,
    formatPageTargetingData: formatPageTargetingData,
    formatBehavioralTargetingData: formatBehavioralTargetingData,
    formatQuarantineTargetingData: formatQuarantineTargetingData,
    getFormTargetingData: getFormTargetingData,
  };
})(window, document);
window.KAMPYLE_COMMON = (function (window, document) {
  ...
  function createFormUrl(url, formId) {
    if (
      KAMPYLE_DATA.isV2Form(formId) &&
      !!KAMPYLE_DATA.getData(
        KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.LIVE_FORM_V2_DEBUG_MODE
      )
    ) {
    ...
    url +=
      "&displayType=" +
      KAMPYLE_DATA.getDisplayType(KAMPYLE_DATA.getFormData(formId));
    url += "&isSeparateFormTemplateFromData=true";
    url +=
      "&domainsListRelativePath=" +
      encodeURIComponent(getDomainsListRelativePath(url));
    return url;
  }
  ...
  function getDomainsListRelativePath(url) {
    var region = KAMPYLE_COMMON.getUrlPrefix().match(
      /((?:[\/](?!\/))[a-zA-Z0-9_-]+[\/a-zA-Z0-9_-]*)$/
    );
    var configurationPath = region
      ? region[0] + "resources/config"
      : "/resources/config";
    if (KAMPYLE_DATA.getMemoryData("context") === "preview") {
      return configurationPath;
    }
    var genericParsedUrl = KAMPYLE_UTILS.getParsedUrl(
      KAMPYLE_EMBED.getGenericLocation()
    );
    ...
    if (genericParsedUrl.hostname !== formParsedUrl.hostname) {
      ...
      if (genericFilePathname && formPathname) {
        ...
        return (
          getRelativePathByFoldersNum(formPathnameParts.length) +
          genericPathnameParts.join("/")
        );
      }
      return "";
    }
  ...
  function getUrlPrefix() {
    var isOnPrem = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.ON_PREM_HYBRID
    );
    return isOnPrem
      ? MDIGITAL.ON_PREM.getPackageRoot() + "/"
  ...
  function getSpinnerTemplate() {
    var spinnerHtml =
      ...
      '" class="neb-loading-spinner">';
    spinnerHtml +=
      '<div id="' + KAMPYLE_CONSTANT.SPINNER_SETTINGS.ID + '"> </div>';
    spinnerHtml += "</div>";
    return spinnerHtml;
  }
  return {
...
window.KAMPYLE_SCREEN_CAPTURE = (function (window, document) {
  var ON_LOAD_ONPREM_CALLBACK_NAME = "onUsersnapLoad";
  ...
  var screenCaptureButtonsOutlineStyle =
    ".scTitle:focus, .scParagraph:focus, .sc-widget-footer-cancel:focus, .sc-widget-footer-capture:focus, p:focus { border: 2px solid #8e8e93 !important; box-shadow: 0 0 7px #8e8e93 !important;}";
  ...
  var screenCaptureWidgetTemplate = [
    ...
    '<link rel="stylesheet" type="text/css" href="' +
      KAMPYLE_CONSTANT.PLACEHOLDERS.URL_PREFIX +
      KAMPYLE_CONSTANT.SRC.OPEN_SANS +
      '" />',
    ...
    '<button onclick="parent.KAMPYLE_SCREEN_CAPTURE.onCancelScreenCaptureClick();" class="sc-widget-footer-cancel top-tip" ' +
      'data-tips="{{cancelScreenCaptureTooltip}}" style="{widgetFooterBtn} {widgetFooterCancel}">',
    "{{cancelScreenCaptureLabel}}",
    "</button>",
    '<button onclick="parent.KAMPYLE_SCREEN_CAPTURE.onSubmitScreenCaptureClick();" class="sc-widget-footer-capture top-tip" ' +
      'data-tips="{{submitScreenCaptureTooltip}}" style="{widgetFooterBtn} {widgetFooterCapture}">',
    spinnerTemplate,
  ...
  var UserSnapModel = function () {
    ...
    this.determineWidgetVersion = function () {
      this._usersnapOnpremWidgetLoaderPath =
        KAMPYLE_UTILS.getNestedPropertyValue(
          MDIGITAL.CONFIGURATION.getOnsiteConfiguration(),
          "usersnapOnpremWidgetLoaderPath"
        );
    };
    this.close = function () {
      window.UserSnap.api.close();
    };
    this.getUrl = function () {
      return (
        KAMPYLE_CONSTANT.PLACEHOLDERS.USERSNAP_ONPREM_HOST_PREFIX +
        this._usersnapOnpremWidgetLoaderPath
      );
    };
    this.initConfiguration = function () {
      window.usersnapConfig = {
        cdnUrl:
          KAMPYLE_CONSTANT.PLACEHOLDERS.USERSNAP_ONPREM_HOST_PREFIX +
          this._usersnapOnpremWidgetLoaderPath.slice(
            0,
            this._usersnapOnpremWidgetLoaderPath.lastIndexOf("/")
          ),
        onLoadCallbackName: ON_LOAD_ONPREM_CALLBACK_NAME,
      };
      window[ON_LOAD_ONPREM_CALLBACK_NAME] = getConfiguration();
    };
    this.getContainer = function () {
      return document.getElementsByName("us-entrypoint-widgetApp")[0];
    };
    this.initWidget = function () {
      window.UserSnap.api.open();
    };
  };
  var hideSpinner = function () {
    var iframeDocument = document.getElementById("nebScIframe").contentDocument;
    var spinner = iframeDocument.getElementById(
      KAMPYLE_CONSTANT.SPINNER_SETTINGS.CONTAINER_ID
    );
    if (!!spinner) {
  ...
  function afterSend(id) {
    ...
    if (formData && !formData.canceledClicked) {
      ...
      KAMPYLE_UTILS.sendMessageToIframe(
        KAMPYLE_DATA.getFormIframeId(formData.formId),
        { action: "takenScreenCapture", data: eventExtraData }
      );
    }
    userSnapInstance.close();
  }
  function getConfiguration() {
    return function (api) {
      ...
      var initParams = {
        serviceUrl: KAMPYLE_CONSTANT.PLACEHOLDERS.USERSNAP_ONPREM_HOST_PREFIX,
        mode: "headless",
        button: null,
        tools: {
          keys: ["highlight"],
        ...
        iframeTitle: KAMPYLE_CONSTANT.IFRAME_SETTINGS.USERSNAP_V4_IFRAME_TITLE,
      };
      initParams.tools.keys[0] = getSelectedTool();
      initParams.tools.colors[0] =
        KAMPYLE_CONSTANT.COLORS.USERSNAP_ONPREM_SELECTION_COLOR;
      initParams.apiKey = KAMPYLE_UTILS.getNestedPropertyValue(
        MDIGITAL.CONFIGURATION.getOnsiteConfiguration(),
        "usersnapEnvId"
      );
      api.init(initParams);
  ...
  function beforeOpen() {
    ...
    KAMPYLE_UTILS.setElementStyle(
      container,
      { display: "block", zIndex: "2147483646" },
      true
    );
    var iframeArr =
      container &&
    ...
    var overFlowType = KAMPYLE_UTILS.getNestedPropertyValue(
      document,
      "body.style.overflow"
    );
  ...
  function handleScrollIfNeeded(formData, scrollType) {
    ...
    if (
      displayType === KAMPYLE_CONSTANT.DISPLAY_TYPES.LIGHTBOX &&
      selectedTool === KAMPYLE_CONSTANT.SCREEN_CAPTURE.DOM_SELECTION_VALUE
    ) {
  ...
  function initScreenCaptureWidget(eventName, data) {
    ...
    KAMPYLE_DATA.setMemoryData(
      KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.FEEDBACK_CORRELATION_UUID,
      formData.feedbackCorrelationUUID
    );
  ...
  function handleCaptureCancellation() {
    ...
    KAMPYLE_EVENT_DISPATCHER.trigger("MDigital_Capture_Cancelled", {
      formId: formId,
      feedbackCorrelationUUID: KAMPYLE_DATA.getMemoryData(
        KAMPYLE_CONSTANT.MEMORY_DATA_FIELDS.FEEDBACK_CORRELATION_UUID
      ),
    });
    var formData = KAMPYLE_DATA.getFormData(formId);
  ...
  function shouldProcessScreenCaptureAsComponent(data) {
    var isScreenCaptureEnabled = KAMPYLE_UTILS.getNestedPropertyValue(
      data,
      "screenCaptureSettings.screenCaptureAsComponentEnabled"
    );
    return (
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.USER_SNAP_AS_COMPONENT
  ...
  function getSelectedTool() {
    ...
    return config.screenCaptureSelectionTool ===
      KAMPYLE_CONSTANT.SCREEN_CAPTURE.DOM_SELECTION
      ? KAMPYLE_CONSTANT.SCREEN_CAPTURE.DOM_SELECTION_VALUE
      : KAMPYLE_CONSTANT.SCREEN_CAPTURE.AREA_SELECTION_VALUE;
  }
  ...
  function overrideDefaultScreenCaptureStyles(customSettings) {
    ...
    if (!!customSettings) {
      res.widget.background = !!customSettings.backgroundColor
        ? customSettings.backgroundColor
        : res.widget.background;
      res.widgetFooterCapture.background = !!customSettings.submitButtoncolor
        ? customSettings.submitButtoncolor
        : res.widgetFooterCapture.background;
      res.widgetFooterCancel.background = !!customSettings.closeButtoncolor
        ? customSettings.closeButtoncolor
        : res.widgetFooterCancel.background;
      res.widgetFooterCancel.color = !!customSettings.closeButtonTextColor
        ? customSettings.closeButtonTextColor
        : res.widgetFooterBtn.color;
      res.widgetFooterCapture.color = !!customSettings.submitButtonTextColor
        ? customSettings.submitButtonTextColor
        : res.widgetFooterBtn.color;
    }
    return res;
  }
  ...
  function showScreenCaptureSpinner() {
    var iframeDocument = document.getElementById("nebScIframe").contentDocument;
    var spinnerContainer = iframeDocument.getElementById(
      KAMPYLE_CONSTANT.SPINNER_SETTINGS.CONTAINER_ID
    );
    if (!!spinnerContainer) {
      KAMPYLE_UTILS.setSpinnerStyle(
        spinnerContainer,
        KAMPYLE_CONSTANT.SPINNER_SETTINGS.SIZE.SMALL,
        iframeDocument
      );
      var text = iframeDocument.getElementById("widgetFooterCaptureText");
      if (!!text) {
  ...
  function formatScreenCaptureWidget(formData) {
    ...
    var formattedText = overrideDefaultScreenCaptureTexts(
      formData.screenCaptureSettings
    );
    ...
    if (!formattedText || !formattedText.cancelScreenCaptureTooltip) {
      widgetHtml = widgetHtml.replace(
        'data-tips="{{cancelScreenCaptureTooltip}}"',
        ""
      );
    }
    for (var prop in formattedText) {
      if (SCREEN_CAPTURE_DEFAULT_TEXTS.hasOwnProperty(prop)) {
        widgetHtml = widgetHtml.replace(
          "{{" + prop + "}}",
          SCREEN_CAPTURE_DEFAULT_TEXTS[prop]
        );
      }
    }
    var formattedStyles = overrideDefaultScreenCaptureStyles(
      formData.screenCaptureSettings
    );
    for (var element in formattedStyles) {
      widgetHtml = KAMPYLE_UTILS.replaceAll(
        widgetHtml,
        "{" + element + "}",
        KAMPYLE_UTILS.styleObjToInline(defaultStyles[element])
      );
    }
    widgetHtml = widgetHtml
      .replace("{tooltipStyles}", screenCaptureTooltipStyles)
      .replace("{buttonsOutlineStyle}", screenCaptureButtonsOutlineStyle);
  ...
  return {
    ...
    shouldProcessScreenCaptureAsComponent:
      shouldProcessScreenCaptureAsComponent,
...
window.KAMPYLE_ONSITE_SDK = (function (window, document) {
  function triggerSdkError(method) {
    var eventData = {
      errorMessage:
        KAMPYLE_CONSTANT.LOG_MESSAGES.ERROR_ONSITE_SDK_CONFIGURATION_MISSING.replace(
          "{{METHOD}}",
          method
        ),
    };
    ...
    switch (value) {
      ...
      case KAMPYLE_CONSTANT.CLOSE_FORM_PARAM_VALUES.EMBEDDED:
        return function (form) {
          return (
            form &&
            form.formType === KAMPYLE_CONSTANT.CLOSE_FORM_PARAM_VALUES.EMBEDDED
          );
        };
  ...
  function closeForm(value) {
    ...
    var allForms = KAMPYLE_DATA.getAllForms();
    var conditionFunc =
      value && getCloseFormByConditionFunc(value.toLowerCase());
    var len = allForms.length,
      curr,
      result = false;
    for (var i = 0; i < len; i++) {
  ...
  function isSurveyDisplayed() {
    ...
    if (
      KAMPYLE_FUNC.isAnyInviteShown() ||
      KAMPYLE_FUNC.isAnyOtherFormAlreadyShown()
    ) {
      return true;
    }
  ...
  function startInviteTargeting() {
    ...
    if (
      !KAMPYLE_DATA.isFeatureEnabled(
        KAMPYLE_CONSTANT.PROPERTY_SETTINGS.MANUALLY_INVITE_TARGETING
      )
    ) {
      KAMPYLE_UTILS.showWarning(
        "Manually invite targeting feature is disabled"
      );
      return false;
    }
    KAMPYLE_EVENT_DISPATCHER.trigger(
      KAMPYLE_CONSTANT.EVENTS.MDIGITAL_MANUALLY_INVITE_TARGETING,
      { skipButtonTargeting: true, skipInviteTargeting: false }
    );
  }
  ...
  function stopLogger() {
    KAMPYLE_DATA.deleteData("mdLogger");
  }
  function printMdigitalLogs() {
    MDIGITAL.LOGGER.printLogs();
  }
  function collectIntegrationData(integrationName) {
    var integration = KAMPYLE_INTEGRATION.getIntegrationData(integrationName);
    if (!integration) {
      console.log("no integration data!");
      return;
    }
    var messageData = {
      action: "updateIntegrationData",
      integrations: integration,
      __isDigitalAction: true,
    };
    KAMPYLE_MESSAGE.updateMessageForAllLoadedForms(messageData);
    return integration;
  }
  return {
    loadForm: loadForm,
    ...
    collectIntegrationData: collectIntegrationData,
...
window.KAMPYLE_INTEGRATION = (function () {
  ...
  function getParameterBySource(getParamsBySourceParameters) {
    switch (getParamsBySourceParameters.source) {
      case "var":
        return KAMPYLE_UTILS.getNestedPropertyValue(
          window,
          getParamsBySourceParameters.param
        );
      case "cookie":
        return KAMPYLE_UTILS.kampyleGetCookie(
          getParamsBySourceParameters.param
      ...
      case "function":
        var func = KAMPYLE_UTILS.getNestedPropertyValue(
          window,
          getParamsBySourceParameters.param
        );
        if (typeof func === "function") {
          try {
            ...
            if (predefinedArgs) {
              ...
              return predefinedArgs instanceof Array
                ? func.apply(
                    getParamsBySourceParameters.context,
                    predefinedArgs
                  )
                : null;
            } else {
      ...
      case "url":
        return KAMPYLE_UTILS.getUrlParam(
          KAMPYLE_UTILS.getNestedPropertyValue(
            window,
            getParamsBySourceParameters.param
          )
  ...
  function isEnabled(integrationData) {
    ...
    var integrationContext = KAMPYLE_UTILS.getNestedPropertyValue(
      window,
      contextParam
    );
  ...
  function prepareIntegrationData(integrationData) {
    ...
    var contextParam =
      INTEGRATION_CONTEXT[integrationData.integrationName] || null;
    var integrationContext = KAMPYLE_UTILS.getNestedPropertyValue(
      window,
      contextParam
    );
  ...
  function getIntegrationData(integrationName) {
    ...
    for (var integrationType in integrations) {
      if (integrations.hasOwnProperty(integrationType)) {
        var currIntegrationTypeList = integrations[integrationType] || [];
        for (var i = 0; i < currIntegrationTypeList.length; i++) {
          currIntegration = currIntegrationTypeList[i];
          if (isEnabled(currIntegration)) {
            if (integrationName) {
              if (currIntegration.integrationName === integrationName) {
                integrationData[integrationType] = [
                  prepareIntegrationData(currIntegration),
                ];
              ...
              integrationData[integrationType] =
                integrationData[integrationType] || [];
              integrationData[integrationType].push(
                prepareIntegrationData(currIntegration)
              );
            }
          }
...
window.MDIGITAL.CUSTOM_EVENTS = (function () {
  ...
  var customEvents = {
    ...
    MDigital_ThankYou_Close: {
      ...
      payload: [FORM_ID, FORM_TRIGGER_TYPE].concat(
        !KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.BACKEND_GENERATED_UUID
        )
          ? FEEDBACK_CORRELATION_UUID
          : FEEDBACK_UUID
      ),
    },
    neb_inviteAccepted: {
  ...
  var submitEventKey = KAMPYLE_FUNC.checkProvision(
    KAMPYLE_CONSTANT.PROVISIONS.DISABLE_EXTRA_FEEDBACK_SUBMIT_EVENT
  )
    ? "neb_feedbackSubmitted"
    : "neb_submitFeedbackSuccess";
  customEvents[submitEventKey] = {
  ...
  var dataToSendMapping = {
    ...
    feedbackCorrelationUUID: {
      exposedProperty:
        KAMPYLE_CONSTANT.EXPOSED_FEEDBACK_KEYS.FEEDBACK_UUID_LABEL,
    },
    feedbackUUID: {
      exposedProperty:
        KAMPYLE_CONSTANT.EXPOSED_FEEDBACK_KEYS.FEEDBACK_UUID_LABEL,
    ...
    content: {
      exposedProperty:
        KAMPYLE_CONSTANT.EXPOSED_FEEDBACK_KEYS.FEEDBACK_CONTENT_LABEL,
      manipulateValueMethod: null,
    },
    formLanguage: {
      exposedProperty:
        KAMPYLE_CONSTANT.EXPOSED_FEEDBACK_KEYS.FORM_LANGUAGE_LABEL,
      manipulateValueMethod: null,
    },
  };
  function init() {
    if (
      KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.DECIBEL_INSIGHT)
    ) {
      window.dxa_digital_listener_disabled = true;
    }
    try {
  ...
  function formatData(eventName, eventDispatcherData) {
    ...
    eventDispatcherData.formTriggerType = KAMPYLE_DATA.getFormTriggerType(
      eventDispatcherData.formId
    );
    return eventDispatcherData;
  }
  function triggerCustomEvent(eventName, eventDispatcherData) {
    var eventDispatcherDataFormatted = formatData(
      eventName,
      eventDispatcherData
    );
    var customEventData = getCustomEventDataToSend(
      eventDispatcherDataFormatted
    );
    if (customEventData) {
      ...
      if (
        ...
        triggerCustomCallback(customEventId, customEventData);
      }
      if (
        KAMPYLE_FUNC.checkProvision(
          KAMPYLE_CONSTANT.PROVISIONS.ENABLE_EVENTS_FOR_CE_DEBUG
        )
      ) {
        if (
          window.MDIGITAL.ANALYTICS &&
          ...
          MDIGITAL.ANALYTICS.triggerDigitalAnalyticsEvent(
            KAMPYLE_CONSTANT.EVENTS.CUSTOM_EVENTS_DEBUG_MESSAGE,
            { customEventId: customEventId, customEventData: customEventData }
          );
        }
      }
    }
    ...
    for (var i = 0; i < eventDispatcherDataFormatted.payload.length; i++) {
      ...
      if (payloadValue !== undefined && payloadValue !== null) {
        ...
        if (mappingObject && mappingObject.exposedProperty) {
          dataToSend[mappingObject.exposedProperty] =
            mappingObject.manipulateValueMethod
              ? mappingObject.manipulateValueMethod.apply(this, [payloadValue])
              : payloadValue;
        } else {
  ...
  function manipulateTriggerTypeValue(formTriggerType) {
    return (
      KAMPYLE_CONSTANT.FORM_TRIGGER_TYPES_MAP[formTriggerType] ||
      KAMPYLE_CONSTANT.CUSTOM_EVENT_OTHER
    );
  }
  ...
  function triggerCustomCallback(customEventName, customEventData) {
    function dxa_ready() {
      window.dxa_digital_integration &&
        dxa_digital_integration(customEventName, customEventData);
    }
    if (typeof window.decibelInsight === "function") {
      window.decibelInsight("ready", dxa_ready);
    } else {
      window._da_readyArray = window._da_readyArray || [];
      window._da_readyArray.push(dxa_ready);
    }
  }
  return {
    init: init,
...
window.MDIGITAL.CUSTOM_PARAMETERS = (function (window, document) {
  function getCustomParamValue(customParam) {
    if (
      !KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.CP_EXT_REFACTOR)
    ) {
      return KAMPYLE_FUNC.getSafeCustomParamVal(customParam);
    }
    var cp = customParam || {};
    var extractedValue = MDIGITAL.CUSTOM_PARAMETERS.extractCPValue(cp);
    return MDIGITAL.CUSTOM_PARAMETERS.castCPValue(
      extractedValue,
      cp.type || ""
    );
  }
  function formatCustomParams(formId) {
    var customParams =
      MDIGITAL.CUSTOM_PARAMETERS.getFormCustomParams(formId) || [];
  ...
  function hasCustomParams(formId) {
    var formData = KAMPYLE_DATA.getFormData(formId);
    return (
      !!formData && !!formData.customParams && !!formData.customParams.length
    );
  }
...
window.MDIGITAL.INIT = (function (window, document) {
  ...
  function init() {
    ...
    var isExtension = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.EXTENSION_ENABLED
    );
    if (isDHH || isExtension) {
      if (!onsiteConfiguration.onsiteDataUrl) {
        ...
        MDIGITAL.CONFIGURATION.fetchOnsiteExternalConfiguration(
      ...
      var formattedData =
        MDIGITAL.CONFIGURATION.formatConfiguration(onsiteConfiguration);
  ...
  function initBaseModules() {
    ...
    MDIGITAL.ANALYTICS.initTracker();
    if (!window.KAMPYLE_EMBED) {
      KAMPYLE_EVENT_DISPATCHER.trigger(
        KAMPYLE_CONSTANT.EVENTS.DIRECT_GENERIC_INJECTION
      );
    }
  }
  function initOnsite(onsiteConfiguration) {
    ...
    MDIGITAL.ANALYTICS.registerConfigurationDependentEvents();
    var isDHH = KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.DHH);
    var isExtension = KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.EXTENSION_ENABLED
    );
    if (isDHH || isExtension) {
      MDIGITAL.ANALYTICS.updateTrackerAnalyticsUrl();
    }
    KAMPYLE_DATA.initOnsiteDefaultState();
    KAMPYLE_FUNC.init();
    sendDataToExtension();
  ...
  function sendDataToExtension() {
    if (
      KAMPYLE_FUNC.checkProvision(KAMPYLE_CONSTANT.PROVISIONS.EXTENSION_ENABLED)
    ) {
      window.postMessage(
        {
          action: "sendMdExtensionData",
          propertyId: KAMPYLE_DATA.getPropertyId(),
          __isDigitalAction: true,
        },
...
window.MDIGITAL.MODAL_DIALOGS = (function () {
  ...
  function onFormShown(eventName, payload) {
    ...
    if (KAMPYLE_UTILS.isModalFormType(formId)) {
      setModalState({
        formId: formId,
        focusedElement: isEventOccuredOnCurrentShownModal(formId)
          ? currentShownModal.focusedElement
          : document.activeElement,
        targetModalId: KAMPYLE_DATA.getFormIframeId(formId),
        isNoJsonModal: false,
        isPreview:
          formId != null && KAMPYLE_UTILS.isFormInPreviewContext(formId),
      });
      setDialogAriaAttributesOnForm(formId);
      if (
        KAMPYLE_UTILS.isSafari() ||
        (KAMPYLE_DATA.isCustomHtmlForm(formId) &&
          !KAMPYLE_FUNC.checkProvision(
            KAMPYLE_CONSTANT.PROVISIONS.IS_CHTML_HANDLE_INIT_FOCUS_INDEPENDENTLY
          ))
      ) {
  ...
  function onNoJsonModalShown(eventName, payload) {
    setModalState({
      ...
      focusedElement: document.activeElement,
      targetModalId:
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.OOPS.CONTAINER_ID_PREFIX +
        payload.formId,
      isNoJsonModal: true,
      isPreview: false,
    });
  ...
  function trapFocus() {
    ...
    try {
      rootWindow.addEventListener(
        KAMPYLE_CONSTANT.BROWSER_EVENTS.FOCUS,
        handleTrapFocus,
        true
      );
    } catch (e) {
      handleRootWindowError(e);
    }
  }
  function releaseFocus() {
    if (currentShownModal.isPreview) return;
    try {
      rootWindow.removeEventListener(
        KAMPYLE_CONSTANT.BROWSER_EVENTS.FOCUS,
        handleTrapFocus,
        true
      );
  ...
  function handleRootWindowError(e) {
    if (
      window.MDIGITAL.ANALYTICS &&
      KAMPYLE_FUNC.checkProvision(
        KAMPYLE_CONSTANT.PROVISIONS.ENABLE_DIGITAL_ANALYTICS
  ...
  function focusOnOopsCloseButton() {
    var closeButton = document.querySelector(
      "#" +
        currentShownModal.targetModalId +
        " #" +
        KAMPYLE_CONSTANT.IFRAME_SETTINGS.OOPS.CLOSE_BUTTON_ID
    );
  ...
  function setDialogAriaAttributesOnForm(formId) {
    ...
    removeDialogAriaAttributesLightBox();
    setDialogAriaAttributesLightBox(
      Object.assign(ariaAttrs, { lang: langCode })
    );
  }
  ...
  function setDialogAriaAttributesLightBox(attributes) {
    MDIGITAL.MODAL_DIALOGS.setDialogAriaAttributes(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.LIGHTBOX.CONTAINER_ID_PREFIX,
      attributes
    );
  }
  function setDialogAriaAttributesInvite(attributes) {
    ...
    MDIGITAL.MODAL_DIALOGS.setDialogAriaAttributes(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.INVITATION.CONTAINER_ID,
      attributes
    );
  }
  ...
  function getPropertyMessageLanguageCode() {
    return KAMPYLE_FUNC.checkProvision(
      KAMPYLE_CONSTANT.PROVISIONS.PROPERTY_MESSAGES
    )
      ? MDIGITAL.PROPERTY_MESSAGES.getPropertyMessageLanguageCode()
      : null;
  }
  function getFormLanguageCode(formId) {
    return MDIGITAL.LOCALIZATION.getFormLanguage(formId);
  }
  ...
  function getFormAnnouncement(formId) {
    ...
    if (
      ...
      title = KAMPYLE_DATA.getMemoryJsonNestedPropertyValue(
        formId,
        "settings.formBasicSettings.formAnnouncement",
        false
      );
    }
  ...
  function setLocalizedAnnouncementOnInvite(formId) {
    var iFrame = document.getElementById(
      KAMPYLE_CONSTANT.IFRAME_SETTINGS.INVITATION.IFRAME_ID
    );
    if (iFrame != null) {
